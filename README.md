...
- フォームは TabControl による 9 タブ構成: **人物** / **企業** / **キャラクター** / **声優キャスティング** / **役職** / **シリーズ書式上書き** / **エピソード主題歌** / **シリーズ種別** / **パート種別**。各タブに DataGridView + 編集パネル + 「新規」「保存」「削除」ボタンを配置（既存 `MastersEditorForm` と同じ操作流儀）。
- 監査列（CreatedAt / UpdatedAt / CreatedBy / UpdatedBy）は全グリッドで自動非表示にする `HideAuditColumns` ヘルパを内蔵。
- クレジット本体（カード / ブロック / エントリ）の DnD 編集 UI は v1.2.1 で別途実装予定（v1.2.0 の段階ではマスタ群の整備に集中し、クレジット本体はリポジトリ層 + DB 整合性トリガーで安全に直書きできる状態まで）。

...
--
-- Table structure for table `movie_bgm_cues`
-- 映画作品の BGM リスト 1 行 = 1 キュー。bgm_cues（TV シリーズのセッション制・劇伴専用）
-- とは概念が異なるため別テーブルとして独立させる。映画にはセッションやパートの
-- 概念が無く、その映画固有の M ナンバー文字列・順序（seq）・サブ順序（sub_seq）と、
-- そのキュー自体が何か（劇伴／歌／その他＝tracks と共通の track_content_kinds 区分）
-- のみを持つ。さらに映画 BGM 特有の「未使用（音源は存在するが本編未使用）」と
-- 「欠番（そもそも制作されていない）」の概念を独立 2 フラグで保持する。
-- m_no は欠番では値が無いため NULL/空を許容し、自然キーにはせず代理キーを主キーにする。
-- series_id は映画系シリーズ（kind_code が MOVIE / MOVIE_SHORT / SPRING / EVENT の
-- いずれか）のみを許容する。MySQL の CHECK は他テーブル（series）を参照できないため、
-- この種別制約は BEFORE INSERT / BEFORE UPDATE トリガーで担保する（tracks 等と同じ流儀）。
--

DROP TABLE IF EXISTS `movie_bgm_cues`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `movie_bgm_cues` (
  `movie_bgm_cue_id` int NOT NULL AUTO_INCREMENT,
  `series_id` int NOT NULL,
  -- 映画内での並び順。0 は新規追加直後の暫定値（bgm_cues.seq_in_session と同じ運用）。
  `seq` int NOT NULL DEFAULT 0,
  -- サブ順序（同一 seq 内での枝番。組曲やバリエーション違いの並び等に使う）。
  `sub_seq` int NOT NULL DEFAULT 0,
  -- その映画固有の M ナンバー文字列（例: "M01", "M14B"）。欠番では値が無いため NULL 可。
  `m_no` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL,
  -- このキュー自体が何か。tracks と共通の track_content_kinds を参照（SONG/BGM/.../OTHER）。
  -- 既定は劇伴 'BGM'。
  `content_kind_code` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT 'BGM',
  -- 曲名・メニュー表記等（任意）。
  `title` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_ja_0900_as_cs_ks DEFAULT NULL,
  `notes` text CHARACTER SET utf8mb4 COLLATE utf8mb4_ja_0900_as_cs_ks,
  -- 未使用フラグ：音源は存在するが本編では使われていないキュー。
  `is_unused` tinyint NOT NULL DEFAULT 0,
  -- 欠番フラグ：番号としては存在するがそもそも制作されていない（音源が存在しない）。
  ...
  -- 未使用と欠番は両立しない（音源があるのに存在しない、は矛盾）。排他を CHECK で担保。
  CONSTRAINT `ck_movie_bgm_cues_unused_missing_exclusive` CHECK (NOT (`is_unused` = 1 AND `is_missing` = 1))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- movie_bgm_cues の series_id は映画系シリーズ限定。MySQL の CHECK は他テーブルを
-- 参照できないため、INSERT / UPDATE 時に series.kind_code を検査するトリガーで担保する。
-- 許容 kind: MOVIE / MOVIE_SHORT / SPRING / EVENT。
--
...
--   1 つの song_recordings が「歌として収録された」のと同時に「劇伴としても扱う」
--   ケースを表現する。tracks 本体は content_kind_code='SONG' / 'BGM' を排他選択する
...
-- song_part_variants マスタに sentinel 行 '_ANY' を追加（パート区別なく適用するため）。
-- 実パート（VOCAL / INST / KARAOKE / 等）と並べて本テーブル専用の人工値として配置する。
-- name_ja は (指定なし)、display_order は NULL（既存パートの順序に割り込ませない）。
...
-- tracks.content_kind_code の SONG → 他種別 変更を拒否するトリガー。
...
--
-- 書籍ジャンルマスタ。product_kinds と同じ流儀（コード PK + 和名 / 英名 / 表示順）。
-- 1 冊が複数ジャンルを持てるため、書籍との対応は book_genre_links（多対多）側で表現する。
--
...
INSERT INTO `book_genres` (`genre_code`,`name_ja`,`name_en`,`display_order`) VALUES
...
--
-- Table structure for table `book_credit_roles`
--
-- 書籍役職マスタ。アニメクレジットの roles とは別系統（roles に混ぜると
-- /creators/roles/ の集計に書籍役職が混入するため）。
-- amazon_role_type は Creators API の contributors[].roleType（"author" / "editor" 等）
-- からの自動マッピング用。表示名 role（"著" / "編集"）より安定しているためこちらで引く。
--

DROP TABLE IF EXISTS `book_credit_roles`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `book_credit_roles` (
  `role_code`        varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL,
  `name_ja`          varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_ja_0900_as_cs_ks NOT NULL,
  `name_en`          varchar(64) DEFAULT NULL,
...
--
-- Dumping data for table `book_credit_roles`
--
...
-- 書籍には品番に相当する自然キーが無い（ISBN は紙のみで Kindle 版には無い）ため代理キー PK。
--
-- release_date は代表発売日（紙があれば紙の発売日、電子のみの本は配信日）。
...
CREATE TABLE `books` (
  ...
  CONSTRAINT `fk_books_publisher_pc` FOREIGN KEY (`publisher_product_company_id`) REFERENCES `product_companies` (`product_company_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
--
-- 書籍 ↔ シリーズ（多対多）。1 冊が複数シリーズにまたがる合同本・オールスターズ本に対応する。
...
--
-- Table structure for table `book_genre_links`
--
-- 書籍 ↔ ジャンル（多対多）。「ムック かつ 設定資料集」のような複数ジャンルに対応する。
...
--
-- Table structure for table `book_credits`
--
-- 書籍クレジット。person_alias_id（マスタ紐付け）と credit_text（フリーテキスト）の併用可で、
-- どちらか一方は必ず入る（ck_book_credits_alias_or_text）。両方入っている場合は
-- 「マスタに紐付いているが誌面の表記が別」を意味し、表示は credit_text、リンク先は alias に取る。
-- サイト表示では alias 紐付け有り = <a>（hover 下線）、テキストのみ = <span>（下線なし）。
--
-- person_alias_id の FK に参照アクションを付けないのは MySQL 8 の制限による
-- （参照アクションで書き換わる列は CHECK 制約に含められない）。alias_id は不変の
-- AUTO_INCREMENT PK で、名義の削除も is_deleted による論理削除運用のため既定の RESTRICT で足りる。
--

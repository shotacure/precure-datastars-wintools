...
public sealed class MovieBgmCue
{
    ...
    /// <summary>
    /// 映画内での並び順。0 は新規追加直後の暫定値で、Catalog 側の編集画面で
    /// 確定値を振る運用（bgm_cues.SeqInSession と同じ思想）。
    /// </summary>
    public int Seq { get; set; }

    /// <summary>
    /// サブ順序（同一 <see cref="Seq"/> 内での枝番。組曲やバリエーション違いの
    /// 並び等に使う）。既定 0。
    /// </summary>
    public int SubSeq { get; set; }

    /// <summary>
    /// その映画固有の M ナンバー文字列（例: "M01", "M14B"）。
    /// 欠番（<see cref="IsMissing"/>）では値が無いため NULL を許容する。
    /// </summary>
    public string? MNo { get; set; }

    /// <summary>
    /// このキュー自体が何か。tracks と共通の track_content_kinds（→ kind_code）を
    /// 参照する（SONG / BGM / DRAMA / RADIO / LIVE / TIE_UP / OTHER）。既定は劇伴 "BGM"。
    /// </summary>
    ...
    /// <see cref="IsMissing"/> とは両立しない（DB 側 CHECK 制約で排他）。
    /// </summary>
    public bool IsUnused { get; set; }

    /// <summary>
    /// 欠番フラグ。番号としては存在するがそもそも制作されていない
    /// （音源が存在しない）。<see cref="IsUnused"/> とは両立しない。
    /// </summary>

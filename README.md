...
| **PrecureDataStars.BDAnalyzer** | WinForms GUI | Blu-ray (.mpls) / DVD (.IFO) のチャプター情報を解析し、各章の尺・累積時間を表示。ディスク挿入の自動検知対応。DB 連携パネルで既存ディスクとの照合・新規商品登録が可能。 |
...
# v1.1.0 → v1.1.1（series_id を products から discs へ移設）
...
スクリプトは `INFORMATION_SCHEMA` で各オブジェクトの存在を確認してから ALTER するため、再実行しても安全です。
...
### v1.1.1 — series_id の所在を products から discs へ移設

**破壊的変更あり**（アプリ・DB 双方にスキーマ変更あり）。
...
- **マイグレーション**: `db/migrations/v1.1.1_move_series_id_to_disc.sql`（v1.1.0 → v1.1.1 の冪等差分 SQL）
- **モデル**:

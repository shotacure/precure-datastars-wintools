...
> **最新 v1.3.7** — コード／README/CSS の縮約に加え、統計ページの脱テーブル刷新・「シリーズ別 TOP5 漢字」「記号出現回数・初使用エピソード」等の新規/改題ページ追加・`/creators/staff/` クリーンアップ・シリーズ詳細のセクション並び替え・CD アナライザ照合の安全化（集計ロジック・順位規則・DB・スキーマは不変）。各機能の詳細仕様は本文の該当章を、全バージョンの変更履歴は [`CHANGELOG.md`](CHANGELOG.md) を参照してください。

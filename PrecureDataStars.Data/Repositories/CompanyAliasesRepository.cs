...
public sealed class CompanyAliasesRepository
{
    ...
    /// <summary>名寄せ「企業屋号の付け替え」を 1 トランザクションで実行する。 指定の alias を、別企業（<paramref name="newCompanyId"/>）に紐付け直す。 親企業（<c>companies</c>）の表示名には一切手を加えず、結合だけを動かす。 切り離されて孤立した（=有効な alias を 1 つも持たなくなった）旧企業は自動で論理削除する。</summary>

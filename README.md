...
# v1.2.4 → v1.3.0：Web 公開用静的サイトジェネレータ追加にともなう 6 本のマイグレ
#                  以下、適用順は任意（互いに依存していない）。
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_add_episodes_duration_minutes.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_add_bgm_cues_seq_in_session.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_rename_episode_theme_songs_seq.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_seed_theme_song_roles.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_add_episode_uses.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_add_roles_successor.sql

# v1.3.0 ブラッシュアップ stage 15d：役職系譜の関係テーブル化
#   旧 roles.successor_role_code 列を撤去し、role_successions 多対多関係テーブルに置換。
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_role_successions_table.sql

# v1.3.0 ブラッシュアップ stage 16：音楽系クレジットの role_code 統一 + 主題歌使用実態フラグ
#   stage 16 では構造化音楽クレジットの credit_role を roles マスタ駆動に揃え、
#   さらに主題歌の使用実態（クレジットと実放送の乖離）を表現できるようにする。
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_song_credits_role_to_varchar.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_bgm_cue_credits_role_to_varchar.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_song_recording_singers_add_role.sql
mysql -u YOUR_USER -p precure_datastars < db/migrations/v1.3.0_episode_theme_songs_usage_actuality.sql
```
...
**v1.3.0 マイグレーションで実施される変更**:

v1.3.0 では Web 公開用静的サイトジェネレータの新設にともなう DB 改修と、その後のブラッシュアップ作業（stage 15d の役職系譜関係テーブル化、stage 16 の音楽系クレジット role_code 統一・主題歌使用実態フラグ追加）を一連のマイグレとして提供する。各マイグレは互いに依存していないため、適用順は任意。

*`v1.3.0_add_episodes_duration_minutes.sql`:*

`episodes.duration_minutes TINYINT UNSIGNED NULL` を `on_air_at` の直後に追加。既存 TV エピソード全件に `30` をバックフィル（マイグレーション済 TV 作品の標準尺）。SiteBuilder のエピソード詳細ページで放送日時を「2004年2月1日 8:30〜9:00」フォーマットに展開するために使用する（尺未登録時は終了時刻を省略）。`INFORMATION_SCHEMA.COLUMNS` で列の存在を確認してから ALTER する冪等スクリプト。

*`v1.3.0_add_bgm_cues_seq_in_session.sql`:*

`bgm_cues.seq_in_session TINYINT UNSIGNED NULL` を追加。録音セッション内での収録順を保持するための列で、SiteBuilder の劇伴詳細ページや音楽カタログ表示で「セッション内の収録順」をソートキーとして使う。

*`v1.3.0_rename_episode_theme_songs_seq.sql`:*

`episode_theme_songs.insert_seq` 列を `seq` にリネーム。旧仕様では「OP/ED は 0 固定 / INSERT は 1〜n」という排他ルール（`ck_ets_op_ed_no_insert_seq` CHECK 制約）があったが、新仕様では `seq` を「エピソード内での劇中順（1, 2, 3, ...）」を表す汎用カラムとして再設計（OP が冒頭にあるとは限らない作品にも対応）。CHECK 制約は撤廃。データ移行は典型的な順序（OP→1, INSERT→2..N, ED→末尾）で再採番する。

*`v1.3.0_seed_theme_song_roles.sql`:*

`roles` マスタに主題歌系の 5 役職をシード投入する。

| role_code | name_ja | name_en | format_kind |
|---|---|---|---|
| `LYRICS` | 作詞 | Lyrics | `THEME_SONG` |
| `COMPOSITION` | 作曲 | Composition | `THEME_SONG` |
| `ARRANGEMENT` | 編曲 | Arrangement | `THEME_SONG` |
| `VOCALS` | うた | Vocals | `THEME_SONG` |
| `LABEL` | レーベル | Label | `THEME_SONG` |

`INSERT IGNORE` で投入するため、既に存在する場合は何もしない（冪等）。本マイグレは stage 16 の音楽系クレジット role_code 統一マイグレ（`v1.3.0_song_credits_role_to_varchar.sql` など）の前提条件となる。

*`v1.3.0_add_episode_uses.sql`:*

`episode_uses` テーブルを新設し、エピソードの各パート内で流れた歌・劇伴・ドラマパート等の使用記録を一元管理する。SONG / BGM / DRAMA / RADIO / JINGLE / OTHER の 6 種別を `content_kind_code` で識別し、それぞれの参照列（`song_recording_id` / `bgm_series_id`+`bgm_m_no_detail` / `use_title_override` 等）を切り替える。整合性は `tracks` と同じ流儀で `BEFORE INSERT` / `BEFORE UPDATE` トリガで担保する。複合 PK は `(episode_id, part_kind, use_order, sub_order)` で、メドレー的に複数曲が連続するケースも `sub_order` で表現可能。

*`v1.3.0_add_roles_successor.sql`（旧版）:*

`roles.successor_role_code` 列を追加し、役職別ランキングを系譜統合する基盤として利用していたが、stage 15d で関係テーブル化（`role_successions`）に置き換えたため、本マイグレは現在は適用しても無害（列が後段の `v1.3.0_role_successions_table.sql` で撤去される）。

*`v1.3.0_role_successions_table.sql`（stage 15d）:*

`role_successions` 関係テーブルを新設し、`roles.successor_role_code` 列を撤去する。役職系譜は「from_role_code → to_role_code」の多対多関係として表現するように変更（旧 1 対多の単一後継参照モデルから、改名・統廃合・分裂・合流を全て表現可能な多対多モデルへ）。クラスタ（連結成分）の判定は SiteBuilder 側の `RoleSuccessorResolver` で Union-Find により行う。`from <> to` の自己参照禁止は MySQL 8 の Error 3823 で CHECK 制約にできず、Repository 層の `ArgumentException` ガードで担保する。

*`v1.3.0_song_credits_role_to_varchar.sql`（stage 16）:*

`song_credits.credit_role` を `enum('LYRICIST','COMPOSER','ARRANGER')` から `varchar(32) + roles マスタへの FK` に変更する。同時に値もリネーム：
...
役職追加が roles マスタの `INSERT` だけで済むようになり、ALTER TABLE 不要。役職リネーム（旧名 → 新名）も `ON UPDATE CASCADE` で自動追従する。実装は作業列 `credit_role_code` を ADD → 旧値リネームしながらコピー → PK DROP → 旧列 DROP → 作業列 RENAME → PK 再構成 → 補助インデックス追加 → roles FK 追加、の 8 ステップ。`INFORMATION_SCHEMA` で各ステップの状態を判定する PREPARE/EXECUTE 形式で冪等。途中失敗時の再実行にも対応。

⚠️ 既存環境でこのマイグレが Error 1553（Cannot drop index 'PRIMARY'）で止まる場合がある：`fk_song_credits_song`（`song_id` → `songs.song_id`）の依存先インデックスが PK 以外に存在しないとき、PK を DROP できない。本マイグレは PK DROP 前に補助インデックス `ix_song_credits_song (song_id)` を別途作成して FK の依存先を移すことでこれを回避する。

⚠️ MySQL Workbench の Safe Update Mode（既定オン）下では UPDATE が Error 1175 で弾かれる。本マイグレは冒頭で `SET SESSION SQL_SAFE_UPDATES = 0` を実行し、Workbench の Preferences を変更することなくそのまま流せるようにしている。

*`v1.3.0_bgm_cue_credits_role_to_varchar.sql`（stage 16）:*

`bgm_cue_credits.credit_role` を `enum('COMPOSER','ARRANGER')` から `varchar(32) + roles FK` に変更。値も `COMPOSITION` / `ARRANGEMENT` にリネーム。実装は `song_credits` 用と同じパターン（補助インデックス `ix_bgm_cue_credits_cue (series_id, m_no_detail)` を PK DROP 前に作成して FK 依存先を移す）。

*`v1.3.0_song_recording_singers_add_role.sql`（stage 16）:*

`song_recording_singers` に `role_code VARCHAR(32) NOT NULL DEFAULT 'VOCALS'` 列を追加し、PK を `(song_recording_id, singer_seq)` から `(song_recording_id, role_code, singer_seq)` の 3 列複合に変更する。`roles` マスタへの FK も追加。これにより録音に紐付く役職を「歌（VOCALS、既定）」だけでなく「コーラス（CHORUS）」等にも拡張可能になる。既存データは全て `VOCALS` 役職として埋まる（DEFAULT バックフィル）。

*`v1.3.0_episode_theme_songs_usage_actuality.sql`（stage 16）:*

`episode_theme_songs` に `usage_actuality ENUM('NORMAL','BROADCAST_NOT_CREDITED','CREDITED_NOT_BROADCAST') NOT NULL DEFAULT 'NORMAL'` 列を追加する。「クレジットされていないが実際には流れた」「クレジットされているが実際には流れていない」という乖離を表現する 3 値フラグ。値の意味と表示挙動は以下の通り：

| usage_actuality | 意味 | エピソード詳細 主題歌セクション | クレジットページ / 曲詳細 |
|---|---|---|---|
| `NORMAL` | クレジット通り、実際に流れた（既定） | 表示 | 表示 |
| `BROADCAST_NOT_CREDITED` | クレジットされていないが確かに流れた | 表示 | **非表示** |
| `CREDITED_NOT_BROADCAST` | クレジットされているが実際には流れていない | **非表示** | 表示（「実際には不使用」注記） |

`is_broadcast_only`（TV 放送版限定の主題歌差し替え）とは別軸の概念で、両者は組み合わせ可能。
...
| パス | 内容 |
|---|---|
| `/` | ホームページ。シリーズ一覧 + 7 つの動的セクション（今日の記念日・次回予告・最新エピソード・今週の記念日・間もなく発売・新着商品・データベース統計）。記念日計算は `wwwroot/assets/anniversaries.js` で JS 動的計算 |
| `/about/` | サイト案内 |
| `/series/` | 全シリーズ索引（TV / 映画・短編 / スピンオフの 3 セクション。TV を縦軸とした年表構造、`parent_series_id` で TV を親にしている秋映画 / 春映画 / 併映短編は字下げ表示。子作品の単独詳細ページは生成しない） |
| `/series/{slug}/` | シリーズ詳細。エピソード一覧（dl 形式）、メインスタッフ 5 役職、劇伴一覧表（M 番号 / メニュータイトル / 作曲 / 編曲 / 尺 / セッション / 使用回数の 7 列、`is_temp_m_no=1` の仮 M 番号は Web 非表示）、主要スタッフ表（脚本・絵コンテ・演出・作画監督・美術監督ら役職別） |
| `/series/{slug}/{seriesEpNo}/` | エピソード詳細（サイトの中核ページ）。サブタイトル（ルビ付き）、フォーマット表（OA / 配信(Amazon Prime) / Blu-ray/DVD のタイムコード）、サブタイトル文字情報（初出・唯一・「N年Mか月ぶり」）、文字統計、パート尺の偏差値、主題歌、使用音声、クレジット階層、前後話ナビ、YouTube 予告埋め込み |
...
v1.3.0 ブラッシュアップ stage 16 で `CreditInvolvementIndex` の集計対象に **音楽系の構造化クレジット 3 ソース**を追加：

1. **`song_credits`** — 主題歌の作詞 / 作曲 / 編曲の連名（`LYRICS` / `COMPOSITION` / `ARRANGEMENT`）。エピソード紐付けは `episode_theme_songs` 経由で解決
2. **`song_recording_singers`** — 録音に紐付く歌唱者連名（`VOCALS` / `CHORUS`）。`PERSON` は `person_alias_id` を、`CHARACTER_WITH_CV` は `voice_person_alias_id` を `ByPersonAlias` に、`character_alias_id` を `ByCharacterAlias` に登録。スラッシュ並列の slash_*_alias_id にも対応
3. **`bgm_cue_credits`** — 劇伴の作曲 / 編曲の連名（`COMPOSITION` / `ARRANGEMENT`）。エピソード紐付けは `episode_uses` 経由で解決（content_kind_code='BGM' でフィルタ）

`Involvement.CreditKind` には主題歌系で `"THEME_SONG"`、劇伴系で `"BGM"` を入れて既存の OP/ED と区別する。これにより人物詳細・キャラクター詳細・企業詳細の各ページから「主題歌の作詞担当としてこのエピソードに関与している」のような関与履歴が逆引きできるようになった。

`episode_theme_songs.usage_actuality` フラグ（v1.3.0 ブラッシュアップ stage 16 追加）に応じた集計分岐：

- `usage_actuality = 'BROADCAST_NOT_CREDITED'`（クレジットなしで流れた）→ クレジット集計対象外、エピソード詳細の主題歌セクションのみ表示
- `usage_actuality = 'CREDITED_NOT_BROADCAST'`（クレジットあって流れていない）→ クレジット集計対象に含める。曲詳細ページの「主題歌使用エピソード一覧」では「（実際には不使用）」注記付きで表示。エピソード詳細の主題歌セクションには出さない
...
| `v1.3.0_role_successions_table.sql`（**stage 15d**） | `role_successions` 関係テーブル新設＋`roles.successor_role_code` 撤去。役職系譜を「from→to」の多対多関係として表現。クラスタ判定は SiteBuilder 側で Union-Find により実装 |
| `v1.3.0_song_credits_role_to_varchar.sql`（**stage 16**） | `song_credits.credit_role` を `enum('LYRICIST','COMPOSER','ARRANGER')` から `varchar(32) + roles FK` に変更し、値を `LYRICS`/`COMPOSITION`/`ARRANGEMENT` にリネーム。役職追加が `roles` の INSERT だけで済むように。PK DROP 前に補助インデックス `ix_song_credits_song` を追加して FK 依存先を移すことで Error 1553 を回避。Workbench の Safe Update Mode 対策 (`SET SESSION SQL_SAFE_UPDATES = 0`) も内蔵 |
| `v1.3.0_bgm_cue_credits_role_to_varchar.sql`（**stage 16**） | `bgm_cue_credits.credit_role` を同様に `varchar(32) + roles FK` 化。値も `COMPOSITION`/`ARRANGEMENT` にリネーム |
| `v1.3.0_song_recording_singers_add_role.sql`（**stage 16**） | `song_recording_singers` に `role_code VARCHAR(32) NOT NULL DEFAULT 'VOCALS'` 列追加 + PK を `(song_recording_id, role_code, singer_seq)` に変更 + `roles` への FK 追加。録音に紐付く役職を VOCALS（既定）以外に CHORUS 等にも拡張可能に |
| `v1.3.0_episode_theme_songs_usage_actuality.sql`（**stage 16**） | `episode_theme_songs` に `usage_actuality ENUM('NORMAL','BROADCAST_NOT_CREDITED','CREDITED_NOT_BROADCAST')` 列追加。クレジットと実放送の乖離を 3 値で表現（クレジットなしで流れた / クレジットあって流れていない、を扱う） |
...
- フッターを連絡先・著作権・免責の 3 段構成に：`<section id="contact">`（X: @shota_ / mailto:shota@precure.tv）と `<div id="copyright">`（© Shota (SHOWTIME) + サークル『SHOWTIME』代表・祥太が個人で運営している旨と、ABCアニメーション様・東映アニメーション様その他各社様とは一切関係ございません旨）を縦に並べる新構造
- 話数ページネーションは「« #N サブタイトル / 1 … 10 11 [12] 13 14 … 50 / #N サブタイトル »」形式で上下両方に配置
...
- 楽曲のテキストフィールド（作詞・作曲・編曲・歌唱者の `_name` 系列）は楽曲詳細でそのまま表示するが、人物・企業の関与集計（`CreditInvolvementIndex`）には反映しない（マスタ駆動の堅実な紐付けに限定）。ただし v1.3.0 ブラッシュアップ stage 16 で `song_credits` / `song_recording_singers` / `bgm_cue_credits` の構造化クレジット行は集計に取り込まれるようになった（テキストフィールドは引き続き集計対象外）
...
**`song_recording_singers` テーブル新設**：1 録音に対する歌唱者連名を順序付きで保持。`billing_kind` が 2 値：
...
複合 PK は当初 `(song_recording_id, singer_seq)` の 2 列構成だったが、v1.3.0 ブラッシュアップ stage 16 で `role_code` 列を追加し `(song_recording_id, role_code, singer_seq)` の 3 列に拡張。`role_code` は `roles` マスタ（typically `VOCALS` / `CHORUS`）への FK で、録音に紐付く役職を歌（VOCALS、既定）以外にもコーラス（CHORUS）等に拡張可能。

既存の `SongRecording.SingerName` フリーテキスト列は温存しており、本テーブルに行が無い録音では従来通りフリーテキストが表示に使われる。

**`song_credits` テーブル新設**：歌の作詞・作曲・編曲のクレジット行を保持（連名対応）。`credit_role`（v1.3.0 ブラッシュアップ stage 16 以降は `roles.role_code` への FK で typically `LYRICS` / `COMPOSITION` / `ARRANGEMENT`）と `credit_seq`（同役職内の連名順）で複合ユニーク。当初は `enum('LYRICIST','COMPOSER','ARRANGER')` の固定 3 値だったが、stage 16 で varchar + roles マスタ駆動に統一した。

**`bgm_cue_credits` テーブル新設**：劇伴の作曲・編曲のクレジット行を保持（同上、stage 16 で `roles.role_code` への FK 化、値は typically `COMPOSITION` / `ARRANGEMENT`）。
...
**Draft セッション方式（全面メモリ化）**：ユーザーの操作は一旦メモリ上の Draft オブジェクトに反映され（カード追加・並べ替え・エントリ編集・DnD 移動など）、画面下部の「💾 保存」ボタン押下時に `CreditSaveService` が 4 フェーズ（削除→新規作成→更新→seq 整合性）を 1 トランザクション内で実行して DB に確定する。未保存中はツリー背景色が薄い黄色になりステータスバーに「★ 未保存の変更あり」が表示される。クレジット切替・シリーズ／エピソード切替・フォーム閉じ時には未保存変更がある場合の確認ダイアログ（保存して続行／破棄して続行／キャンセル）を出してデータロストを防ぐ。

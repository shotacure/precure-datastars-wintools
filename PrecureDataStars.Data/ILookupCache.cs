...
{
    /// <summary>人物名義 ID から表示名を解決する（<c>display_text_override</c> 優先、無ければ <c>name</c>）。 未ヒット時は <c>null</c>。</summary>
    Task<string?> LookupPersonAliasNameAsync(int aliasId);

    /// <summary>企業屋号 ID から表示名を解決する。未ヒット時は <c>null</c>。</summary>
    Task<string?> LookupCompanyAliasNameAsync(int aliasId);

    /// <summary>ロゴ ID から表示名を解決する。テンプレ展開時はロゴそのものではなく 「親屋号名」を表示するのが通常運用なので、戻り値は実装側の都合に従う。 未ヒット時は <c>null</c>。</summary>
    Task<string?> LookupLogoNameAsync(int logoId);

    /// <summary>人物名義 ID から「人物詳細ページへリンク化済みの HTML 断片」を返す。</summary>
    Task<string?> LookupPersonAliasHtmlAsync(int aliasId);

    /// <summary>企業屋号 ID から「企業詳細ページへリンク化済みの HTML 断片」を返す。</summary>
    Task<string?> LookupCompanyAliasHtmlAsync(int aliasId);

    /// <summary>ロゴ ID から「親屋号名 + 企業詳細リンク」相当のリンク化済み HTML 断片を返す。</summary>

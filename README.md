...
> **最新 v1.3.8** — ホーム「今月のカレンダー」のキャプションを刷新し、月名を約 2 倍に拡大して中央に配置、左右に前月／翌月送りボタンを追加（クライアント完結。表示データは月日ベースのため JSON・Generator・DB・スキーマは不変）。各機能の詳細仕様は本文の該当章を、全バージョンの変更履歴は [`CHANGELOG.md`](CHANGELOG.md) を参照してください。

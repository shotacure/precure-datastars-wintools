...
6. 役職層 2 表の作成: `roles`（NORMAL/SERIAL/THEME_SONG/VOICE_CAST/COMPANY_ONLY/LOGO_ONLY の 6 書式区分。原作・連載・脚本・演出・声の出演など 26 件を `INSERT IGNORE` で初期投入）／ `series_role_format_overrides`（シリーズ × 役職 × 期間で書式テンプレを上書き、PK に `valid_from` を含む）。
7. クレジット本体 4 段階の作成: `credits`（1 件 = 1 枚分のクレジット表示。シリーズ単位 or エピソード単位で OP/ED 各 1 件まで、`scope_kind` × `series_id`/`episode_id` の排他は **トリガーで担保**）／ `credit_cards`（クレジット内のカード 1 枚 = 1 行、`presentation` が `CARDS` か `ROLL`）／ `credit_card_roles`（カード内の役職、`tier` 1/2 の 2 段組 + `order_in_tier` でレイアウト）／ `credit_role_blocks`（役職下のブロック、`rows` × `cols` の枠と先頭企業名フィールド）／ `credit_block_entries`（ブロック内のエントリ、`entry_kind` PERSON/CHARACTER_VOICE/COMPANY/LOGO/SONG/TEXT で参照先列が切り替わる、整合性は **トリガーで担保**）。
...
- `characters` … キャラクターマスタ（全プリキュアを通じて統一管理する設計のため `series_id` を持たない）。`character_kind` は MAIN / SUPPORT / GUEST / MOB / OTHER の 5 区分。All Stars・春映画・コラボ等で複数シリーズに登場するキャラは同一 `character_id` を共有する。
- `character_aliases` … キャラクターの名義（"美墨なぎさ" / "キュアブラック" / "ブラック" のような表記揺れ）。
...
- `roles` … クレジット内の役職マスタ。`role_format_kind` で「この役職下のエントリは どのような書式・参照を取るか」を分類する 6 区分: NORMAL（普通の役職: 名義列）／ SERIAL（連載、`format_template` でシリーズ別表記を切り替え）／ THEME_SONG（主題歌、entry が song_recording と label を持つ）／ VOICE_CAST（声の出演、entry がキャラクター名義 + 人物名義のペア）／ COMPANY_ONLY（企業のみ）／ LOGO_ONLY（ロゴのみ）。原作・連載・脚本・演出・声の出演・制作著作など 26 件を初期投入する。
...
- `credit_card_roles` … カード内に登場する役職 1 つ = 1 行。`tier`=1（上段）/ 2（下段）+ `order_in_tier` でカード内のレイアウト位置を保持。`role_code` を NULL にできるのは「ブランクロール」用途（ロゴ単独表示の枠など）。
- `credit_role_blocks` … 役職下のブロック 1 つ = 1 行。多くは 1 役職 1 ブロック。`rows` × `cols` は表示の枠（左→右、行が埋まれば次の行）。`leading_company_alias_id` でブロック先頭に企業名を出すケースに対応。
- `credit_block_entries` … ブロック内のエントリ 1 つ = 1 行。`entry_kind` に応じて参照先カラムが決まる: PERSON / CHARACTER_VOICE（人物 + キャラクター）／ COMPANY / LOGO / SONG / TEXT（マスタ未登録のフリーテキスト退避口）。`affiliation_company_alias_id` / `affiliation_text` で人物名義の小カッコ所属、`parallel_with_entry_id` で「A / B」併記の自参照を表現可能。整合性はトリガーで担保。

**主題歌**

- `episode_theme_songs` … エピソード × 主題歌の紐付け。`theme_kind` は OP / ED / INSERT の 3 値、OP / ED は `insert_seq=0` の 1 行のみ、INSERT は `insert_seq=1, 2, ...` と複数可（CHECK で担保）。クレジットの THEME_SONG エントリはここから歌情報を引いてレンダリングする想定。
...
- **v1.2.0 工程 B' — 本放送限定フラグの導入とエピソード主題歌のコピー機能**: 同じエピソードでも本放送と Blu-ray・配信で異なる主題歌が流れる作品があるが、実態としては「OP もしくは ED のみ本放送だけ例外的に異なる」というケースが大半で、本放送・Blu-ray・配信の三者は基本的に同じ主題歌を共有する。この実態に合わせ、`episode_theme_songs` テーブルには `is_broadcast_only TINYINT(1)` フラグ列を追加し（既定 0 = 全媒体共通、1 = 本放送限定の例外行）、PK を `(episode_id, is_broadcast_only, theme_kind, insert_seq)` の 4 列複合に再構築。データの一貫性を保つため、`credits` テーブルにも同じ `is_broadcast_only` フラグを追加し、UNIQUE 2 本を `(series_id, is_broadcast_only, credit_kind)` および `(episode_id, is_broadcast_only, credit_kind)` の 3 列構成に拡張した。マイグレーション SQL は冪等（既存列・既存 PK/UNIQUE の構成を `INFORMATION_SCHEMA` で確認してから ALTER）で書かれており、過去に 4 値 ENUM の `release_context` 列を作っていた環境からも、既存 PK/UNIQUE の作り直しと旧列 DROP 経由で `is_broadcast_only` への移行が冪等に実行される。エピソード主題歌タブには「**他話からコピー...**」ボタンを追加し、専用ダイアログ `EpisodeThemeSongCopyDialog` で 3 段階の操作（[1] コピー元読み込み: 全媒体共通行 / 本放送限定行をチェックボックスで選択 → [2] コピー先範囲・本放送フラグ扱いの指定 → [3] プレビュー編集）を経て、最終的な「すべて保存」を押した時点で初めて `BulkUpsertAsync` がトランザクションで動く（プレビュー段階では DB は一切触らない）。クレジット編集フォーム左ペインも同様に「本放送限定行も表示」チェックボックスで本放送限定クレジットを必要に応じて可視化できる。
...
- クレジット本体（カード / ブロック / エントリ）の DnD 編集 UI は v1.2.0 後続工程で別途実装予定（マスタ群の整備までを工程 A で完了。後続工程で検索・オートコンプリート / クレジット本体編集 / 行 DnD 並べ替えを順次追加する）。

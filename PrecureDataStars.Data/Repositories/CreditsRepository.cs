...
/// <para>
/// シリーズ単位 or エピソード単位で OP/ED 各 1 件まで。scope_kind と series_id /
/// episode_id の整合性は DB 側 CHECK 制約 ck_credits_scope_consistency で担保。
/// </para>
/// </summary>
public sealed class CreditsRepository
{
    ...
    /// <summary>指定シリーズに紐付くクレジット（scope=SERIES）一覧を取得する。</summary>
    ...
    /// <summary>指定エピソードに紐付くクレジット（scope=EPISODE）一覧を取得する。</summary>
    ...
    /// <summary>更新（scope_kind / series_id / episode_id / credit_kind は変更しない用途を想定）。</summary>

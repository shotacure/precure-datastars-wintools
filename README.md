...
- **中央列**：サブタイトル
- **右列**：スタッフバッジ群を 1 行ぶんの font-size `0.7em` で密表示
...
##### `<li>` を block 化（古い 3 列 grid 設定の撤廃）

新テンプレ構造は `<li class="ep-row">` 直下に `<div class="ep-row-main">` を置き、その `.ep-row-main` を grid container として機能させる 2 階層構造。しかし CSS 側に旧仕様（`<li>` 自体を 3 列 grid `4.2em 1fr 13em` にして、番号・タイトル・放送日を直接子として並べる構造）の `.episodes-index-list li { display: grid; ... }` ルールが残っていたため、新テンプレでは：

- `<li>` が grid container（3 列 `4.2em 1fr 13em`）になる
- 唯一の子 `<div class="ep-row-main">` が grid の col 1（4.2em）に押し込まれる
- その狭い 4.2em の中で `.ep-row-main` 自身が `5em 1fr 1fr` の grid を組もうとして全列極限まで圧縮、タイトル文字が縦書きのように 1 文字ずつ折り返す事象が発生

`.episodes-index-list li` を `display: block` に変更して旧 grid 設定を撤廃。`<li>` は padding と border-bottom だけ持つ単純な block 要素に。古い CSS クラス（`.episodes-index-no` 等）はテンプレからは出力されなくなったが、CSS 定義はそのまま残して後方互換は維持。

##### タイトル列の最低幅保証

スタッフ列の中身が長いシリーズ（連名スタッフが多い・名前が長い等）でタイトル列が極端に潰れる事象を防ぐため、`grid-template-columns` を `5em minmax(0, 1fr) minmax(0, 1fr)` → `5em minmax(15em, 1fr) minmax(0, 1fr)` に変更。タイトル列に **最低 15em** の min-width を保証し、行の半分以上は常にタイトル領域として確保する。スタッフ列は `minmax(0, 1fr)` のままで、タイトル側を圧迫しない設計。
...
- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`（`.page-section-nav` 幅 220→340 / 左寄せ追従、`.episodes-index-list li` を `display: block` 化して旧 3 列 grid を撤廃、`.episodes-index-list .ep-row-main` 3 列グリッド（タイトル列に min 15em 保証）、`.ep-row-no-date` 縦積み、`.episodes-index-list .ep-row .staff-badges-row` 70% フォント、`.home-episode-list` を 4 要素対応の 2 段グリッドに再構成 + モバイル grid placement リセット、`.home-product-kind` nowrap 撤廃 + kind 列幅 8em→9em）
- `README.md`（本エントリ追加）

...
> **最新 v1.3.5** — （Stage 1）`persons` / `characters` に誕生日カラム（生年＋公開可否 `birth_year_visibility`＋月・日）を追加し、既存 `precures` の誕生日をキャラへバックフィルする冪等マイグレを同梱。（Stage 2）`precures` の誕生日カラムを撤去（バックフィル後の原子的削除）。（Stage 3）ホームに「今月のカレンダー」を新設し、「今日の記念日」を誕生日対応に拡張（キャラクター／人物誕生日をエピソードより上に表示）。あわせて（先行分）シリーズ一覧 TV サブ行のプリキュア表示をバッジ化。プリキュアマスタに地色カラム `key_color` を追加し、地色の相対輝度から文字色（暗/明グレー）を自動算出してどんな地色でも本文が読めるようにした。バッジ表記はキャラクター正式名称（`characters.name`）を優先し、声優を「(CV: ○○)」で後置。TV サブ行の「スタッフ」「プリキュア」見出しラベルを撤去し、プリキュアバッジ行を上・スタッフバッジ行を下に並べ替えた。各機能の詳細仕様は本文の該当章を参照。
>
...
| `/creators/staff/` | スタッフ一覧。役職順（既定）/ 五十音順 / 初参加順（シリーズ別セクション）/ 参加話数が多い順 の 4 タブ。役職順以外は人物と企業・団体を 1 リストに混在（個人/団体バッジ＋絞り込みトグル。トグルは役職順タブ非アクティブ時のみ表示）。表にシリーズ列は持たない。旧 `/persons/`・`/companies/` 索引、旧役職統計索引・総合集計の役割を統合 |
...
ホーム「今日の記念日」は v1.3.5 Stage 3 で**誕生日対応**に拡張した。埋め込み JSON `home-anniversary-data` は エピソード放送日に加えて、映画公開日・キャラクター誕生日・人物誕生日を 1 配列に種別タグ `k`（`ep` / `mv` / `cb` / `pb`）付きで内包する（プロパティ名は容量削減のため短縮形。`HomeGenerator.BuildCalendarDataJsonAsync`）。`anniversaries.js` は `k==='ep'` のみ従来ロジック（今日／今週）に流し、`cb` / `pb` のうち閲覧日と月日が一致するものを抽出して「今日の記念日」内でエピソード行より**上**に積む。キャラクター誕生日は対象を `character_kind` が `PRECURE` / `ALLY` のキャラに限定し、`characters.name` をシリーズ表記行とともに表示（プリキュアはキーカラーバッジを添える。バッジ文字色は地色の相対輝度から暗/明グレーを自動算出）。代表シリーズは「`series_precures` のうち放送開始が最も早いシリーズ」、代表 precure は「最小 `precure_id`」で決定的に解決する。人物誕生日は氏名リンクで、生年は `birth_year_visibility = 'PUBLIC'` かつ判明時のみ年齢を併記する。映画（`mv`）は今日の記念日には出さずカレンダー専用。

ホーム「今月のカレンダー」は v1.3.5 Stage 3 で新設した、閲覧した瞬間の当月 1 か月分を表示する JS 動的カレンダー（`calendar.js`、`home-anniversary-data` を `anniversaries.js` と共有）。前月／翌月ナビは持たず常に当月のみ。各日セルにその月日の項目をチップで縦に積み、優先順は**キャラクター誕生日 > 映画公開日 > 人物誕生日 > TV 放送**。プリキュア誕生日チップは変身前名義をキーカラーバッジで、`ALLY` は名称の素バッジで表示。映画はシリーズ略称、TV 放送は「シリーズ略称＋#話数」で表示する（カレンダー UI に限り `title_short` を用いるポリシー例外。前述）。本日セルはハイライトし、日曜・土曜は配色で区別。データ空・JS 無効環境では section ごと非表示。凡例を併記する。
...
記念日・カレンダーは「閲覧日の月日」と各エンティティの `birth_month` / `birth_day` の一致だけを見る。生年・年齢の表示は `birth_year_visibility = 'PUBLIC'` かつ `birth_year` 非 NULL のときのみ。既存 `precures` の誕生日（月日のみ）は `transform_alias_id → character_aliases.character_id` 経由で対応キャラへ移送する（生年は元々持たないため `birth_year` は `NULL`・`birth_year_visibility` は既定 `PUBLIC` のまま、情報欠落なし）。`precures.birth_month` / `birth_day` は v1.3.5 Stage 2 で撤去済み。ホームの「今月のカレンダー」「誕生日対応の今日の記念日」は v1.3.5 Stage 3 で実装済み（次章参照）。Catalog 編集 UI への誕生日入力欄移設は後続 Stage で実施する。コンパクトなカレンダー表示に限り `series.title_short` を用いる方針例外を設けた（本書「シリーズ系テーブル」の `title_short` 行に明記）。
...
各バージョンの詳細な変更履歴は [`CHANGELOG.md`](CHANGELOG.md) に移設しました。工程単位の詳細は Git のコミット履歴および GitHub のリリースノートを参照してください。

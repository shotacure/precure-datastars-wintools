...
public sealed class EpisodeUse
{
    ...
    /// <summary>同 <see cref="UseOrder"/> 内のサブ順。通常は 0、メドレー等で複数曲が連続するときに 1, 2, 3, ... と振る。</summary>
    public byte SubOrder { get; set; } = 0;

    // ── 内容種別と多態参照 ──

    /// <summary>内容種別コード（→ track_content_kinds）。</summary>
    ...
    /// <summary>SONG 時のサイズ種別コード（→ song_size_variants）。 フルサイズ・TVサイズ・映画サイズ等。SONG 以外では NULL。</summary>
    public string? SongSizeVariantCode { get; set; }

    /// <summary>SONG 時のパート種別コード（→ song_part_variants）。 通常歌入り・オリジナル・カラオケ・コーラス入り等。SONG 以外では NULL。</summary>
    public string? SongPartVariantCode { get; set; }

    /// <summary>BGM 時に参照する bgm_cues の第 1 列（シリーズ ID）。</summary>
    ...
    /// <summary>内容種別がテキスト系（DRAMA / RADIO / JINGLE / OTHER）のときの表示文字列。 SONG / BGM では用途外（参照列で実体を引くため）。</summary>

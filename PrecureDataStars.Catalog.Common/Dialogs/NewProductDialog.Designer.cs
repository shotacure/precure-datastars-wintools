...
partial class NewProductDialog
{
    ...
    // NumericUpDown から TextBox に変更。価格は数千円単位なので上下ボタンでの入力は非実用的、
    ...
    /// <summary>
    /// 税込価格表示用の読み取り専用 TextBox。
    /// </summary>
    private TextBox txtPriceInc = null!;
    private Label lblDiscCount = null!;
    private NumericUpDown numDiscCount = null!;

    // 「レーベル既定」「販売元既定」として、商品社名マスタ（product_companies）の既定フラグ社を
    // 取得して ReadOnly TextBox に表示する 2 行。
    ...
    private void InitializeComponent()
    {
        ...
        // 既定社表示の TextBox は読み取り専用（ユーザー編集不可）。

...
- `credit_card_roles` … カード内に登場する役職 1 つ = 1 行。`tier`=1（上段）/ 2（下段）+ `group_in_tier`（同 tier 内のサブグループ番号、1 始まり、v1.2.0 工程 E 追加）+ `order_in_group`（同グループ内の左右順、1 始まり、旧 `order_in_tier` から改名）でカード内のレイアウト位置を保持。サブグループが 1 個しかない（従来の 2 列構成と等価な）カードは `group_in_tier=1` だけを使う。`role_code` を NULL にできるのは「ブランクロール」用途（ロゴ単独表示の枠など）。

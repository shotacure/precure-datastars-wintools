...
/// <see cref="DisplayTextOverride"/> はユニット名義などで定形外の長い表示文字列が
/// 必要なケース用。非 NULL の場合、表示ロジックは <see cref="Name"/> よりこの値を
/// 優先する。例: name=「プリキュアシンガーズ+1」、override=「プリキュアシンガーズ+1(五條真由美、池田 彩、うちやえゆか、二場裕美)」。
/// ユニット名義の構成メンバー（連名の中身）は <c>person_alias_members</c> テーブルで
/// 管理する。本モデルから直接列としては保持しない
...
public sealed class PersonAlias
{
    ...
    /// <summary>名義の英語表記。英文クレジット出力で使用。</summary>
    public string? NameEn { get; set; }

    /// <summary>表示用の上書き文字列 NULL）。 非 NULL のとき、アプリ側の表示ロジックは <see cref="Name"/> ではなくこの値を使う。 通常のユニット名義（"Berryz工房" 等、表示が name で済むもの）では NULL のままにし、 "プリキュアシンガーズ+1(...)" のような定形外の長い表示テキストが必要なときだけ埋める。</summary>
    ...
    /// <summary>表示用文字列を返す。 <see cref="DisplayTextOverride"/> が非空ならそちらを、なければ <see cref="Name"/> を返す。</summary>

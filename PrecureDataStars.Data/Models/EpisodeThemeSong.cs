...
public sealed class EpisodeThemeSong
{
    ...
    /// <summary>本放送限定フラグ（PK 構成）。 false (0) = 本放送・Blu-ray・配信ともに共通（既定）。 true (1) = 本放送限定の例外行（同 episode/theme_kind の 0 行と並立する）。</summary>
    public bool IsBroadcastOnly { get; set; }

    /// <summary>主題歌区分（PK 構成、"OP"/"ED"/"INSERT"）。</summary>
    public string ThemeKind { get; set; } = "OP";

    /// <summary>エピソード内での劇中順（PK 構成、1, 2, 3, ...）。 OP/ED と INSERT を区別せず「劇中で流れる順序」を表す。OP が冒頭にあるとは 限らない作品にも対応するため、数値そのものに OP=1 / ED=末尾 のような決まりは無い。 同一 (episode_id, is_broadcast_only) 内では PK の一部としてユニーク。</summary>
    public byte Seq { get; set; }

    /// <summary>使用実態フラグ。</summary>
...
/// <summary>episode_theme_songs.usage_actuality の値を表す定数群。</summary>

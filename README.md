...
- `README.md`: 先頭バナーに v1.1.5 の Blu-ray 全走査機能を追記、プロジェクト詳細表の BDAnalyzer / CDAnalyzer 説明を更新、「音楽カタログ登録 → B. BD/DVD の登録」セクションの Blu-ray 行を全走査モードに対応する記述に書き換え + 「Blu-ray PLAYLIST フォルダ全走査の仕様」サブセクションを新設、本変更履歴エントリを (1) (2) のサブセクション構造に再編して Blu-ray 側の詳細を追記

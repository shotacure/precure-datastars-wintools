...
public sealed class CreditBulkApplyService
{
        ...
        string name, int lineNo, BulkParseResult parsed, CancellationToken ct)
    {
        ...
        string normalizedRaw = NameSimilarity.NormalizeForCompare(name);
        ...
        if (all.Count > 0 && normalizedRaw.Length > 0)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (NameSimilarity.IsLikelyTypo(normalizedRaw, a.Name))
                {
    ...
    // 誤字候補判定（正規化・文字種構成・編集距離）は Data.Text.NameSimilarity に集約されている。
    ...
    private async Task WarnIfSimilarPersonAliasAsync(string rawName, CancellationToken ct)
    {
        ...
        string normalizedRaw = NameSimilarity.NormalizeForCompare(rawName);

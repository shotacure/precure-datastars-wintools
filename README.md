...
### v1.3.0 リリース直前 最終調整 — 左サイドナビ幅を倍に確保・`/episodes/` を 3 列レイアウトに再構成

GitHub 公開ビルド準備の最終段で目視確認したデザイン上の細部 2 点を修正。`Directory.Build.props` の `<Version>` / `<AssemblyVersion>` / `<FileVersion>` は `1.3.0` 据え置き（次回コード変更からリビジョンインクリメントして `1.3.1` 系列に移行）。
...
- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`（`.page-section-nav` 幅 220→340 / 左寄せ追従、`.episodes-index-list .ep-row-main` 3 列グリッド、`.ep-row-no-date` 縦積み、`.episodes-index-list .ep-row .staff-badges-row` 70% フォント）
- `README.md`（本エントリ追加）

...
### v1.3.0 ブラッシュアップ stage 22 — 商品索引にシリーズ別／ジャンル別タブを導入、商品詳細の発売元・販売元表示を構造化 ID 経由に整理
...
- `PrecureDataStars.SiteBuilder/Generators/ProductsGenerator.cs`：`GenerateIndex` に `discsByProduct` 引数追加、`BuildKindSections` / `BuildSeriesSections` / `ClassifyProductIntoSeriesBucket` を新設、テンプレ用 DTO を 2 系統セクション併存型に再編
- `PrecureDataStars.SiteBuilder/Templates/products-index.sbn`：タブ UI（2 タブ + パネル + インラインスクリプト）に置換
- `PrecureDataStars.SiteBuilder/Templates/products-detail.sbn`：発売元・販売元行を `LabelText` / `DistributorText` 参照に差替（旧 3 行ブロックを撤去）
- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`：`.products-tabs` 配下のタブ UI スタイル一式を追加
- `README.md`：本エントリ追加 + 商品索引ページ仕様 + 商品詳細の発売元・販売元表示仕様の更新

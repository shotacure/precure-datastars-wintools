...
partial class CreditMastersEditorForm
{
    ...
    // 15 タブ構成（先頭にプリキュア、続柄／家族関係を含む。声優キャスティング専用タブは持たない）
    private TabPage tabPrecures = null!;
    private TabPage tabPersons = null!;
    private TabPage tabPersonAliases = null!;
    private TabPage tabCompanies = null!;
    private TabPage tabCompanyAliases = null!;
    private TabPage tabLogos = null!;
    private TabPage tabCharacters = null!;
    private TabPage tabCharacterAliases = null!;
    private TabPage tabCharacterRelationKinds = null!;
    private TabPage tabCharacterFamilyRelations = null!;
    ...
    // ─────────────── 声優キャスティングは専用タブを持たない ───────────────
    // credit_block_entries の CHARACTER_VOICE エントリで一元管理するため、関連 UI 群
    ...
    // roles.hide_role_name_in_credit を切り替える CheckBox。
    ...
    // 詳細パネルのシリーズ選択コンボ（「（既定 / 全シリーズ）」または特定シリーズ）
    ...
    private CheckBox chkEtsBroadcastOnly = null!;
    ...
    private Button btnCopyEts = null!;
    private Button btnRangeCopyEts = null!;
    ...
    private TextBox txtPaNameEn = null!;
    ...
    // 表示上書きテキスト（ユニット名義などで定形外表示が必要なときに使う）と
    ...
    private TextBox txtCaNameEn = null!;
    ...
    private TextBox txtCaaNameEn = null!;
    // character_aliases に valid_from / valid_to 列は無いため、対応する UI 入力欄も持たない。
    ...
    private void InitializeComponent()
    {
        ...
        // 各タブの内容構築。BuildPrecuresTab / BuildCharacterRelationKindsTab /
        ...
        // タブ並び：プリキュアを先頭に置き、その後ろに既存マスタ群（親→名義→新マスタの順）。
        // 「親マスタ → その名義」を隣接させる構成は 踏襲。続柄／家族関係は
        ...
        // プリキュアタブの内容（変身名義 4 本＋肌色ピッカー＋家族グリッド）が
        // 1100x720 ではキツキツになるため、両方向に広く取る（横は SkinColorPicker 560 +
    ...
    private void BuildCharactersTab()
    {
        ...
        // 旧コードでは "MAIN/SUPPORT/GUEST/MOB/OTHER" の文字列を Items に直書きしていたが、
        // character_kind がマスタ化されたので、Designer ではハードコードしない
    ...
    private void BuildRoleOverridesTab()
    {
        ...
        // フィールド名（cboOvSeries, gridRoleOverrides, cboOvRole, cboOvTemplateSeries,
        //   txtOvFormatTemplate, txtOvNotes, btnSaveOverride, btnDeleteOverride, dtOvFrom,
        //   dtOvTo, chkOvToNull）はソース全体での参照箇所が多いため維持している。
        //   dtOvFrom / dtOvTo / chkOvToNull は使わないが、コンパイル維持のため
        //   フィールドだけ生成して画面には追加しない。
        ...
        // 操作ボタンはグリッド直下に置く（詳細パネル右上だと画面幅が狭いとき視認しづらいため）。
        ...
        // valid_from / valid_to は使わない。フィールドは他参照箇所の都合で残し、パネル外に置く。
        ...
        // 役職コンボは詳細パネル外（上部の cboOvSeries）に統一しているため、
    ...
    private void BuildEpisodeThemeSongsTab()
    {
        ...
        // 追加：他話からのコピーボタン。EpisodeThemeSongCopyDialog を起動する。
        btnCopyEts = new Button { Text = "他話からコピー...", Location = new Point(620, 82), Size = new Size(140, 28) };
        // 範囲コピーボタン。EpisodeThemeSongRangeCopyDialog を起動する。
    ...
    private void BuildPersonAliasesTab()
    {
        ...
        // 共同名義 person_id の追加用に「検索...」ボタンを横に追加
    ...
    private void BuildCharacterAliasesTab()
    {
        ...
        // 英語表記 (name_en) は Y=82 行に置く。備考は Y=150。

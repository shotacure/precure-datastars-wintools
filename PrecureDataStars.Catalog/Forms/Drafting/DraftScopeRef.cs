...
public sealed class DraftScopeRef
{
    ...
    /// <summary>クレジット全体を指す場合の参照。 <see cref="Kind"/> = <see cref="ScopeKind.Credit"/> のときのみ非 null。</summary>
    public DraftCredit? Credit { get; init; }

    /// <summary>カード単位を指す場合の参照。 <see cref="Kind"/> = <see cref="ScopeKind.Card"/> のときのみ非 null。</summary>
    public DraftCard? Card { get; init; }

    /// <summary>Tier 単位を指す場合の参照。 <see cref="Kind"/> = <see cref="ScopeKind.Tier"/> のときのみ非 null。</summary>
    public DraftTier? Tier { get; init; }

    /// <summary>Group 単位を指す場合の参照。 <see cref="Kind"/> = <see cref="ScopeKind.Group"/> のときのみ非 null。</summary>
    public DraftGroup? Group { get; init; }

    /// <summary>Role 単位を指す場合の参照。 <see cref="Kind"/> = <see cref="ScopeKind.Role"/> のときのみ非 null。</summary>
    ...
    /// <summary>このスコープを表す日本語のラベル（ダイアログ上部表示用）。</summary>
...
/// <summary><see cref="DraftScopeRef"/> が指す粒度。</summary>

...
**破壊的変更なし**（DB スキーマ変更なし、既存ソースの編集も最小限：ソリューションファイル・Directory.Build.props・.gitignore・README のみ）。新規プロジェクト 1 本を追加するマイナーアップ。

`precure.tv` での Web 公開を見据えて、ローカル MySQL を読み出して静的 HTML 一式を書き出すコンソールアプリを新設した。AWS S3 への同期や CloudFront 連携は本ツール範囲外で、純粋に「DB → 静的ファイル」変換のみを行う。

このリリースは段階的実装の第 1 弾（**叩き台 = タスク 1〜3 範囲**）。残りのページ種別（プリキュア・キャラ・人物・屋号・商品・楽曲・劇伴・サブタイトル統計・役職別ランキング）と SEO 関連（sitemap.xml / robots.txt / JSON-LD）、検索機能は後続タスクで段階的に追加する。

**新規プロジェクト**: `PrecureDataStars.SiteBuilder`（.NET 9 コンソール、`PrecureDataStars.Data` を ProjectReference）。

**追加 NuGet 依存**:
- `Scriban 5.10.0`（テンプレートエンジン）
- `System.Configuration.ConfigurationManager 9.0.9`（接続文字列読み取り、既存 Episodes / TitleCharStatsJson と統一）

**ソリューション構成**:

```
PrecureDataStars.SiteBuilder/
├── PrecureDataStars.SiteBuilder.csproj
├── App.config.sample                            … 接続文字列・出力先・ベース URL の設定サンプル
├── Program.cs                                   … エントリポイント
├── Configuration/
│   └── BuildConfig.cs                           … App.config からの設定ロード
├── Pipeline/
│   ├── SiteBuilderPipeline.cs                   … 全体オーケストレータ
│   ├── BuildContext.cs                          … 全 Generator から読まれる共有データ
│   ├── BuildLogger.cs                           … 進捗・警告ログ
│   └── BuildSummary.cs                          … ページ数集計
├── Data/
│   └── SiteDataLoader.cs                        … 共有データの初期一括ロード
├── Generators/
│   ├── HomeGenerator.cs                         … "/", "/about/" を生成
│   ├── SeriesGenerator.cs                       … "/series/", "/series/{slug}/"
│   └── EpisodeGenerator.cs                      … "/series/{slug}/{epNo}/" — 中核
├── Rendering/
│   ├── ScribanRenderer.cs                       … Scriban のロード・キャッシュ・レンダリング
│   ├── LayoutModel.cs                           … _layout.sbn 用の共通モデル
│   ├── PageRenderer.cs                          … 2 段レンダリング（コンテンツ → レイアウト）
│   ├── TitleCharInfoRenderer.cs                 … BuildTitleInformationPerCharAsync の Web 移植
│   ├── FormatTableModel.cs                      … フォーマット表モデル
│   ├── FormatTableBuilder.cs                    … パート行から累積タイムコードを計算
│   └── CreditTreeRenderer.cs                    … クレジット階層を HTML 化（簡易版）
├── Utilities/
│   ├── PathUtil.cs                              … URL パス→出力ファイルパス変換
│   └── HtmlUtil.cs                              … HTML エスケープ・秒数フォーマッタ
├── Templates/
│   ├── _layout.sbn                              … 共通レイアウト
│   ├── home.sbn / about.sbn
│   ├── series-index.sbn / series-detail.sbn
│   └── episode-detail.sbn                       … 中核ページのテンプレ
└── wwwroot/
    └── assets/
        └── site.css                             … 共通スタイル（フレームワーク不採用）
```

**生成されるページ（v1.3.0 タスク 1〜3 範囲）**:

| URL パターン | 内容 |
|---|---|
| `/` | サイトトップ（シリーズ一覧グリッド + 特徴紹介） |
| `/about/` | サイト案内・運営者・権利表記 |
| `/series/` | 全シリーズ索引（年代順表） |
| `/series/{slug}/` | シリーズ詳細（基本情報 + エピソード一覧 + 外部 URL） |
| `/series/{slug}/{seriesEpNo}/` | エピソード詳細（後述、本リリースの中核） |

**エピソード詳細ページの構成（中核）**:

1. サブタイトル表示（`title_rich_html` のルビ付き HTML をそのまま流す）
2. 基本情報テーブル（放送日時・各種ナンバリング・外部 URL・YouTube 予告埋め込み）
3. **フォーマット表**: OA / 配信 / 円盤の各バージョンの累積開始時刻と尺を併記する 22 パート種別対応の表
4. **サブタイトル文字情報**: Episodes エディタの `BuildTitleInformationPerCharAsync` ロジックを `TitleCharInfoRenderer` として移植。「`「文字」… [初出] [唯一] N年Mか月(P話)ぶりQ回目 『シリーズ』第N話「サブタイトル」(YYYY.M.D)以来`」形式の HTML を生成。badge の色分け CSS 付き
5. **サブタイトル文字統計**: `episodes.title_char_stats` JSON の `length` と `categories` をテーブル化。`System.Text.Json` でパース
6. **パート尺偏差値**: `EpisodePartsRepository.GetPartLengthStatsAsync` を直接呼び出し、AVANT / PART_A / PART_B のシリーズ内・歴代の順位と偏差値を表示。Episodes エディタと同じ MySQL ウィンドウ関数（`RANK / AVG / STDDEV_POP`）を使うため値は完全一致
7. **主題歌**: `episode_theme_songs` から OP / ED / 挿入歌、`is_broadcast_only=1` の本放送限定行も区別表示
8. **クレジット階層**: `credits.scope_kind = 'EPISODE'` の OP / ED について、`Card → Tier → Group → Role → Block → Entry` を構造保持で HTML 化。Entry は 5 種別（PERSON / CHARACTER_VOICE / COMPANY / LOGO / TEXT）対応、`affiliation` 小カッコ所属表記対応、`is_broadcast_only` マーカー付き、`col_count` を CSS Grid のカラム数として反映
9. 前後話ナビ

**設計判断**:

- **テンプレートエンジン: Scriban**: Razor は WinForms ソリューションへの組み込みが煩雑で、Scriban の方が依存関係も軽く、テンプレート構文がシンプル。`html.escape` 等の組み込みフィルタも揃っている
- **2 段レンダリング**: Razor の `@RenderBody()` 相当が Scriban に無いため、各 Generator が「コンテンツ HTML を先に生成」→「`_layout.sbn` の `Content` に詰めて再レンダリング」する素朴な構造に落とし込んだ。テンプレ間の `include` も `_layout.sbn` 経由で同フォルダから解決
- **CSS フレームワーク不採用**: `precure.tv` の規模感と保守コストを考えて、Tailwind 等は導入せず素の CSS で構築。CSS 変数で色を管理し、必要なときだけスタイルを足していく方針
- **既存リポジトリの再利用**: `PrecureDataStars.Data` の Repository 層をそのまま使うため、SiteBuilder 専用の DB アクセスは追加しない。集計クエリ（GetPartLengthStatsAsync 等）も既存メソッドを直接呼ぶことで Episodes エディタとの値の一致を保証
- **既存ソースの編集なし**: クレジット階層レンダリングは Catalog 側 `CreditPreviewRenderer` を直接共有せず、SiteBuilder 内に簡易版 `CreditTreeRenderer` を新規実装した（既存資産の安定運用を優先、共通化は将来課題）
- **出力先のデフォルト**: `out/site/`（リポジトリ直下からの相対）。`.gitignore` で `out/` を追跡対象外に設定

**変更ファイル**:

- 新規: `PrecureDataStars.SiteBuilder/` 配下一式（.csproj / App.config.sample / .cs ファイル群 / .sbn テンプレ群 / site.css）
- 変更: `precure-datastars-wintools.sln`（SiteBuilder プロジェクトを追加、新規 GUID `B845168A-9B4E-4881-9D38-BC0F86B5B16F`）
- 変更: `Directory.Build.props`（Version: `1.2.4` → `1.3.0`）
- 変更: `.gitignore`（`out/` を追加）
- 変更: `README.md`（本セクション・トップバナー・プロジェクト詳細表・主要ワークフローの「Web 公開用静的サイト生成」節を追加）

**未対応・後続タスクで追加予定**（タスク 4 以降のスコープ）:

- プリキュア（`/precures/`）・キャラクター（`/characters/`）・人物（`/people/`）・屋号（`/companies/`）・商品（`/products/`）・ディスク（`/discs/`）・楽曲（`/songs/`）・録音（`/recordings/`）・劇伴（`/series/{slug}/bgm/`）の各詳細ページ
- 50 音インデックス（人物・キャラ・屋号）
- 役職別ランキング `/stats/roles/`
- サブタイトル統計群 `/stats/subtitles/`（添付 SQL を移植）
- SEO 関連（sitemap.xml / robots.txt / canonical 強化 / JSON-LD / OGP 強化）
...
#### エピソード詳細ページのブラッシュアップ（v1.3.0 仕上げ作業）

リリース直前のブラッシュアップとして、サイトの中核ページであるエピソード詳細を中心に、表記の正確性・閲覧性・遷移性をまとめて改善した。スキーマ変更を 1 件含むがマイグレは冪等で安全、UI/UX 改善が主体。

##### スキーマ変更：episodes.duration_minutes の追加

- **マイグレ**：`db/migrations/v1.3.0_add_episodes_duration_minutes.sql` で `episodes` テーブルに `duration_minutes TINYINT UNSIGNED NULL` 列を `on_air_at` の直後に追加。INFORMATION_SCHEMA で列存在を確認してから ALTER を発行する冪等な作りで、既存の有効エピソード（is_deleted=0）のうち duration_minutes が NULL のものに 30 を一括バックフィル（既存の TV シリーズエピソードは例外なくすべて 30 分番組であるため、一括投入が業務として正しい）。
- **モデル・リポジトリ**：`Episode.DurationMinutes (byte?)` を追加し、`EpisodesRepository` の SELECT/INSERT/UPDATE 全 SQL に `duration_minutes` を組み込み。新規エピソードは NULL 許可のままで、エディタで都度入力する運用。
- **Episodes エディタ**：`EpisodesEditorForm.Designer.cs` に `lblDurationMinutes`（"尺(分)"）と `numDurationMinutes`（NumericUpDown、範囲 0–255、既定値 30）を `dtOnAirAt` の右隣に配置。`EpisodesEditorForm.cs` の `BindEpisode` / `ClearEditor` で値を読み込み（NULL は 30 表示）、`SaveAsync` で保存（0 入力は NULL 保存）、`AddAsync` の新規行は直前話の値（無ければ 30）を継承。

##### エピソード詳細ページの表示改善

- **放送日時の終了時刻併記**：`EpisodeGenerator.FormatJpDateTimeWithDuration` で `OnAirAt` と `DurationMinutes` から「2004年2月1日 8:30〜9:00」形式の文字列を組み立てる。尺未登録（NULL）のエピソードは従来通り「2004年2月1日 8:30」で出す（後方互換）。
- **「いま現在の参照点」キャプション**：サブタイトル文字情報・パート尺統計情報の説明文末尾に「（yyyy年m月d日現在、『○○プリキュア』第N話時点）」を付与。これらは毎週放送が進むと値が変わる（順位や偏差値が変動する、初出文字が確定する等）ため、サイトを見た時点を明示する。`BuildContext.LatestAiredTvEpisode` に「ビルド時刻以前で OnAirAt が最大の TV エピソード」を集約してサイト全体で参照点を統一し、`SiteDataLoader.LoadAsync` 内で 1 回だけ算出してログに出力。
- **主題歌セクションを縦リスト 1 行表現に**：5 列テーブル形式から `<ul class="theme-songs-list">` で 1 曲 = 1 行（`OP「タイトル」　うた：歌唱者　備考`）の縦リストに変更。要素間は `<span class="ts-sep">　</span>` の全角スペースで区切る。楽曲タイトルは `SongLink` が引けていれば `/songs/{song_id}/` への `<a>` 付き、`IsBroadcastOnly` 行は末尾に「（本放送のみ）」を併記。スタイルは `.theme-songs-list` セレクタ群（`.ts-kind` / `.ts-title` / `.ts-variant` / `.ts-singer` / `.ts-notes` / `.ts-bcast`）に集約し、旧 `.theme-songs-table` セレクタは互換のため残す。
- **フォーマット表ヘッダの正式名称化**：「配信」を「配信(Amazon Prime)」、「円盤」を「Blu-ray/DVD」に変更。説明文も「本放送・配信(Amazon Prime)・Blu-ray/DVD で…」に整え、何のメディアを比較しているかを明示。時刻列（`.format-table .timecode`）と尺列（`.format-table .duration`）の左右パディングを 6px に圧縮し、表全体を約 70% 幅にコンパクト化。
- **パート尺統計表の 2 段ヘッダ化**：上段で「パート（rowspan=2）/ 『○○プリキュア』（colspan=2）/ 歴代プリキュア全体（colspan=2）」、下段で「順位 / 偏差値 / 順位 / 偏差値」を並べる構造に再編。シリーズ略称は `EpisodeGenerator` 側で `SeriesTitleShortQuoted = $"『{series.TitleShort ?? series.Title}』"` を組み立ててテンプレに渡す。表内の値は接頭辞（「○○プリキュア内」「歴代」）を除去した `{順位} / {総数} 位` のシンプル表記に変更（`PartLengthStatRow` から `SeriesShort` プロパティを撤去）。
- **クレジットセクション h3 のサイズ統一**：`.credit-section h3.credit-kind-heading` のフォントサイズを 16px から 1.05em に変更し、サイト共通 h3（外部サイト見出し・使用音声見出し等）と完全に同じ大きさに揃える。
- **すべての `<th>` を太字に統一**：基本情報の通算サブテーブル（`.totals-table th`）の `font-weight: normal` を `600` に変更し、サイト全体の表ヘッダの太字運用を統一。
- **ページネーション端ボタンに長尺ラベル**：「« 前話」「次話 »」を「« #N サブタイトル」「#N サブタイトル »」に変更。`EpisodeContentModel.PrevPagerLabel` / `NextPagerLabel` で `$"#{prev.SeriesEpNo} {prev.TitleText}"` を組み立てて渡す。CSS は `min-width: auto; max-width: 18em;` ＋ `text-overflow: ellipsis;` で過剰に長いサブタイトルでも横にあふれず省略表示。下部のサブタイトル付き前後ナビ（旧 `prev-next-nav prev-next-bottom`）は廃止し、上下ともページネーション 1 種類だけのシンプル構成に。

##### クレジットセクションのリンク化と協力行レイアウト

- **すべての表示要素をそれぞれの詳細ページにリンク化**：`CreditTreeRenderer` を全面改修。役職名は `BuildRoleNameHtml(roleCode, roleName, roleMap)` で `/stats/roles/{role_code}/` への `<a>` に包む（VOICE_CAST 系は `/stats/voice-cast/`）。人物名義は既存の `StaffNameLinkResolver.ResolveAsHtml` 経由で `/persons/{id}/` へ（共有名義は添字付き複数リンク化）。企業屋号は新設の `BuildCompanyAliasHtmlAsync(aliasId, displayName)` で屋号 → 親 company_id を `LookupCache.LookupCompanyIdFromAliasAsync` で解決して `/companies/{cid}/` へ。ロゴは `BuildLogoHtmlAsync(logoId)` で「屋号名」に置換した上で同じく `/companies/{cid}/` へ（CI バージョンラベルは省く）。`ResolveEntryHtmlAsync` が PERSON / CHARACTER_VOICE / COMPANY / LOGO / TEXT の 5 種別すべてをリンク済み HTML 断片で返すようにし、`fallback-table` / `fallback-vc-table` の各セル、`leading_company` 行、絵コンテ・演出融合行、テンプレ非マッチ時の自動 2 列ラップ、すべての描画パスでリンク版を使用。
- **役職名カラムを右寄せ**：`.credit-section table.fallback-table td.role-name` と `.credit-section table.fallback-vc-table td.role-name` に `text-align: right;` を追加。さらに `padding-right: 16px;` で半角スペース 1 個分以上の余白をエントリ列との間に確保。
- **VC テーブルの actor-cell 列幅統一**：`.credit-section table.fallback-vc-table td.actor-cell` に `min-width: 12em;` を追加。同テーブル内で声優名表示の左端が縦に揃い、複数行の声優名カラムが視覚的に整列する。
- **協力行のレイアウト変更**：CASTING_COOPERATION 由来の「協力」行を、従来の「`<td colspan=2><strong>協力</strong>　屋号 屋号</strong></td>`」から、3 セル構成（`<td class="role-name">協力</td><td class="character-cell"></td><td class="actor-cell">屋号一覧</td>`）に変更。役職名カラムに「協力」を置き、屋号一覧は声優名カラム（actor-cell）に出すことで他の VC 行と列構造が揃う。屋号も `<a href="/companies/{cid}/">` でリンク化、屋号間の区切りは全角スペース。CSS で `.cooperation-row td.role-name` に `text-align: right; font-weight: bold;` を当てて、他の役職名と同じ右寄せ・太字スタイルに統一。
- **`StaffNameLinkResolver` の `CreditTreeRenderer` への注入**：従来は `EpisodeGenerator` 専用だった `StaffNameLinkResolver` を `CreditTreeRenderer` のコンストラクタ第 11 引数として渡し、クレジット階層の隅々まで人物詳細リンクを行き届かせる。

##### サイト全体のスタイル変更

- **リンク色を本文色に統一**：CSS `a { color: #0a55c8; }` を `a { color: var(--fg); }` に変更。デフォルトでは下線も出さず、ホバー時のみ下線を表示する控えめなスタイルに。情報密度の高いデータベースサイトとしてリンクが目立ちすぎないようにする方針。
- **フッターを連絡先・著作権・免責の 3 段構成に刷新**：`_layout.sbn` の `<footer class="site-footer">` ブロックを丸ごと差し替え、`<footer id="pageFooter">` 内に `<section id="contact">`（h2「お問い合わせ」+ X/メールリンク）と `<div id="copyright">`（© Shota (SHOWTIME) + 「サークル『SHOWTIME』代表・祥太が個人で運営しています。ABCアニメーション様、東映アニメーション様、その他各社様とは一切関係ございません。」）を縦に並べる。連絡先は `https://x.com/shota_` と `mailto:shota@precure.tv`。CSS は `#pageFooter` / `#contact` / `#copyright` のセレクタ群で定義、サイト全体のコンテナ幅で中央配置。免責文は ABC アニメーション・東映アニメーション・その他各社との無関係を明記し、個人運営の非公式データベースであることを示す。

##### 設計判断のメモ

- **duration_minutes は NULL 許可のまま**：既存全 TV を 30 でバックフィルしたため、現状ほぼ全行が 30 で埋まっている。とはいえ将来の短尺映画・配信短編・特番（拡大版・縮小版）等で 30 以外の値が必要になる場面があるため、必須化（NOT NULL DEFAULT 30）はしない。新規エピソード追加時のエディタは既定 30 を表示しつつ、明示的に 0 を入れることで NULL 保存も可能（「尺不明」の意思表示）。
- **「いま現在の参照点」を BuildContext に集約する理由**：エピソード詳細ページが個別に `episodes` テーブル全件を再走査して最新話を求めると N×全話走査になる（エピソード数の二乗オーダ）。`SiteDataLoader.LoadAsync` で 1 度だけ計算して `BuildContext.LatestAiredTvEpisode` に持たせ、各 Generator が参照する設計にすると O(N+M)。同時に「サイト全体の『いま』が一致しているか」という運用上の不安も `BuildContext` 1 箇所を見れば確認可能になる。
- **協力行を 3 セル構成にした意図**：VC テーブルは（役職名 / キャラ名 / 声優名）の 3 列構成。旧協力行は 2 セル（colspan=2）で「役職と屋号一覧を 1 列に詰める」造りだったが、3 列構造との整合性が取れていなかった。新仕様では役職名カラムに「協力」を置き、屋号一覧は声優名カラムに出すことで、VC テーブル全体が 3 列で安定する。
- **VOICE_CAST 役職を `/stats/voice-cast/` に集約する理由**：声優の出演履歴は「役職別ランキング」よりも「演じたキャラクターの詳細」の方が圧倒的に情報量が多いため、`/stats/roles/CHARACTER_VOICE/` のような汎用ページにするより、専用の `/stats/voice-cast/` で 3 セクション（メイン / サブ / ゲスト）構造を持たせた方が読み手にとって有益。`BuildRoleNameHtml` 側でリンク先を場合分けすることで、両者を矛盾なく共存させている。
...
- 誕生日：`birth_month TINYINT UNSIGNED`（1-12、任意、CHECK 制約）と `birth_day TINYINT UNSIGNED`（1-31、任意、CHECK 制約）の 2 列に正規化。和文「m月d日」と英文「Month d」の表示はアプリ側で生成（DB に和文・英文の文字列を二重には持たない設計）。
- 声優：`voice_actor_person_id INT NULL`（FK → `persons.person_id`、`ON DELETE SET NULL`）。`character_voice_castings` を廃止する代わりに「プリキュアごとの主担当声優を 1 発で引きたい」用の便宜参照カラムとして配置。正は `credit_block_entries` の登場行。
- 肌色：HSL（`skin_color_h SMALLINT UNSIGNED` 0-360、`skin_color_s` / `_l TINYINT UNSIGNED` 0-100）と RGB（`skin_color_r` / `_g` / `_b TINYINT UNSIGNED` 0-255）を併記。CHECK は H/S/L のみ；R/G/B は型範囲が一致するため CHECK 不要。運用安定までは GUI の肌色ピッカーで両者を並べて目視確認 +ΔE 評価する設計。
...
- 整合性トリガ `tr_precures_check_character_bi` / `tr_precures_check_character_bu`：4 本の alias FK が指す `character_id` がすべて同一であることを `BEFORE INSERT` / `BEFORE UPDATE` で検証し、不整合なら `SIGNAL SQLSTATE '45000'` で拒否する（業務ルール「変身前後で別キャラになるレギュラープリキュアは存在しない」を DB レイヤーで強制）。MySQL 8.0 では CHECK 制約から別テーブル参照ができないため、`credit_block_entries` の整合性検証と同じトリガパターンで実装している。NULL の `transform2` / `alt_form` はチェックスキップ。

**`character_relation_kinds` テーブル新設**（キャラクター続柄マスタ、PK `relation_code`）：

- 「自分から見た続柄」のコード集合。`name_ja` / `name_en` / `display_order`（10 単位飛び番、UNIQUE）/ `notes` / 監査列を持つ `part_types` 系の小さなマスタ。
- 初期投入 13 種：`FATHER`（父）/ `MOTHER`（母）/ `BROTHER_OLDER`（兄）/ `BROTHER_YOUNGER`（弟）/ `SISTER_OLDER`（姉）/ `SISTER_YOUNGER`（妹）/ `GRANDFATHER`（祖父）/ `GRANDMOTHER`（祖母）/ `UNCLE`（叔父・伯父）/ `AUNT`（叔母・伯母）/ `COUSIN`（いとこ）/ `PET`（ペット）/ `OTHER_FAMILY`（その他家族）。プリキュア作品で頻出する続柄を網羅。業務側で必要があれば後から追加・並べ替え可能（`CharacterRelationKindsRepository.BulkUpdateDisplayOrderAsync` で 2 段階 UNIQUE 退避方式の DnD 対応）。

**`character_family_relations` テーブル新設**（家族関係、汎用、複合 PK）：

- 1 行 = 「`character_id`（自分）から見た `related_character_id`（相手）の続柄が `relation_code`」。プリキュア限定ではなく、敵キャラの兄弟関係や脇役のペット関係などにも使える設計。
- PK は (`character_id`, `related_character_id`, `relation_code`) の 3 列複合。同じキャラ同士で続柄が違う行（例: 義理＋実父など）は別行で持てる。
- FK: `character_id` / `related_character_id` 双方とも `ON DELETE CASCADE`（関連キャラが消えたら関係も消える）、`relation_code` は `ON DELETE RESTRICT`。
- 自己参照禁止トリガ `tr_cfr_check_no_self_bi` / `tr_cfr_check_no_self_bu`：自分自身（`character_id == related_character_id`）の関係を `SIGNAL SQLSTATE '45000'` で拒否する。当初 CHECK 制約 `ck_cfr_no_self` で書く想定だったが、MySQL 8.0.16+ の「FK の参照アクション（CASCADE 等）で使う列を CHECK 制約から参照不可」(Error 3823) という制約に該当するため定義不能。`precures` の `character_id` 整合性検証と同じトリガパターンに統一した。
- 双方向で完全表現するときは A→B（`FATHER`）と B→A（逆続柄、例: `SON` を将来追加する場合）の 2 行を別途立てる運用（自動補完はトリガでは行わず、UI 側の明示操作に委ねる方針）。
- インデックス：`related_character_id`（逆引き）と `relation_code`（マスタ集計）。

**`character_voice_castings` テーブル撤去**：

- 業務ルール「ノンクレ除いて、その役柄でクレジットされている＝キャスティング」が固まったことを受けて、専用テーブル `character_voice_castings` を撤去（無条件 DROP、業務側で 0 件を確認済み）。
- 声優キャスティングの所在は `credit_block_entries` の `CHARACTER_VOICE` エントリ（`character_alias_id` + `person_alias_id` の 2 軸）に一元化。代役・暫定担当・モブの区別は credit 側の所属クレジット（OP / ED / 各話 / 映画など）と任意のテキストエントリで表現する。
- 関連 C# 資産（`Models/CharacterVoiceCasting.cs` / `Repositories/CharacterVoiceCastingsRepository.cs`）も物理削除。

#### マイグレーション SQL

`db/migrations/v1.2.4_add_precures_and_family.sql` を新設。STEP 1: `character_voice_castings` の件数を NOTE 表示してから DROP / STEP 2: `character_relation_kinds` を `CREATE TABLE IF NOT EXISTS` + `INSERT IGNORE` で新設・初期投入 / STEP 3: `character_family_relations` を新設 / STEP 4: `precures` を新設 / STEP 5: `precures` の整合性トリガ 2 本を `DROP TRIGGER IF EXISTS` してから再作成。`INFORMATION_SCHEMA` で各オブジェクトの存在を確認してから DDL を発行するため再実行しても安全な冪等スクリプト。

#### データ層

新規モデル 3 本：`Models/Precure.cs`、`Models/CharacterRelationKind.cs`、`Models/CharacterFamilyRelation.cs`。新規リポジトリ 3 本：

- **`PrecuresRepository`** … 標準の CRUD（GetAllAsync / GetByIdAsync / InsertAsync / UpdateAsync / SoftDeleteAsync / DeleteAsync）に加え、一覧表示用の軽量プロジェクション `GetListAsync()` を提供。`PrecureListRow` 型で「変身前 alias 名 / 変身後 alias 名 / 声優氏名」を JOIN 済みで返すため、グリッド側はこの型をそのままバインドできる。`BirthdayJa` プロパティで「7月1日」表示文字列を即座に取り出せる。
- **`CharacterRelationKindsRepository`** … `CharacterKindsRepository` と同じ流儀（GetAllAsync / GetByCodeAsync / InsertAsync / UpdateAsync / DeleteAsync / `BulkUpdateDisplayOrderAsync` の 6 メソッド）。`BulkUpdateDisplayOrderAsync` は UNIQUE な display_order に対する 2 段階退避（200 番台 → 本番値）方式で衝突を回避する。
- **`CharacterFamilyRelationsRepository`** … `GetByCharacterAsync`（自分視点の家族）／`GetByCharacterWithNamesAsync`（相手キャラ名・続柄表示名を JOIN した軽量行 `CharacterFamilyRelationListRow` で返却、グリッド表示用）／`InsertAsync` / `UpsertAsync`（複合 PK）/ `DeleteAsync` / `ReplaceAllForCharacterAsync`（character_id 単位の一括置換、家族グリッドの「保存（置き換え）」ボタン用）。

`Models/CharacterVoiceCasting.cs` と `Repositories/CharacterVoiceCastingsRepository.cs` は物理削除。

#### Catalog GUI

**`CreditMastersEditorForm` を 13 タブ → 15 タブに再編** し、ウインドウサイズを 1100×720 → 1500×850 に拡張：

- **「プリキュア」タブ**を **先頭** に追加。SplitContainer の上下分割で、上段にプリキュア一覧グリッド（`PrecureListRow` 直バインド）、下段に詳細編集パネル。詳細パネルは「キャラクター選択コンボ → 4 名義コンボ（変身前 / 変身後 / 変身後 2 / 別形態）→ 誕生日（月コンボ + 日コンボ + 和英プレビュー）→ 声優（person_id + 検索ボタン + 表示名ラベル）→ 肌色ピッカー → 学校 / クラス / 家業 → 備考 → 家族グリッド + 続柄・相手キャラ追加コンボ + 追加 / 削除ボタン → 新規 / 保存 / 削除ボタン」と縦に並ぶ。キャラクター選択コンボを変えると、4 名義コンボの選択肢がそのキャラ配下の `character_aliases` のみに絞り込まれ、変身前後で別キャラを選んでしまう事故を UI レベルで防ぐ。任意 2 本の名義コンボには先頭に「(未設定)」項目が入る。プリキュア行を選択すると変身前 alias の `character_id` を引いて家族グリッドが自動でロードされ、家族の追加・削除はその場で `character_family_relations.UpsertAsync` / `DeleteAsync` が走り即座に DB 反映する流儀。
- **「キャラクター続柄」タブ**を新設（キャラクター名義タブの直後）。`character_relation_kinds` マスタの CRUD 画面。続柄コード／日本語表示／英語表示／表示順／備考 + 新規・保存・削除ボタンの定型構成。
- **「家族関係」タブ**を新設（続柄タブの直後）。`character_family_relations` の汎用編集 UI。「自分」キャラを選ぶとそのキャラの家族関係一覧がグリッドにロードされ、下部のフォーム（相手キャラ・続柄・表示順・備考）で行追加／削除をメモリ上で行い、「保存（置き換え）」ボタンで `character_id` 単位の一括置換を行う。プリキュアタブの家族グリッドが「即時保存」型なのに対し、こちらは「メモリ上で編集してまとめて保存」型の扱いやすさを担当する。
- **「声優キャスティング」タブを撤去**。フォーム内のフィールド宣言（`cboVcCharacter` / `gridVoiceCastings` / `numVcPersonId` / `cboVcKind` / `dtVcFrom` / `chkVcFromNull` / `dtVcTo` / `chkVcToNull` / `txtVcNotes` / `btnNewVoiceCasting` / `btnSaveVoiceCasting` / `btnDeleteVoiceCasting` / `btnPickVcPersonId`）と `BuildVoiceCastingsTab()` メソッド、関連イベント結線、`ReloadVoiceCastingsAsync` / `OnVoiceCastingRowSelected` / `ClearVoiceCastingForm` / `ResolveVoicePersonNameAsync` / `SaveVoiceCastingAsync` / `DeleteVoiceCastingAsync` の 6 メソッドをすべて撤去。`HideAuditColumns(gridVoiceCastings)` も削除。
- 新規 partial ファイル **`CreditMastersEditorForm.PrecureTabs.cs`** に新タブ 3 つの Designer 部 + ロジック部を集約し、既存 12 タブ実装に対する差分を局所化している。`WirePrecureTabsEvents()` / `LoadPrecuresTabAsync()` / `LoadCharacterRelationKindsTabAsync()` / `LoadCharacterFamilyRelationsTabAsync()` / `RefreshPrecureTabComboSourcesAsync()` / `RefreshCharacterFamilyTabComboSourcesAsync()` / `OnPrecureRowSelectedAsync()` / `ClearPrecureForm()` / `UpdateBirthdayPreview()` / `SavePrecureAsync()` / `DeletePrecureAsync()` / 続柄マスタ CRUD / 家族関係 CRUD / `ExtractNullableIntFromCombo` などのヘルパを実装。

**肌色ピッカー UserControl `SkinColorPickerControl`** を新規追加（`PrecureDataStars.Catalog/Forms/Controls/SkinColorPickerControl.cs`）：

- HSL（H 0-360 / S 0-100 / L 0-100）と RGB（R/G/B 0-255）の 6 つの NumericUpDown と「未設定」チェックを 2 行構成で配置。
- 各群の右側に 60×28 px の色プレビューパネルを並べ、「HSL→ / RGB→」のラベルを添える。値変更のたびに即時更新。
- 「色差 ΔE (CIE76):」ラベルにバッジを表示。HSL から RGB を復元した色と直接 RGB 値の色を、CIE Lab 空間でのユークリッド距離（CIE76）で比較し、`ΔE < 2.3` で「✓ 許容範囲（緑）」、`< 5.0` で「△ 要確認（黄）」、それ以上で「× 不一致（赤）」のバッジを出す。両方未設定や片方のみ入力時は中立表示。
- 内部実装：`HslToRgb`（標準的な HSL→RGB 公式、整数丸め）、`SrgbToLab`（sRGB→線形 RGB→XYZ D65→CIE Lab、`SrgbToLinear` / `LabF` 補助関数つき）、`ComputeCie76DeltaE`。`SetHsl(h, s, l)` / `SetRgb(r, g, b)` / `GetHsl()` / `GetRgb()` の 4 つの API でデータ層との往復を担当。

**Program.cs / MainForm.cs の DI 配線更新**：`CharacterVoiceCastingsRepository` の生成・引き渡しを撤去し、新規 3 リポジトリ（`PrecuresRepository` / `CharacterRelationKindsRepository` / `CharacterFamilyRelationsRepository`）を追加。`CreditMastersEditorForm` のコンストラクタ引数も `voiceCastingsRepo` を除外して新 3 本を末尾に追加した形に変更。

#### 実装ファイル

- `db/schema.sql`、`db/migrations/v1.2.4_add_precures_and_family.sql`、`db/migrations/v1.2.4_add_name_en_columns.sql`
- `PrecureDataStars.Data/Models/{Precure,CharacterRelationKind,CharacterFamilyRelation}.cs`
- `PrecureDataStars.Data/Repositories/{PrecuresRepository,CharacterRelationKindsRepository,CharacterFamilyRelationsRepository}.cs`
- `PrecureDataStars.Catalog/Forms/Controls/SkinColorPickerControl.cs`（UserControl）
- `PrecureDataStars.Catalog/Forms/CreditMastersEditorForm.{cs,Designer.cs}`（声優キャスティング撤去 + ClientSize 拡張 + タブ並び替え + name_en 4 タブ対応）
- `PrecureDataStars.Catalog/Forms/CreditMastersEditorForm.PrecureTabs.cs`（新規 partial、3 タブの Designer 部 + ロジック部、レイアウト 2 カラム化）
- `PrecureDataStars.Catalog/Program.cs`、`PrecureDataStars.Catalog/MainForm.cs`（DI 配線）
- `Directory.Build.props`（バージョン 1.2.3 → 1.2.4）
- 削除: `PrecureDataStars.Data/Models/CharacterVoiceCasting.cs`、`PrecureDataStars.Data/Repositories/CharacterVoiceCastingsRepository.cs`

#### 内部追加：`name_en` 4 表対応（対称性確保）

v1.2.4 開発中に「親テーブル `persons` / `companies` は `name_en` を持つのに、名義テーブル `person_aliases` / `company_aliases` と `characters` 自体・`character_aliases` が持っていない」という対称性破れに気付き、4 表に揃えて `name_en VARCHAR(128) DEFAULT NULL` 列を追加した。配置は `name_kana` の直後（既存 `persons.name_en` のパターンに準拠）。マイグレーションは独立ファイル `v1.2.4_add_name_en_columns.sql` を新設して、本体マイグレ（プリキュア追加）とは別スクリプトとして冪等に実行できる構成にしている。

GUI 側もマスタ管理 4 タブ（人物名義・企業屋号・キャラクター・キャラクター名義）の編集パネルに「英語表記」テキストボックスを `name_kana` フィールドの直下に追加。座標調整が必要な箇所は左カラム下部のフィールド群を Y+32 シフトさせて挿入し、右カラム（共同名義リストや CRUD ボタン群）は据え置き。`On*RowSelected` の読み込みと `Save*Async` の書き込みにも `NameEn` プロパティの往復を追加した。

なお `PersonAliasesRepository.RenameAsync`（改名 = 新 alias を INSERT して predecessor リンクで履歴化する経路）と `CharactersRepository.CreateCharacterWithAliasAsync`（QuickAdd 経由）は内部 INSERT 文で `name_en` 列を含めない（DEFAULT NULL のまま）。改名・QuickAdd の入力ダイアログには英語表記欄を設けず、英語表記は後からマスタ画面で補完する運用とした（インライン作成のミニフォームを肥大化させない方針）。

#### 内部追加：プリキュアタブのレイアウト全面再設計

初版の `BuildPrecuresTab()` では `SplitContainer.SplitterDistance = 280` で一覧グリッドを 280px 高に固定していたが、デフォルト行高 22px ではヘッダ込みで実質 11 行しか入らず、4-5 件のプリキュアを登録した時点で「下にスクロールしないと残りが見えない」という狭さになっていた。さらに編集パネルが単一カラムで縦に長く積み上がる構成（4 名義コンボ → 誕生日 → 声優 → 肌色 → 学校 → クラス → 家業 → 備考 → 家族グリッド + ボタン）になっていて、グリッド上下の全体バランスが悪かった。

v1.2.4 内で次のように作り直した：

- `SplitterDistance` を 280 → 400 に拡張。一覧グリッドが 18 行程度見えるようになり、シリーズ毎のプリキュア登録が一覧で俯瞰できる。
- 編集パネルを **2 カラムレイアウト** に再構成。1500px ウインドウ前提で、左カラム（x=12、label 100 + input 360 帯）と右カラム（x=520、同帯）の対称配置に変更。
- 4 名義コンボを **2x2 グリッド配置**：変身前 / 変身後 を Y=46 の左右並列、変身後 2 / 別形態 を Y=78 の左右並列。
- **誕生日（左）と声優（右）を Y=114 で並列配置**：誕生日は月コンボ + 日コンボ + 和英プレビューラベル（左カラム内コンパクトに）、声優は num + 検索ボタン + 表示名ラベル（右カラム内に）。
- **学校（左）/ クラス（右）を Y=146 で並列配置**。
- 家業 / 備考 / 肌色 / 家族グリッド はフル幅（label 100 + input 868、x=12..1000）に拡張。
- **CRUD ボタン（新規 / 保存・更新 / 選択行を削除）は右上 absolute 配置**（x=1100、Y=14/46/78）。エディタ全体を見渡しながら主要操作にアクセスできる。
- `gridPrFamily` を 580×140 → 868×130 にしてキャラ名と続柄表示の判読性を改善。家族追加用の続柄・相手キャラコンボ + 追加 / 削除ボタンも家族グリッド直下に並列配置（コンボ幅も拡張）。
...
**1. `person_aliases.display_text_override` 列追加**（VARCHAR(1024) NULL）

ユニット名義などで定形外の長い表示文字列が必要なケース用。非 NULL のときアプリ側の表示ロジックは `name` より優先してこの値を使う。
...
- ロジック: `COALESCE(NULLIF(display_text_override, ''), name)` で表示名解決

**2. `person_alias_members` テーブル新設**（複合 PK: `parent_alias_id` + `member_seq`）

ユニット名義の構成メンバーを順序付きで保持する。

| 列 | 型 | 内容 |
|---|---|---|
| `parent_alias_id` | INT NOT NULL | ユニット側の名義 ID（→ `person_aliases.alias_id`） |
| `member_seq` | TINYINT UNSIGNED NOT NULL | 連名表示順（1 始まり） |
| `member_kind` | ENUM('PERSON','CHARACTER') NOT NULL | メンバー種別 |
| `member_person_alias_id` | INT NULL | PERSON のとき → `person_aliases.alias_id` |
| `member_character_alias_id` | INT NULL | CHARACTER のとき → `character_aliases.alias_id` |

CHECK 制約で `member_kind` と参照列の整合を担保（PERSON のときは person 側のみ非 NULL、CHARACTER のときは character 側のみ非 NULL）。同ユニット内に同一メンバーは 1 回まで（種別ごとに UNIQUE）。

**ネスト禁止 + 自己参照禁止トリガー** `tr_pam_no_nested_unit_bi` / `tr_pam_no_nested_unit_bu` を BEFORE INSERT/UPDATE で実装：
- 自分自身を PERSON メンバーとして追加しようとしている → 拒否
- 追加しようとしている PERSON メンバーが既にユニット（誰かを抱えている） → 拒否
- このユニット自身が既に他ユニットのメンバー → 拒否

自己参照禁止は当初 CHECK 制約 `ck_pam_no_self` で表現していたが、MySQL 8.0.16+ の「FK 参照アクションで使う列を CHECK の比較で参照不可」(Error 3823) 制限により定義不能のため、上記トリガーに統合した（INSERT/UPDATE 時点で `SIGNAL SQLSTATE '45000'` で弾く）。これによりユニットの 1 階層フラットさが保証される（CHARACTER メンバーは alias の世界が分かれているのでネスト・自己参照問題は発生しない）。FK は両方 ON DELETE RESTRICT：メンバーが先に消えるとユニット整合性が崩れるため、先にユニット側の構成行を片付けてから消す運用を強制する。

**3. `song_credits` テーブル新設**（複合 PK: `song_id` + `credit_role` + `credit_seq`）

歌の作家連名（作詞 / 作曲 / 編曲）を順序付きで保持。

| 列 | 型 | 内容 |
|---|---|---|
| `credit_role` | ENUM('LYRICIST','COMPOSER','ARRANGER') NOT NULL | 役 |
| `credit_seq` | TINYINT UNSIGNED NOT NULL | 連名順（1 始まり） |
| `person_alias_id` | INT NOT NULL | 名義参照 |
| `preceding_separator` | VARCHAR(8) NULL | seq>=2 のときの「前 seq との区切り文字」（`・` / `＆` / `、` / ` / ` / ` with ` 等）。seq=1 では NULL |

既存 `songs.{lyricist|composer|arranger}_name` フリーテキスト列は **温存**。本テーブルに行があるとき表示優先、無ければ従来テキスト。

**4. `song_recording_singers` テーブル新設**（複合 PK: `song_recording_id` + `singer_seq`）

歌唱者連名を順序付きで保持。`billing_kind` は **2 値**:
...
「キャラ名のみ（CV 表記なし）」「声優(キャラ) の語順倒置」は **未正規化扱い**で本モデルに含めない。それらの表記は元データを正規化するか、ユニット名義の `display_text_override` で逃がす。

| 列 | 型 | 内容 |
|---|---|---|
| `billing_kind` | ENUM('PERSON','CHARACTER_WITH_CV') NOT NULL | 連名行の種別 |
| `person_alias_id` | INT NULL | PERSON のとき必須 |
| `character_alias_id` | INT NULL | CHARACTER_WITH_CV のとき必須 |
| `voice_person_alias_id` | INT NULL | CHARACTER_WITH_CV のとき必須（CV） |
| `slash_person_alias_id` | INT NULL | スラッシュ相方（PERSON 主名義側、最大 1 個） |
| `slash_character_alias_id` | INT NULL | スラッシュ相方（CHARACTER_WITH_CV 主名義側、最大 1 個） |
| `preceding_separator` | VARCHAR(8) NULL | 前 seq との区切り（`、` / `&` / ` with ` 等） |
| `affiliation_text` | VARCHAR(64) NULL | 所属表記（`with ヤング・フレッシュ` 等の補助テキスト用） |

CHECK 制約で `billing_kind` と各列の整合を担保（PERSON のとき character 系は全て NULL、CHARACTER_WITH_CV のとき person 系（slash 含む）は NULL）。

スラッシュ相方は最大 1 個（2-way）まで。`花咲つぼみ/キュアブロッサム(CV:水樹奈々)` のような同 CV のスラッシュ並列を 1 行で表現できる。

既存 `song_recordings.singer_name` フリーテキスト列は **温存**。

**5. `bgm_cue_credits` テーブル新設**（複合 PK: `series_id` + `m_no_detail` + `credit_role` + `credit_seq`）

劇伴の作家連名（COMPOSER / ARRANGER）を順序付きで保持。`bgm_cues` は (`series_id`, `m_no_detail`) で 1 意のため 4 列複合 PK。既存 `bgm_cues.{composer|arranger}_name` は **温存**。

#### 表示優先規則（フォールバック）

任意の音楽クレジット表示で、構造化行があれば **優先**、無ければフリーテキスト：

| 表示対象 | 優先 1 | フォールバック |
|---|---|---|
| 歌の作詞/作曲/編曲 | `song_credits` 該当 role の連結文字列 | `songs.lyricist_name` 等 |
| 歌の歌唱者 | `song_recording_singers` の連結文字列 | `song_recordings.singer_name` |
| 劇伴の作曲/編曲 | `bgm_cue_credits` 該当 role の連結文字列 | `bgm_cues.composer_name` 等 |

連結ロジックは Repository 側で実装（`SongCreditsRepository.GetDisplayStringAsync` / `SongRecordingSingersRepository.GetDisplayStringAsync` / `BgmCueCreditsRepository.GetDisplayStringAsync`）。連名は `preceding_separator` を seq=2 以降の連結子として使う（seq=1 は前置き無し）。alias の表示名は `COALESCE(NULLIF(display_text_override, ''), name)` 規則。

#### `ThemeSongsHandler` の改修（テンプレ展開）

`{THEME_SONGS}` および `{#THEME_SONGS}` ループ構文の展開で使う `ThemeSongsHandler.FetchAsync` が、`song_id` / `song_recording_id` も SELECT し、構造化クレジットがある曲・録音は表示文字列を上書きする：
...
foreach (var r in rows)
{
    if (r.SongId > 0)
    {
        string lyr = await songCredits.GetDisplayStringAsync(r.SongId, SongCreditRole.Lyricist, ct);
        if (!string.IsNullOrEmpty(lyr)) r.LyricistName = lyr;
        // composer / arranger 同様
    }
    if (r.SongRecordingId > 0)
    {
        string sing = await recordingSingers.GetDisplayStringAsync(r.SongRecordingId, ct);
        if (!string.IsNullOrEmpty(sing)) r.SingerName = sing;
    }
}
```

これにより、`episode_theme_songs` と JOIN したテンプレ展開でも構造化クレジットが優先表示される。既存テンプレ（`{LYRICIST}` / `{COMPOSER}` / `{ARRANGER}` / `{SINGER}` プレースホルダ）はそのまま動作する。

#### 新リポジトリ（4 本）

| クラス | 主な API |
|---|---|
| `PersonAliasMembersRepository` | `GetByParentAsync` / `IsRegisteredAsMemberAsync` / `HasMembersAsync` / `Insert` / `Update` / `Delete` / `DeleteAllByParent` / `ReplaceAllAsync` |
| `SongCreditsRepository` | `GetBySongAsync` / `GetBySongAndRoleAsync` / `GetDisplayStringAsync` / `Insert` / `Update` / `Delete` / `ReplaceAllByRoleAsync` |
| `SongRecordingSingersRepository` | `GetByRecordingAsync` / `GetDisplayStringAsync` / `Insert` / `Update` / `Delete` / `ReplaceAllAsync` |
| `BgmCueCreditsRepository` | `GetByCueAsync` / `GetByCueAndRoleAsync` / `GetDisplayStringAsync` / `Insert` / `Update` / `Delete` / `ReplaceAllByRoleAsync` |

ENUM 値は文字列で DB と読み書きし、各 Model の enum と相互変換する（Dapper の ENUM マッピングを避けるための単純化）。

`PersonAliasesRepository` には以下を追加：
- `display_text_override` 列を SELECT / INSERT / UPDATE に組み込み
- `GetDisplayNameAsync(int aliasId)` — `COALESCE(NULLIF(display_text_override, ''), name)` を返す
- `FindByExactNameAsync(string name)` — name または display_text_override の完全一致で検索（移行ツール用）

#### 新フォーム: `MusicCreditsMigrationForm`

メニュー「編集 → 音楽クレジット名寄せ移行...」から起動する移行支援 UI。

**目的**: 既存フリーテキスト（`songs.{lyricist|composer|arranger}_name`、`song_recordings.singer_name`、`bgm_cues.{composer|arranger}_name`）と `person_aliases` の **完全一致**を引き、選択行を構造化テーブル（`song_credits` / `song_recording_singers` / `bgm_cue_credits`）に **手動で** 1 名義ずつ **トランザクション一括** 移行する。

**ワークフロー**:
1. 「対象名義」を picker で選択（`PersonAliasPickerDialog` を再利用）
2. 「シリーズ」フィルタを任意で選択
3. 「対象列」のチェック（歌:作詞 / 歌:作曲 / 歌:編曲 / 歌:歌唱 / BGM:作曲 / BGM:編曲）
4. 「exact match で検索」ボタン
5. 結果グリッドで対象を絞り込み（既に同役の構造化行があれば自動で `Checked=false` + `AlreadyHasStructured` フラグ）
6. 「選択行を構造化テーブルに反映」で 1 トランザクションに **全行を投入**

**設計上の制約**（意図的な絞り込み）:
- 自動分解は行わない。連名 / ユニット / キャラ(CV) など機械分解の難しい表記は移行ツールが触らない。運用者が「単一名義として登録すべき行」だけを picker で順次処理する想定。
- 各行とも `seq=1` の **単一行** としてのみ INSERT する。連名の作り込みは別途エディタ（v1.2.4 で追加予定）で行う。
- 歌唱クレジットは `billing_kind=PERSON` で固定挿入する（CHARACTER_WITH_CV はエディタ側で設定）。
- 元のフリーテキスト列は触らない（バックアップとして温存）。
- `name` および `display_text_override` の両方を一致候補とする（後者は OR 条件）。
- **スペース正規化**: 既存フリーテキスト（`songs.lyricist_name` 等）はスペース無し（例: `本名陽子`）で格納される運用、一方 `person_aliases.name` は姓と名の間に半角/全角スペース有り（例: `本名 陽子`）で格納される運用のため、`name` 比較は両辺の半角SP・全角SP を全除去してから一致判定する（SQL 側で `REPLACE(REPLACE(col, ' ', ''), '　', '')`、パラメータ側でも同じ規則で正規化）。`display_text_override` 側はユニット長文表記の意味を保つため厳密一致のまま。表記揺れ（"五條真由美"/"五条真由美" 等の漢字違い）は **DB 側のフリーテキスト** または **alias 側** をあらかじめ正規化してから移行する運用。

#### 編集 UI 拡張（SongsEditorForm / BgmCuesEditorForm / CreditMastersEditorForm）

歌・劇伴・名義マスタの各エディタに、構造化クレジット用のインライン UI を追加した。

**`SongsEditorForm`**：曲詳細パネルと録音詳細パネルの末尾に「構造化クレジット」GroupBox を追加。
- 曲側：作詞 / 作曲 / 編曲の各役について、現状の連結文字列を表示する Label と「編集...」ボタンを並べた 3 行レイアウト。
- 録音側：歌唱者の連結文字列を表示する Label と「編集...」ボタン。
- 「編集...」を押すと、作詞 / 作曲 / 編曲は <see cref="PersonAliasCreditsEditDialog"/>、歌唱者は <see cref="SongRecordingSingersEditDialog"/> が開く。
- ダイアログ OK 時に Repository の `ReplaceAllByRoleAsync` / `ReplaceAllAsync` でトランザクション一括保存し、表示ラベルを再計算する。

**`BgmCuesEditorForm`**：cue 詳細パネル末尾に同様の GroupBox を追加。
- 作曲 / 編曲の各役について、連結文字列ラベルと「編集...」ボタン。
- 同じく <see cref="PersonAliasCreditsEditDialog"/> を開いて編集。

**`CreditMastersEditorForm` 人物名義タブ**：
- `display_text_override` を入力する TextBox を追加（VARCHAR(1024) NULL）。空のままなら `name` が表示に使われる旨をヒントラベルで案内。
- 「ユニットメンバー編集...」ボタンを追加。<see cref="PersonAliasMembersEditDialog"/> を開いて当該名義の構成メンバー（PERSON / CHARACTER）を順序付きで編集できる。`ReplaceAllAsync` で一括保存。ネスト禁止は DB トリガーが担保するため、違反時は例外で差し戻す。

#### 編集 UI で使う 3 つの新ダイアログ

| ダイアログ | 用途 | 呼び出し元 |
|---|---|---|
| `PersonAliasCreditsEditDialog` | PERSON 名義のみで構成される連名（作詞 / 作曲 / 編曲、BGM 作曲 / 編曲）の編集 | `SongsEditorForm`, `BgmCuesEditorForm` |
| `SongRecordingSingersEditDialog` | 歌唱者連名の編集（billing_kind 切替 / CV / スラッシュ相方を含む完全表現） | `SongsEditorForm` |
| `PersonAliasMembersEditDialog` | ユニット名義の構成メンバー編集（PERSON / CHARACTER 混在可） | `CreditMastersEditorForm` |

各ダイアログとも、グリッド + 詳細編集パネルの 2 ペイン構成。`+ 追加` / `✓ 反映` / `✕ 削除` / `↑ 上` / `↓ 下` ボタンで操作する。Seq の振り直しは並び替え・削除のたびに自動で行う。OK 押下時は呼び出し側に DTO リストを返すだけで、永続化は呼び出し側の責任（Repository の `ReplaceAll*` を 1 トランザクションで実行）。

#### DI / メニュー配線

- `Program.cs` で 4 本の新リポジトリ（`PersonAliasMembersRepository` / `SongCreditsRepository` / `SongRecordingSingersRepository` / `BgmCueCreditsRepository`）を生成し、`MainForm` に注入
- `MainForm` のコンストラクタに 4 引数追加。`SongsEditorForm` には 4 リポジトリ（`PersonAliasesRepository` / `SongCreditsRepository` / `SongRecordingSingersRepository` / `CharacterAliasesRepository`）、`BgmCuesEditorForm` には 2 リポジトリ（`PersonAliasesRepository` / `BgmCueCreditsRepository`）、`CreditMastersEditorForm` には 1 リポジトリ（`PersonAliasMembersRepository`）を追加注入
- `MainForm.Designer.cs` の編集メニューに「音楽クレジット名寄せ移行...」を末尾セパレータの下に追加

#### 範囲外（v1.2.4 以降の検討項目）

本リリースには以下は含まれない。スコープの妥当性を見て v1.2.4 以降で順次検討する:
- `MusicCreditsMigrationForm` の連名一括登録モード（現状 seq=1 単発のみ。連名は各エディタの構造化クレジットダイアログから組み立てる前提）
- `character_aliases.display_text_override` 列の追加（v1.2.3 では `person_aliases` のみに追加。現状ユースケースでキャラ名義の override が必要な場面が把握できていないため）
- 構造化クレジットの一括 CSV エクスポート / インポート

#### 互換性

- DB レイヤ: マイグレーション SQL は冪等。既存データへの破壊的変更なし。
- アプリ: 新リポジトリ / 新フォームの追加のみ。既存フォームのコンストラクタ・呼び出しに変更なし。
- テンプレ展開: 構造化行が無い曲・録音・cue は従来通りフリーテキストで表示されるため、既存テンプレートは無修正で動く。

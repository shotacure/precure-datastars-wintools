...
public sealed class SeriesRelationKind
{
    ...
    /// <summary>日本語表示名（子 → 親方向、例: "続編"・"映画"・"パート作品"）。 子作品ページから親作品を参照する文脈、または親作品ページの子作品リストで 子側に付けるバッジテキストとして使う。</summary>
    public string NameJa { get; set; } = "";

    /// <summary>日本語表示名（逆向き＝親 → 子方向、例: "前作"・"TVシリーズ"・"セット作品"）。 親作品ページから子作品を見るときに「親側」が文脈になる場面で使う。 例：子「映画 MH」のページに親「無印 MH」を載せるとき、親側のバッジを "TVシリーズ" にする等。 既定値は空文字。空文字のときの取り扱いは利用側 (Site 側) でフォールバックを定義する想定。</summary>
    public string NameJaReverse { get; set; } = "";

    /// <summary>英語表示名（子 → 親方向、任意）。</summary>
    public string? NameEn { get; set; }

    /// <summary>英語表示名（逆向き＝親 → 子方向）。 <see cref="NameJaReverse"/> の英語版。同様に空のときは利用側でフォールバック想定。</summary>

...
public sealed class SongRecordingSinger
{
    ...
    /// <summary>役職コード（→ roles.role_code、複合 PK 第 2 列、追加）。 既定値は <c>VOCALS</c>。CHORUS など別役職を運用者が定義した場合はその値が入る。</summary>
    ...
    /// <summary>主名義（PERSON）：人物名義参照。 <see cref="BillingKind"/> が PERSON のとき非 NULL。</summary>
    public int? PersonAliasId { get; set; }

    /// <summary>主名義（CHARACTER_WITH_CV）：キャラクター名義参照。 <see cref="BillingKind"/> が CHARACTER_WITH_CV のとき非 NULL。</summary>
    public int? CharacterAliasId { get; set; }

    /// <summary>CV（声優）の人物名義参照（<see cref="BillingKind"/>=CHARACTER_WITH_CV のとき必須）。</summary>
    public int? VoicePersonAliasId { get; set; }

    /// <summary>スラッシュ並列の相方（PERSON 主名義側）。最大 1 個。</summary>
    public int? SlashPersonAliasId { get; set; }

    /// <summary>スラッシュ並列の相方（CHARACTER_WITH_CV 主名義側）。最大 1 個。 例: 主＝キュアブラック / 相方＝美墨なぎさ、CV は両者共通。</summary>
...
/// <summary><see cref="SongRecordingSinger.BillingKind"/> の種別。 文字列表現は DB の ENUM 値と一致させる。</summary>

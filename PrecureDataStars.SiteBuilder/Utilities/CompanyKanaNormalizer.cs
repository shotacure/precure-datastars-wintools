...
{
    /// <summary>取り除くべき法人格表記のパターン（先頭・中間・末尾、いずれの位置に出ても除去）。 文字列比較は順次行うため、長いパターンを先に書いて部分一致の食い合いを避ける。</summary>
    ...
    /// <summary>kana 比較用キーを返す。法人格表記をすべて取り除いた残り文字列が比較対象になる。 元文字列が NULL/空の場合は空文字を返す。</summary>
    ...
    /// <summary><see cref="NormalizeForSort"/> 結果を比較する <c>StringComparer</c> 互換の比較関数。 LINQ の <c>OrderBy</c> に渡せる。</summary>

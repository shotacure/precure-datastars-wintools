...
public sealed class BulkParseResult
{
    ...
    /// <summary>「適用ボタン無効化」レベルの警告が 1 件以上あるか。 true のときダイアログ側は適用ボタンを Disabled にしてユーザーに修正を促す。</summary>
...
/// <summary>パース結果における 1 カード分の塊。 テキスト中の <c>---</c> 単独行で区切られる。</summary>
...
public sealed class ParsedGroup
{
    ...
    /// <summary>Group の備考（Notes に保存される。</summary>
    public string? Notes { get; set; }
}

/// <summary>パース結果における 1 役職分の塊。 テキスト中の「行末コロン」行（例: <c>"脚本:"</c> や <c>"声の出演："</c>）で開始し、 次の役職または区切り行が現れるまで配下のエントリを集める。</summary>
public sealed class ParsedRole
{
    /// <summary>役職表示名（テキスト上の見出し。例: "脚本", "演出", "声の出演"）。コロンは含まない。</summary>
    public string DisplayName { get; init; } = "";

    /// <summary>適用フェーズで解決された role_code（roles テーブルの主キー）。 パース直後は null。<see cref="CreditBulkApplyService"/> が name 一致 / 部分一致で引き当てる。</summary>
    public string? ResolvedRoleCode { get; set; }

    /// <summary>適用フェーズで解決された role_format_kind（"NORMAL"/"VOICE_CAST"/etc.）。</summary>
...
{
    /// <summary>先頭の <c>[XXX]</c> 行があった場合に企業屋号として保持する生テキスト。 設定されていれば適用時に <c>credit_role_blocks.leading_company_alias_id</c> に解決する。</summary>
    public string? LeadingCompanyText { get; set; }

    /// <summary>ブロック内の行群（タブ区切り展開済み）。</summary>
    public List<ParsedEntryRow> Rows { get; } = new();

    /// <summary>ブロック内の各行のタブ数の最大値 + 1 = 表示カラム数の意図。 既定 1（縦並び）。タブが含まれる行があれば 2 以上になる。 は <see cref="ColCountExplicit"/> が true のとき <c>@cols=N</c> 構文で明示された値、 false のときは従来どおりタブ数推測値が入る。</summary>
    public int ColCount { get; set; } = 1;

    /// <summary><see cref="ColCount"/> が <c>@cols=N</c> 構文によって明示指定されたかどうか。 true のときはタブ数推測値より優先され、逆翻訳エンコーダもこの状態を保持する。</summary>
...
/// <summary>パース結果における 1 エントリ。</summary>
public sealed class ParsedEntry
{
    ...
    /// <summary>「旧名義 =&gt; 新名義」記法におけるキャラクター名の旧表記参照キー。</summary>
    ...
    /// <summary>本放送限定フラグ。</summary>
    public bool IsBroadcastOnly { get; set; }

    /// <summary>A/B 併記の継続行フラグ。</summary>
    public bool IsParallelContinuation { get; set; }

    /// <summary>エントリ単位の備考。</summary>
    public string? Notes { get; set; }
...
/// <summary>パース時に検出された警告 1 件。</summary>

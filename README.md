...
- **v1.2.0 工程 B' — リリース文脈の導入とエピソード主題歌のコピー機能**: 同じエピソードでも本放送と Blu-ray・配信で異なる主題歌が流れる作品があるため、`episode_theme_songs` テーブルに `release_context ENUM('BROADCAST','PACKAGE','STREAMING','OTHER')` 列を追加し、PK を `(episode_id, release_context, theme_kind, insert_seq)` の 4 列複合に再構築。データの一貫性を保つため、`credits` テーブルにも同じ `release_context` 列を追加し、UNIQUE 2 本を `(series_id, release_context, credit_kind)` および `(episode_id, release_context, credit_kind)` の 3 列構成に拡張した。マイグレーション SQL は冪等（既存列・既存 PK/UNIQUE の構成を `INFORMATION_SCHEMA` で確認してから ALTER）で書かれており、既存行は全て `BROADCAST` バックフィルとなる。エピソード主題歌タブには「**他話からコピー...**」ボタンを追加し、専用ダイアログ `EpisodeThemeSongCopyDialog` で 3 段階の操作（[1] コピー元読み込み → [2] コピー先範囲・文脈指定 → [3] プレビュー編集）を経て、最終的な「すべて保存」を押した時点で初めて `BulkUpsertAsync` がトランザクションで動く（プレビュー段階では DB は一切触らない）。コピー元のリリース文脈で「全て」を選んだときはコピー先文脈の上書きを禁止して PK 衝突を未然に防ぐ仕様。

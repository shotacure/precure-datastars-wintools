...
/// <summary><see cref="MySqlConnection"/> の生成を抽象化するインターフェース。 リポジトリ層は本インターフェースを介して接続を取得するため、 テスト時にモック差し替えが可能になる。</summary>
public interface IConnectionFactory
{
    /// <summary>未オープンの <see cref="MySqlConnection"/> を返す。 呼び出し側で <c>Open</c> / <c>OpenAsync</c> を実行すること。</summary>
    MySqlConnection Create();

    /// <summary>オープン済みの <see cref="MySqlConnection"/> を返す。 呼び出し側で <c>using</c> による破棄を行うこと。</summary>
    /// <param name="ct">キャンセルトークン。</param>
    /// <returns>オープン済みの接続。</returns>
    Task<MySqlConnection> CreateOpenedAsync(CancellationToken ct = default);
}

/// <summary><see cref="IConnectionFactory"/> の MySQL 実装。 <see cref="DbConfig"/> から接続文字列を受け取り、<see cref="MySqlConnection"/> を生成する。</summary>
public sealed class MySqlConnectionFactory : IConnectionFactory
{
    private readonly DbConfig _config;

    /// <summary><see cref="MySqlConnectionFactory"/> の新しいインスタンスを生成する。</summary>

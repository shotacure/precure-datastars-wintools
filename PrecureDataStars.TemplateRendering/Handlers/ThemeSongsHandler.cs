...
public static class ThemeSongsHandler
{
    ...
    /// <param name="lookup">名義・役職 ID をリンク化済み HTML へ解決するインターフェース。</param>
    ...
    /// <param name="lookup">名義・キャラ ID → リンク化 HTML 解決インターフェース。</param>
    /// <param name="ct">キャンセルトークン。</param>
        ...
        CancellationToken ct = default)
    {
        ...
        // song_id / song_recording_id も取得して、後段の構造化クレジット解決に使う。
        // usage_actuality 列を取得し、
    ...
    private static async Task<string> RenderSingleSongBlockHtml(ThemeSongRow r, ILookupCache lookup)
    {
        ...
        // CREDITED_NOT_BROADCAST のときだけ注記。

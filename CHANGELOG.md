...
- **商品索引・楽曲索引のモバイル時に種別バッジを商品名の上に縦積み配置**：狭い画面（≤640px）でタイトル文字列が折り返したあとに右上のバッジが浮いて見える不自然な改行レイアウトを解消するため、`.products-card-grid` と `.songs-card-grid` のメディアクエリで `grid-template-columns: 1fr` の縦 1 列に倒し、`.products-card-badge-cell` / `.songs-card-badge-cell` を `order: -1` で 1 行目（タイトル上）に持ち上げる。バッジは `justify-self: start` で左寄せのラベル的扱いとし、meta 行（発売日／品番／価格／枚数）の flex 折り返しも安定する。デスクトップ表示（>640px）は従来通り 2 列 grid で右にバッジが付くレイアウトを維持。

...
### v1.3.1 — `PrecureDataStars.SiteBuilder` の UX 改善（SNS シェア導線・OGP / 構造化データ拡張・運営情報ページ整備・SEO 補助ファイル整理）

`Directory.Build.props` の `<Version>` / `<AssemblyVersion>` / `<FileVersion>` を `1.3.0` → `1.3.1` にインクリメント。v1.3.0 系列で「Web 公開できる状態」に到達したことを受け、本リビジョンでは公開後のユーザ体験まわりを横断的に整理する。**主な変更は SiteBuilder 側のみで、Catalog / Episodes 等の WinForms アプリや DB スキーマには影響しない**。

#### 全ページ共通の SNS シェアボタン群

`_layout.sbn` 末尾の `<main>` 内で共通パーシャル `_share-buttons.sbn` を `include` し、本文末尾に下記 6 種のシェアボタンを常設で表示する：

- **X (旧 Twitter)** — `https://x.com/intent/tweet?text=...&url=...&hashtags=...`
- **Facebook** — `https://www.facebook.com/sharer/sharer.php?u=...`
- **Bluesky** — `https://bsky.app/intent/compose?text={本文 + URL}`
- **はてなブックマーク** — `https://b.hatena.ne.jp/entry/{スキーマなし URL}`
- **LINE** — `https://social-plugins.line.me/lineit/share?url=...`
- **URL をコピー** — クライアントサイドの `navigator.clipboard.writeText` でコピー、未対応環境では `<textarea>` + `document.execCommand('copy')` フォールバック。コピー成功時に `#shareToast` を約 1.8 秒間フェード表示

シェア URL の組み立てはすべてサーバサイド（Scriban テンプレ）で完結。ボタンに渡す `ShareText` / `ShareUrl` / `ShareHashtags` は `PageRenderer` 側で `LayoutModel` に自動補完する：

- `ShareUrl` = `BaseUrl + CanonicalPath`（`BaseUrl` が空のときは空文字 → パーシャル側で出力ごとスキップ）
- `ShareText` = `"{PageTitle} | {SiteName}"`（PageTitle が空ならサイト名のみ）
- `ShareHashtags` = `"プリキュア,プリキュアデータベース"` を既定値として固定

URL コピーボタンに付ける軽量 JS は `wwwroot/assets/share-buttons.js` として追加し、`_layout.sbn` の末尾で `defer` 読み込み。`.share-buttons` 要素が DOM に無いページでは何もせず終了するため、グローバル読み込みでも無害。

#### OGP / 構造化データの拡張

3 系統を追加：

1. **サイト共通既定 OG 画像の自動補完** — `App.config` に `DefaultOgImage` キーを追加。設定すると `PageRenderer` が `LayoutModel.OgImage` 未指定ページに既定画像を流し込み、結果 `_layout.sbn` 側の Twitter カードが全ページ `summary_large_image` 形式となる。実体画像（推奨 1200×630 PNG）の用意・配置は運用側で行う（`wwwroot/og-default.svg` にサンプルデザインの SVG 版を同梱、PNG への書き出しは別途）。
2. **`BreadcrumbList` 構造化データの自動生成** — `PageRenderer` が既存の `Breadcrumbs` 配列を `Schema.org` の `BreadcrumbList` JSON-LD に変換し、`LayoutModel.BreadcrumbJsonLd` 経由で `_layout.sbn` に 2 本目の `<script type="application/ld+json">` として出力。位置情報を絶対 URL（`BaseUrl + 各 crumb の Url`）で組むため、検索エンジンが SERP 上にパンくず階層を表示する手がかりになる。
3. **ホーム限定の `WebSite` + `SearchAction` 構造化データ** — `home.sbn` 内に直接 `<script type="application/ld+json">` ブロックを 1 個追加。`SearchAction` の `target` URL は `/?q={search_term_string}` 形式で組み、サイト内検索の存在を構造化データで明示する。

#### SVG ファビコン + ブランドカラー

`wwwroot/favicon.svg` を新設し、`_layout.sbn` の `<head>` に `<link rel="icon" type="image/svg+xml" href="/favicon.svg">` を追加。SVG なのでアイコンのスケール劣化が発生しない。あわせて `<meta name="theme-color" content="#ec4899">` を追加し、モバイルブラウザのアドレスバー色をプリキュアピンクで統一する。

モチーフはピンクグラデの 5 角星 + 中央の白ハート（`viewBox="0 0 64 64"`）。`#ec4899` を中心に `#fce7f3 / #fbcfe8 / #f472b6 / #db2777` の同系色でまとめ、サイト全体の意匠と揃える。

#### 運営情報ページ 3 ページの新設

`PolicyPagesGenerator` を `Generators/` 直下に新設し、下記 3 ページを生成する：

- **`/privacy/`** — プライバシーポリシー。Cookie / アクセス解析 / 広告配信 / 第三者からの提供 / オプトアウト / お問い合わせ の 6 セクション構成。`PrivacyContentModel { SiteName, HasAdSense, HasGa4 }` の bool 値で、AdSense / GA4 が `App.config` で有効化されているときだけ該当節の詳細を表示する条件分岐。
- **`/disclaimer/`** — 免責事項。情報の正確性 / 著作権・知的財産権 / 外部リンク / サイト運営の変更 の 4 セクション構成。
- **`/contact/`** — お問い合わせ。X (`@shota_`) とメール (`shota@precure.tv`) の 2 窓口を `.contact-channels` の `dl` レイアウトで提示。誤情報報告時に添えてほしい情報（URL・該当箇所・参考にした一次資料）のガイダンス付き。

3 ページとも `LayoutModel.Breadcrumbs` でパンくずを設定し、上で述べた `BreadcrumbList` 構造化データの自動生成と整合する。生成は `AboutGenerator` 直後（DB 依存の無い軽量ページ群と同じタイミング）で行う：

```csharp
// SiteBuilderPipeline.cs より抜粋
new AboutGenerator(ctx, pageRenderer).Generate();
new PolicyPagesGenerator(ctx, config, pageRenderer).Generate();  // ★ v1.3.1 追加
```

`_layout.sbn` のフッタには `#copyright` の直前に `<nav class="policy-links">` を追加し、4 リンク（このサイトについて / プライバシーポリシー / 免責事項 / お問い合わせ）を `|` 区切りで横並び表示。モバイルでは折り返して縦並びになるよう `flex-wrap`。あわせて `about.sbn` 末尾に `#operations-info` セクションを追加し、本文側からも 3 ページに誘導する導線を確保。

#### `robots.txt` の多 User-agent 構成 + `ads.txt` 自動出力

`SeoGenerator` を拡張。3 ブロック構成の `robots.txt` を出力するようにした：

1. **個別の `Disallow: /` 群** — AI 学習・LLM データ収集系（`GPTBot` / `ChatGPT-User` / `ClaudeBot` / `anthropic-ai` / `Claude-Web` / `CCBot` / `Google-Extended` / `Applebot-Extended` / `Bytespider` / `PerplexityBot` 等）と、バックリンク・SEO 解析系（`AhrefsBot` / `SemrushBot` / `MJ12bot` / `DotBot` / `PetalBot` / `BLEXBot` / `DataForSeoBot` 等）の計 26 種を個別に `Disallow: /` で明示拒否。リソース消費が大きいクローラを書き分けることで、行儀の良いクローラの初回 fetch を抑制する。
2. **主要検索エンジンへの `Crawl-delay: 10`** — `Bingbot` / `Slurp` (Yahoo) / `DuckDuckBot` に対して 10 秒の控えめなクロール間隔を指定。Google 公式は `Crawl-delay` を無視するためエントリを置かない。
3. **その他全クローラ許可 + サイトマップ参照** — 従来通り `User-agent: * / Allow: /` と `Sitemap: ...` を末尾に出力。

`robots.txt` は紳士協定であり強制力は無いが、規約に従う行儀の良いクローラには初回 fetch 前に効く。実流量制御は CDN（Cloudflare 等）や WAF のレートリミットで別途行う前提。

あわせて `App.config` の `GoogleAdSenseClientId` が設定されているとき、IAB 標準形式の **`ads.txt`** を出力する：

```text
# precure-datastars ads.txt
google.com, pub-XXXXXXXXXXXXXXXX, DIRECT, f08c47fec0942fa0
```

入力値が `ca-pub-XXXX...` 形式なら `ca-` を剥がして `pub-XXXX...` に正規化、既に `pub-` で始まる値はそのまま使う。それ以外の形式は警告ログを出して書き出しスキップ（不正な `ads.txt` を出さない安全側の動作）。

#### ヒーローセクションのグラデ背景 + 統計ランキング表のメダル装飾

`home.sbn` の hero section に `hero-gradient` クラスを追加し、CSS で淡ピンク → クリームの縦グラデーション背景を当てる。要素の余白・配置は既存のまま、背景だけのバリエーション。

統計ランキング表の TOP 3 行は、CSS の `:has()` セレクタを使って **テンプレ無変更で** 金 / 銀 / 銅のメダル装飾を当てる：

```css
table:has(th.col-rank) tbody tr:nth-child(1) td.col-rank { /* 金 */ }
table:has(th.col-rank) tbody tr:nth-child(2) td.col-rank { /* 銀 */ }
table:has(th.col-rank) tbody tr:nth-child(3) td.col-rank { /* 銅 */ }
```

`:has()` はモダンブラウザ（Chrome 105+ / Safari 15.4+ / Firefox 121+）で対応済み、未対応ブラウザでは装飾だけが落ちて順位列の値表示自体には影響しない。ランキングは Wimbledon 形式（同点同順位）のため、同順位が複数いる場合は `nth-child` の特性上 TOP 3 「行枠」に対する近似装飾になる（厳密な同位者並列対応はしない）。

#### `App.config.sample` に追加した設定キー

```xml
<!-- OGP の og:image に使うサイト共通既定画像（v1.3.1 追加）。絶対 URL を指定。
     例：https://precure.tv/og-default.png。未設定時は従来通り画像未設定として扱う。
     画像実体は wwwroot/ 直下に手動配置、推奨サイズは 1200×630 px。 -->
<add key="DefaultOgImage" value="" />
```

#### エピソード詳細の `MetaDescription` 動的構築と `TVEpisode` JSON-LD 拡充

`EpisodeGenerator` のエピソード詳細ページ向け `<head>` 情報を実データから組み立てるよう改修。従来は全エピソードで「{シリーズ} 第N話「{サブタイトル}」のフォーマット表・スタッフ・主題歌情報。」という機械的な定型文を出していたが、サブタイトル以外がほぼ同じだったため検索エンジンからは重複コンテンツに近い扱いで、SERP の CTR にも反映されにくかった。

**新しい `MetaDescription` の構成**：

```
「{シリーズ}」第N話「{サブタイトル}」({YYYY年M月D日}放送)。{役職1}:{人物}、{役職2}:{人物}。{OP|ED}「{曲名}」、{OP|ED}「{曲名}」。
```

組み立てルールは下記のとおり：

- 基本骨格は「シリーズ名・話数・サブタイトル・放送日」を必ず含める
- 主要スタッフは `BuildStaffRowsAsync` の結果（脚本→絵コンテ・演出系→演出→絵コンテ→作画監督→美術監督 の順）から最大 2 役職
- 主題歌は OP / ED の 2 種のみを対象として最大 2 曲（挿入歌は字数節約のため除外）
- 各セグメント追加の前に `targetMaxChars=140` を超えないかを確認、超えそうな段で打ち切り（meta description は 120〜160 字程度で切り詰められるため保守的に置く）

具体例：

```
「ふたりはプリキュア」第1話「私たちが変身！？ありえない！」(2004年2月1日放送)。脚本:川崎良、絵コンテ・演出:西尾大介。OP「DANZEN!ふたりはプリキュア」、ED「ゲッチュウ！らぶらぶぅ?」。
```

**`TVEpisode` JSON-LD の拡充**：従来は `name` / `episodeNumber` / `datePublished` / `inLanguage` / `url` / `partOfSeries(name+url)` のみだったところに、以下のプロパティを追加した：

- `description` — 上記 `MetaDescription` と同じ文面を流用（検索エンジンが OGP と JSON-LD を整合的に解釈できるよう揃える）
- `director` — `Person` 型の配列。`BuildStaffRowsAsync` 結果から「演出」単独行または「絵コンテ・演出」統合行の人物を抽出
- `creator` — `Role` でラップした `Person` 型の配列（`roleName: "脚本"` を明示）。Google 公式ドキュメントが TVEpisode の典型例として示している入れ子構造で、SERP 上で「脚本: 〇〇」のラベル付き表示候補となる

`EpisodeGenerator.cs` に下記の private static ヘルパを 4 個追加：

- `BuildMetaDescription(series, ep, staffRows, themeRows)` — description 文字列の組み立て
- `ExtractDirectorPersons(staffRows)` — director プロパティ用の人名配列抽出（「演出」または「絵コンテ・演出」）
- `ExtractScreenplayPersons(staffRows)` — creator プロパティ用の人名配列抽出（「脚本」のみ）
- `SplitNamesLine(namesLine)` — 「、」連結文字列を個別人名配列に分割するユーティリティ

スコープを限定する観点から、`actor`（キャラクター→声優の解決）プロパティとエピソード別の動的 OG 画像は本リビジョンでは見送り、サイト共通既定 OG 画像（v1.3.1 で追加した `DefaultOgImage` 自動補完経路）に任せる方針。

#### シリーズ詳細の `MetaDescription` 動的構築と `TVSeries` / `Movie` JSON-LD 拡充

`SeriesGenerator` のシリーズ詳細ページ向け `<head>` 情報も、エピソード詳細と同じ要領で実データから組み立てるよう改修。従来は「{シリーズ} のエピソード・スタッフ・楽曲情報。」という機械的な固定文だった。

**新しい `MetaDescription` の構成**：

```
『{シリーズ}』({YYYY年}放送開始|公開[、全N話])。主役プリキュア：{変身名}({CV})、{変身名}({CV})[ほか]。プリキュアシリーズのエピソード・スタッフ・楽曲を網羅したデータベース。
```

組み立てルール：

- 「放送開始」と「公開」は `KindCode` 判定で書き分け（MOVIE / MOVIE_SHORT / SPRING は公開表記、それ以外は放送開始表記）
- 映画系は「全N話」セグメントを省略
- 主役声優は `SeriesPrecureRow` の並び（紐付け順＝主役→脇役）から最大 2 名
- 採用しなかった主役プリキュアが居る場合は「ほか」を末尾に
- 締めの定型文「プリキュアシリーズの…網羅したデータベース。」は 140 字に収まるときのみ追加

**`TVSeries` / `Movie` JSON-LD の拡充**：従来の `name / description / startDate / endDate / numberOfEpisodes / url / inLanguage` に加えて：

- `description` — `MetaDescription` と同じ文面を流用
- `genre` — `"アニメ"` を固定で付与（ジャンル絞り込みの SERP 表現で有効）
- `actor` — `Person` 型の配列。`SeriesPrecureRow` の `VoiceActorName` を重複排除して並べる

具体例：

```
『ふたりはプリキュア』(2004年放送開始、全49話)。主役プリキュア：キュアブラック(本名陽子)、キュアホワイト(ゆかな)。プリキュアシリーズのエピソード・スタッフ・楽曲を網羅したデータベース。
```

#### 人物詳細の `MetaDescription` 動的構築と `Person` JSON-LD 拡充

`PersonsGenerator` の人物詳細ページ向け `<head>` 情報も実データ化。従来は「{人名} のプリキュア関連クレジット一覧。」という固定文だった。

**新しい `MetaDescription` の構成**：

```
{人名}は、プリキュアシリーズで{役職1}({N話})・{役職2}({N話})・{役職3}({N話})などを担当。
```

組み立てルール：

- 担当役職は `BuildPersonInvolvementGroupsAsync` の結果（`InvolvementGroup.Count`＝担当エピソード数）から、担当話数の多い順で最大 3 件
- 各役職は「役職名(N話)」形式、`・` で連結
- 役職ラベルが空（"(役職未設定)"）の行は除外
- 該当役職が無い人物は従来の「{人名} のプリキュア関連クレジット一覧。」にフォールバック
- 140 字を超えそうな段で打ち切り

**`Person` JSON-LD の拡充**：従来の `name / alternateName / givenName / url` に加えて：

- `description` — `MetaDescription` と同じ文面
- `jobTitle` — 上位 3 役職の日本語ラベル（`"監督"` / `"脚本"` / `"演出"` 等）の配列。アニメスタッフが複数役職を兼ねる実態に合わせて配列で表現

#### 404 ページ（`/404.html`）の新設

新規 `NotFoundGenerator` を `Generators/` 直下に新設。出力ルート直下に `404.html` を書き出す。AWS S3 + CloudFront、Cloudflare Pages などの静的ホスティング側で「404 応答時に本ファイルを返す」設定（ErrorDocument / Custom Error Response）を別途設定して機能させる前提（本ツールはファイルを置くだけ）。

ページ構成：

- ヒーロー：「ページが見つかりませんでした」+ サイトトップへ誘導する説明文
- サイト内の主要セクション 10 リンクの一覧（ホーム / シリーズ一覧 / エピソード一覧 / プリキュア / キャラクター / 音楽 / 人物 / 企業・団体 / 統計 / このサイトについて）
- サイト内検索の案内
- リンク切れ報告先（お問い合わせページ）への誘導

`PageRenderer` に末尾スラッシュ URL 規約外のページ用ルートとして `RenderAndWriteToOutputFile` メソッドを新設（出力ファイル名と canonical パスを直接指定する形）。本メソッド経由で書き出したページは `WrittenPages` に登録されず、`sitemap.xml` には掲載されない（404 は SERP インデックス対象外であるべきため）。

#### ホームに `Organization` 構造化データを追加

`home.sbn` の既存 `WebSite` JSON-LD に並べて、サイト運営者を表す `Organization` 型の構造化データを追加した：
...
```json
{
  ...
  "url": "/",
  "logo": "/favicon.svg",
  "description": "プリキュアシリーズのエピソード・スタッフ・楽曲・キャラクター情報を横断的に閲覧できる、個人運営の非公式データベース。",
  "sameAs": ["https://x.com/shota_"]
}
```

`sameAs` に運営者の X アカウントを置くことで、Google ナレッジパネルの出典帰属で「誰のサイトか」が読み取れる手がかりを与える。`logo` は v1.3.1 で導入した `/favicon.svg` を兼用（Schema.org 仕様では PNG/JPG が推奨だが、SVG でも valid とみなす実装が多く、専用ロゴ未用意時の暫定）。

#### 残り 5 種の詳細ページの `MetaDescription` 動的構築と JSON-LD 拡充

エピソード・シリーズ・人物に続き、楽曲・商品・キャラクター・企業・プリキュアの全詳細ページにも、同じ要領で実データから組み立てた `MetaDescription` を採用する。あわせて各ページの JSON-LD にも `description` ほか各種プロパティを追加して、SERP 上のリッチスニペット候補要素を増やす。

**楽曲詳細** (`/songs/{id}/`)：

- 構成：「『{シリーズ}』の{楽曲種別}「{曲名}」。歌唱:{歌手1}、{歌手2}。作詞:{X}、作曲:{Y}。」
- 歌手は `RecordingView.SingerName` から重複排除して最大 2 名、作詞・作曲は片方しか無くてもその片方を残す
- `MusicComposition` JSON-LD に `description` / `genre: "アニメソング"` を追加（既存の `lyricist` / `composer` プロパティはそのまま温存）

**商品詳細** (`/products/{catalog_no}/`)：

- 構成：「『{タイトル}』({YYYY年M月D日}発売、{ProductKindLabel})。{N枚組、}{発売元:Label、}収録{N}曲。」
- 複数枚組のときのみ「N枚組」、発売元・トラック数も登録されていれば段付け
- `MusicAlbum` / `Product` JSON-LD に `description` を追加。`isMusicAlbum=true` のときはトラック数を `numberOfTracks` プロパティとして追加（`Product` 型では当該プロパティが定義されていないため、音楽商品のみ）

**キャラクター詳細** (`/characters/{id}/`)：

- 構成：「{キャラ名}は、プリキュアシリーズに登場する{キャラ種別}。CV:{声優1}、{声優2}など。{N作品}に出演。」
- 声優は `VoiceCastRow.VoiceActorNames`（連名連結）を `、` で割って重複排除して最大 2 名、3 名以上いれば「など」
- 出演シリーズ数は `VoiceCastRow.SeriesTitle` の Distinct カウント
- 従来 JSON-LD 未出力だったが、Schema.org の `Person` 型を流用して `name / alternateName / givenName / description / url` を出力するよう新規追加（Schema.org に専用の "Character" 型は存在せず、Google 公式ドキュメントもフィクションキャラには `Person` 流用を推奨）

**企業詳細** (`/companies/{id}/`)：

- 構成：「{会社名}は、プリキュアシリーズで{役職1}({N作品})・{役職2}({N作品})などを担当した企業・団体。」
- 役職は `InvolvementGroup.Count` 降順で最大 3 件、`(役職未設定)` は除外
- 関与役職が 1 件も無い場合は従来の「{会社名} のプリキュア関連クレジット一覧。」にフォールバック
- `Organization` JSON-LD に `description` を追加（既存の `name / alternateName / foundingDate / dissolutionDate / url` はそのまま温存）

**プリキュア詳細** (`/precures/{id}/`)：

- 構成：「{変身名}は、本名「{変身前名}」のプリキュア。CV:{声優}。誕生日:{M月D日}。」
- 変身前名・声優名・誕生日のいずれも空であってもクラッシュせずに、当該セグメントを省略するだけの設計
- 従来 JSON-LD 未出力だったが、Schema.org の `Person` 型を流用して `name / alternateName / description / url` を出力するよう新規追加。`alternateName` は変身前名・変身後 2・別形態などの全名義からメインタイトル以外を重複排除して並べる

#### アクセシビリティ強化（スキップリンク + 印刷用スタイル）

UX 改善テーマの正攻法として、アクセシビリティの基本を 2 点整備した：

**「本文へスキップ」スキップリンク** — `_layout.sbn` の `<body>` 開始直後に `<a href="#main-content" class="skip-link">本文へスキップ</a>` を配置。`<main>` 要素には `id="main-content" tabindex="-1"` を付与してスキップ先として機能させる。

通常時は CSS で画面外（`top: -48px`）に押し出して非表示にし、Tab フォーカス時のみピンク背景のラベルが画面左上に滑り込んで現れる挙動。スクリーンリーダ・キーボードユーザがヘッダ・グローバルナビを毎回読まずに本文へ到達できる。

**印刷用スタイル** — `@media print` ブロックで、ブラウザの「印刷」「PDF として保存」操作時に本文だけを綺麗に印字するよう全面整備：

- 非表示：スキップリンク・モバイルナビ各要素・サイトヘッダ・サイト内検索・左サイドセクションナビ・SNS シェアボタン・シェアトースト・フッタの政策リンク群
- 残す：パンくず（経路情報として有用）、フッタの copyright + 免責（出典明示として価値あり）
- 装飾系の背景・グラデを撤廃してインク量を節約
...
- `PrecureDataStars.SiteBuilder/Templates/disclaimer.sbn`（免責事項本文）
- `PrecureDataStars.SiteBuilder/Templates/contact.sbn`（お問い合わせ本文）
- `PrecureDataStars.SiteBuilder/wwwroot/favicon.svg`（SVG ファビコン）
- `PrecureDataStars.SiteBuilder/wwwroot/og-default.svg`（既定 OG 画像のサンプルデザイン SVG）
- `PrecureDataStars.SiteBuilder/wwwroot/assets/share-buttons.js`（URL コピー + トースト表示）
- `PrecureDataStars.SiteBuilder/Generators/NotFoundGenerator.cs`（`/404.html` 出力）
- `PrecureDataStars.SiteBuilder/Templates/not-found.sbn`（404 ページ本文）

改修：

- `Directory.Build.props`（`1.3.0` → `1.3.1`）
- `PrecureDataStars.SiteBuilder/App.config.sample`（`DefaultOgImage` キー追加、`SitePublishedYear` の説明追記）
- `PrecureDataStars.SiteBuilder/Configuration/BuildConfig.cs`（`DefaultOgImage` プロパティ + App.config 読み出し）
- `PrecureDataStars.SiteBuilder/Rendering/LayoutModel.cs`（`ShareText` / `ShareUrl` / `ShareHashtags` / `BreadcrumbJsonLd` フィールド追加）
- `PrecureDataStars.SiteBuilder/Rendering/PageRenderer.cs`（OG 画像の `DefaultOgImage` 自動補完、シェア用テキスト/URL の自動生成、`BreadcrumbList` JSON-LD 自動生成、末尾スラッシュ URL 規約外ページ用の `RenderAndWriteToOutputFile` メソッド追加）
- `PrecureDataStars.SiteBuilder/Generators/SeoGenerator.cs`（`robots.txt` を 3 ブロック構成に拡張、`ads.txt` 出力）
- `PrecureDataStars.SiteBuilder/Generators/EpisodeGenerator.cs`（`MetaDescription` 動的構築ヘルパ `BuildMetaDescription` 追加、`TVEpisode` JSON-LD に `description` / `director` / `creator` プロパティ追加、補助の人物抽出ヘルパ `ExtractDirectorPersons` / `ExtractScreenplayPersons` / `SplitNamesLine` 追加）
- `PrecureDataStars.SiteBuilder/Generators/SeriesGenerator.cs`（シリーズ詳細の `MetaDescription` 動的構築ヘルパ `BuildSeriesMetaDescription` 追加、`TVSeries` / `Movie` JSON-LD に `description` / `genre` / `actor` プロパティ追加）
- `PrecureDataStars.SiteBuilder/Generators/PersonsGenerator.cs`（人物詳細の `MetaDescription` 動的構築ヘルパ `BuildPersonMetaDescription` 追加、`Person` JSON-LD に `description` / `jobTitle` プロパティ追加）
- `PrecureDataStars.SiteBuilder/Generators/SongsGenerator.cs`（楽曲詳細の `MetaDescription` 動的構築ヘルパ `BuildSongMetaDescription` 追加、`MusicComposition` JSON-LD に `description` / `genre` プロパティ追加）
- `PrecureDataStars.SiteBuilder/Generators/ProductsGenerator.cs`（商品詳細の `MetaDescription` 動的構築ヘルパ `BuildProductMetaDescription` 追加、`MusicAlbum` JSON-LD に `description` / `numberOfTracks` プロパティ追加、`Product` 型にも `description` 追加）
- `PrecureDataStars.SiteBuilder/Generators/CharactersGenerator.cs`（キャラクター詳細の `MetaDescription` 動的構築ヘルパ `BuildCharacterMetaDescription` 追加、`Person` 型 JSON-LD 出力を新規追加：`name` / `alternateName` / `givenName` / `description` / `url`）
...
GitHub 公開ビルド準備の最終段で目視確認したデザイン上の細部 3 点を修正。`Directory.Build.props` の `<Version>` / `<AssemblyVersion>` / `<FileVersion>` は `1.3.0` 据え置き（次回コード変更からリビジョンインクリメントして `1.3.1` 系列に移行）。

#### 左サイド縦タイムラインナビのラベル幅を約 2 倍に拡張

シリーズタイトルが「ふたりはプリキ…」のように途中で切れて読みにくかったため、`.page-section-nav` の幅を `220px → 340px` に拡張。`left` の中心引きも `(100vw - 960px) / 2 - 240px` → `(100vw - 960px) / 2 - 360px` に追従させ、ワイドデスクトップ（≥1440px）では本文 container と被らない位置取りを維持。1180〜1439px の中間幅では従来通り z-index で nav を本文前面に出してラベル全体が読めることを優先する（本文への被りは既存仕様と同じ運用）。

#### `/episodes/` ランディングを 1 行 3 列レイアウトに再構成

旧仕様は「メイン段 + スタッフ段」の 2 段構成（スタッフ段はメイン段の下に折り返し）。サブタイトル右の空白が活用しきれない・スタッフが折り返しで段数を取りすぎる、という観点で 1 行 3 列に組み直した：

- **左列（5em）**：第N話 + 放送日 を縦積みでひとブロック化（`.ep-row-no-date`）。テキストは flex-end で右寄せして列幅にぴたっと揃える
- **中央列（1fr）**：サブタイトル
- **右列（1fr）**：スタッフバッジ群を 1 行ぶんの font-size `0.7em` で密表示

スコープは `.episodes-index-list .ep-row-main` の指定のみに留め、シリーズ詳細の `dl.ep-list` には影響しない（あちらは別 grid を `<dt>` / `<dd>` 自身に当てているため）。モバイル（≤640px）では従来通り縦積みフォールバック、スタッフ段の font-size は読みやすさのため `0.78em` に戻す。

#### ホーム最新／予定エピソード行の改行崩れと「(年)」位置の修正

旧仕様：`.home-episode-list li` が `grid-template-columns: 11em 16em 1fr` の 3 列、テンプレ側で 4 要素（日付・シリーズ名・年・エピソードタイトル）を auto-flow で流す構造。結果、3 要素目の「(2026)」が広い 1fr 列（col 3）の左端に貼り付いてシリーズ名から離れ、4 要素目のエピソードタイトルが次行の狭い日付列（11em）に流れ込んで「森亜るるかの秘\n密」のように途中で折り返していた。

新仕様：明示的に `grid-row` / `grid-column` を当て直して 2 段構成にする。HTML テンプレは無変更、CSS のみで修正：

- **row 1**：`[日付(11em)] [シリーズ名(auto)] [年(1fr, justify-self: start)]`
  - 年セルを 1fr 列に `justify-self: start` で配置することで、auto 幅のシリーズ名列の右端に貼り付くように描画される
  - 結果「名探偵プリキュア！(2026)」のように間隔が詰まる
- **row 2**：エピソードタイトル（col 2 から末尾までスパン）
  - col 1（日付ぶん）は空けて視覚的にインデント、col 2 以降にスパンすることで十分な幅を確保
  - 結果「第15話　森亜るるかの秘密」が 1 行で収まる

モバイル（≤640px）の縦積みフォールバックでは、デスクトップで指定した `grid-column` / `grid-row` を `auto` にリセットして暗黙列が生成されないようにする。

#### ホーム音楽商品リスト：種別オーバーフローの修正

旧仕様：`.home-product-kind` に `white-space: nowrap` が指定されていて、列幅 8em に収まらない長い種別（「オリジナル・サウンドトラック」など 13 字）がタイトル列に食い込んで重なり表示になっていた。

新仕様：

- 列幅を `8em → 9em` にわずかに広げ、短い種別（「主題歌シングル」「ボーカルアルバム」等の 7 字）でもバランス良く収まるように
- `.home-product-kind` から `white-space: nowrap` を撤廃し、入りきらないラベルは「・」「ー」等の自然な改行ポイントで折り返す
- `line-height: 1.4` を指定して、2 行になっても日付行とベースラインが大きく崩れないように

#### 変更ファイル一覧

- `PrecureDataStars.SiteBuilder/Templates/episodes-index.sbn`（`<li>` を 3 列構成に再構成、`.ep-row-no-date` 導入）
- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`（`.page-section-nav` 幅 220→340 / 左寄せ追従、`.episodes-index-list .ep-row-main` 3 列グリッド、`.ep-row-no-date` 縦積み、`.episodes-index-list .ep-row .staff-badges-row` 70% フォント、`.home-episode-list` を 4 要素対応の 2 段グリッドに再構成 + モバイル grid placement リセット、`.home-product-kind` nowrap 撤廃 + kind 列幅 8em→9em）
- `README.md`（本エントリ追加）

### v1.3.0 公開直前のデザイン整理 第 N+3 弾 — 人物一覧の 2 タブ化（50音順 / 初クレジット順）・企業一覧の初クレジット順化・クレジット履歴の主題歌種別反映

`Directory.Build.props` の `<Version>` / `<AssemblyVersion>` / `<FileVersion>` は **`1.3.0` のまま据え置き**。v1.3.0 公開直前のサイトテンプレ調整シリーズ（stage 21・続編・公開直前のデザイン整理）と同じ位置づけで、v1.3.0 系列内の継続作業として扱う。

#### 人物一覧（`/persons/`）の 2 タブ化（50音順 / 初クレジット順）

人物索引ページを「50音順」と「初クレジット順」の 2 タブ構成に再編した。`/products/` の `.products-tabs` と同方式のインライン JavaScript でタブ切替を実装し、`section-nav.js`（左サイドの縦タイムライン型ナビ）はアクティブなタブパネル配下の `section[id]` を `MutationObserver` で拾うので、タブ切替時にナビが自動的に再構築される。

**「50音順」タブ（既定）の仕様**：

- セクション分け = `あ・か・さ・た・な・は・ま・や・ら・わ・その他` の最大 11 セクション（メンバー 0 件のセクションは出さない）。
- セクションキーの判定は `PersonsGenerator.GetKanaRowKey(string kana)` 静的ヘルパで以下のように正規化する：
  - カタカナ（U+30A1〜U+30F6）はひらがな（U+3041〜U+3096）に寄せる。
  - `ゔ` は「わ」セクション、`ゐ` / `ゑ` は「あ」セクション。
  - 濁音・半濁音（`が` `ば` `ぴ` 等）は対応する清音範囲に自然に振り分けられる（ユニコードコードポイント連続範囲を利用）。
  - 小書き仮名（`ぁ・ぃ・ぅ・ぇ・ぉ・ゃ・ゅ・ょ・ゎ・っ`）も対応する清音セクションへ。
  - 数字・アルファベット・記号など想定外文字、および `name_kana` 空は「その他」。
- 各セクション内のソートは kana 昇順 → 名前 Ordinal 昇順。
- クレジット 0 件の人物も含めて全人物が必ずどこかのセクションに表示される（無関与は muted で詳細リンクなし、従来仕様を踏襲）。

**「初クレジット順」タブの仕様**：

- セクション = 当該人物が最初にクレジットされたシリーズ。セクション順はシリーズ放送開始日昇順。
- 「初クレジット」の判定は `PersonsGenerator.ComputeFirstCreditSeries(aliasIds)` で、`InvolvementKind.Person` / `InvolvementKind.CharacterVoice` の Involvement のみを対象に `(シリーズ放送開始日, シリーズ内話数)` の最小値を採用。シリーズ全体スコープ（EpisodeId=null）は話数 0 として優先扱い。
- 各セクション内のソートは「シリーズ内初登場話数昇順 → kana → 名前」。
- クレジット 0 件の人物はこのタブには載せない（「クレジット未登録」セクションは作らない方針）。

**DTO / テンプレ**：

- `PersonsGenerator` の `PersonIndexModel` から旧 `Persons` フラットリストを撤去し、`KanaSections` / `DebutSections` の 2 セクション群を持たせる。新規 nested class `PersonIndexKanaSection` / `PersonIndexDebutSection` を追加。各 row には `FirstCreditSeriesId` / `FirstCreditSeriesEpNo` を載せて、両セクションのソートに使う。
- `Templates/persons-index.sbn` を全面書き換え。`.persons-tabs.products-tabs` 構造（既存 `.products-tabs` の CSS を流用）の中に `.tab-panel#persons-tab-panel-kana` と `.tab-panel#persons-tab-panel-debut` を並べ、末尾にインラインスクリプトでタブ切替を実装。各 `section` に `data-section-nav-label` / `data-section-nav-year` / `data-section-nav-count` を付与して左サイドナビ自動チップ化に対応。

#### 企業・団体一覧（`/companies/`）の初クレジット順化

企業索引ページを「初クレジット順セクション 1 軸（タブ無し）」に再編。50 音順表示は廃止（数が増えてきたら改めて検討する想定）。

- セクション = 当該企業が最初にクレジットされたシリーズ。セクション順はシリーズ放送開始日昇順。
- 「初クレジット」の判定は `CompaniesGenerator.GenerateAsync` 内で、当該企業の全屋号と配下ロゴから集めた Involvement（Member 種別は除外）の `(シリーズ放送開始日, シリーズ内話数)` の最小値。シリーズ全体スコープは話数 0 として優先扱い。
- 各セクション内のソートは「シリーズ内初登場話数昇順 → kana → 名前」。
- **クレジット 0 件の企業は索引から完全除外**する。詳細ページ `/companies/{companyId}/` は引き続き生成するため、直リンクは切れない。
- `CompanyIndexModel` から旧 `Companies` フラットリストを撤去し、`DebutSections` を持たせる。新規 nested class `CompanyIndexDebutSection` を追加。`CompanyIndexRow` に `FirstCreditSeriesId` / `FirstCreditSeriesEpNo` を追加。
- `Templates/companies-index.sbn` をシリーズ別セクション形式に全面書き換え。各 `section` に `data-section-nav-*` 属性を付与。

#### 人物詳細クレジット履歴の主題歌種別（OP / ED / 挿入歌 / 劇伴）反映

旧仕様では、主題歌の作詞・作曲・編曲を担当した人物の詳細クレジット履歴で、役職名が単に「作詞」「作曲」「編曲」と表示されていた。OP の作曲家と ED の作曲家が同一人物の場合、両方の関与が同じ「作曲」セクションに合流してしまい、どちらがどちらか判別できない問題があった。これを「オープニング主題歌 作曲」「エンディング主題歌 作曲」「挿入歌 作曲」のように、主題歌種別をプレフィックスとして冠する形に改修した。

`song_music_classes.name_ja` をそのまま流用するので、`OP` → `オープニング主題歌`、`ED` → `エンディング主題歌`、`INSERT` → `挿入歌` の自然な日本語ラベルになる（`episode_theme_songs.theme_kind` は形式上 ENUM `('OP','ED','INSERT')` だが、コード値が `song_music_classes.class_code` と一致するためマスタを流用できる）。

劇伴系（`bgm_cue_credits` 由来）は `song_music_classes` の対象外なので、ハードコードで「劇伴」プレフィックスを冠する：「劇伴 作曲」「劇伴 編曲」。`credit_block_entries` 由来（脚本・演出・絵コンテなど）は従来通りプレフィックス無し。

**実装詳細**：

- `Pipeline/CreditInvolvementIndex.cs`：
  - `Involvement` クラスに `string? ThemeKind` プロパティを追加。
  - `song_credits` 巡回 SQL と `song_recording_singers` 巡回 SQL の SELECT 句に `ets.theme_kind AS ThemeKind` を追加。
  - DTO `SongCreditInvRow` / `SingerInvRow` に `ThemeKind` プロパティを追加。
  - 主題歌系 Involvement 生成箇所（song_credits の 1 箇所、song_recording_singers の PERSON 主名義・スラッシュ並列・CHARACTER_WITH_CV 主キャラ・スラッシュ並列キャラの計 4 箇所＋1 箇所）すべてに `ThemeKind = r.ThemeKind` を設定。
  - BGM 系（`bgm_cue_credits` 由来）の Involvement は `ThemeKind` を設定しない（null のまま）。CreditKind が `BGM` であることをマーカーに、後段で「劇伴」プレフィックスを冠する。
- `Generators/PersonsGenerator.cs`：
  - `SongMusicClassesRepository` をコンストラクタで生成し、`GenerateAsync` の冒頭で `song_music_classes` 全件を 1 回だけロードして `_songMusicClassMap` に保持。
  - `BuildPersonInvolvementGroupsAsync` のグループキーを `RoleCode` 単独から `(CategoryPrefix, RoleCode)` の複合に拡張。`CategoryPrefix` の判定は `EntryKind` を見て：
    - `SONG_CREDIT` / `RECORDING_SINGER` → `ThemeKind`（`OP` / `ED` / `INSERT`）
    - `BGM_CUE_CREDIT` → `BGM`
    - その他（credit_block_entries 由来）→ 空文字（プレフィックス無し）
  - 表示順は「カテゴリプレフィックス順 → 役職 `display_order` 順」。カテゴリプレフィックス順は `song_music_classes.display_order` を踏襲（プレフィックス無し=0、OP=1、ED=2、INSERT=3、BGM=99）。
  - 役職ラベルは「{プレフィックス日本語名} {役職 NameJa}」を半角スペース連結（例：「オープニング主題歌 作曲」）。プレフィックス無しは従来通り役職 NameJa のみ。マスタ未登録の不明コードはコード値そのまま暫定表示。
  - `BuildPersonRolesLabel`（人物一覧の役職カラム）も同じく `(CategoryPrefix, RoleCode)` 複合キーで集計するように改修。一覧と詳細でラベル表記を合わせる。

これにより、たとえば OP 作曲・ED 作曲・挿入歌 作曲を兼任した作曲家の人物詳細ページでは：
...
のように 3 つの独立した役職セクションに分かれて表示されるようになる。

#### シリーズ別 記号出現回数（`/stats/subtitles/symbols-by-series/`）の動的集計化

旧仕様では `SubtitleStatsRepository.GetSymbolCountsBySeriesAsync` で 16 種類の記号（`!`・`?`・`・`・`〜`・`&`・`(`・`)`・`…`・`、`・`♪`・`☆`・`♡`・`。`・`♥`・`「`・`」`）を SQL 内に決め打ちで SUM 集計しており、新たな記号がサブタイトルに使われても集計に載らない構造だった。テンプレートも 16 個の `<th>` がハードコーディングされ、`r.Symbols.Exclamation` 形式の固定参照になっていた。

新仕様では、歴代サブタイトルの `title_char_stats` から**実際に登場した記号・句読点を全種類**動的に収集し、**初出が早い順**で列を並べ、種類数（カラム数）も毎回カウントしてタイトル・説明文に反映する。`/stats/subtitles/chars/symbols-order/`（記号出現回数、全件初使用順）と同じ「記号」判定 REGEXP（`\p{Han}|[々]|\p{Hiragana}|\p{Katakana}|[A-Za-z]|[0-9]|[ 　]` の否定）を使うので、句読点（`、`・`。`・`「`・`」`）・絵文字・特殊記号などが過不足なく含まれる。

**実装詳細**：

- `PrecureDataStars.Data/Repositories/SubtitleStatsRepository.cs`：
  - 旧 `GetSymbolCountsBySeriesAsync`（16 種固定 SQL）と DTO `SeriesSymbolRow` を **削除**。
  - 新メソッド `GetSeriesSymbolCellsAsync(CancellationToken)` を追加。`episodes.title_char_stats` JSON を `JSON_TABLE` で展開し、上記 REGEXP で記号のみ抽出して `(SeriesId, SeriesTitle, SeriesSlug, Char, Cnt)` フラットセル行群を返す。`CONVERT(...) COLLATE utf8mb4_bin` で同一文字保証。
  - 新 DTO `SeriesSymbolCell` を追加。
- `PrecureDataStars.SiteBuilder/Generators/SubtitleStatsGenerator.cs`：
  - `GenerateSymbolsBySeriesAsync` を全面書き換え：
    1. 既存 `GetSymbolsByFirstAppearAsync` で記号一覧を初出順で取得（SQL 側で `ORDER BY FirstBroadcastDate, FirstEpisodeId, Char` 済み）。これを列順序の正典とする。
    2. 新 `GetSeriesSymbolCellsAsync` で `(SeriesId, Char) → Cnt` の辞書を物理化。
    3. シリーズごとに、列順序と同じ並びでセル値配列（無関与記号は 0）を組み立て。
  - ページタイトルは `$"シリーズ別 記号 {symbolCount} 種出現回数"` で動的化。パンくず短縮ラベルは固定「シリーズ別 記号出現回数」（列数の変動でリンクテキストが断片化しないように）。
- `PrecureDataStars.SiteBuilder/Templates/stats-subtitles-symbols-by-series.sbn`：
  - `<h1>シリーズ別 記号 {{ SymbolCount }} 種出現回数</h1>` と説明文の「{{ SymbolCount }} 種」を動的化。
  - `<thead>` の `<th>` 群を `{{~ for sym in Symbols ~}}<th class="col-sym">{{ sym | html.escape }}</th>{{~ end ~}}` で動的描画。
  - `<tbody>` の `<td>` 群を `{{~ for c in r.Cells ~}}<td class="col-sym">{{ if c > 0 }}{{ c }}{{ end }}</td>{{~ end ~}}` で動的描画。横スクロールは既存 `.stats-symbols-scroll` CSS をそのまま流用。
- `PrecureDataStars.SiteBuilder/Templates/stats-subtitles-index.sbn`：
  - 索引リンク文言を「シリーズ別 記号 16 種出現回数」→「シリーズ別 記号出現回数」に修正、説明文を「歴代サブタイトルから自動収集した全記号を初出順に」に更新。

これにより、新しい記号がサブタイトルに登場するたびに列が自動的に追加され、運用者が SQL や DTO・テンプレを書き換える必要がなくなる。逆に運用上「列が想定より多すぎる／少なすぎる」場合はそれ自体がデータ品質サインになり、`title_char_stats` の再計算や名寄せ作業のきっかけになる。

#### スタッフ表示の役職色付きバッジ化（サイト全体で色マッピング固定）と関連レイアウト統一

シリーズ一覧 TV サブ行のスタッフ欄が「役職テキストラベル + 担当エピソード最多 1 名のみ + 役職/名義リンクなし + 所属屋号なし」の貧弱な表示だったのを、**色付き役職バッジ + 連名全員 + 役職/人物リンク + 所属屋号 muted 併記**の構造化表示に格上げ。同じ役職バッジの仕組みをサイト全体（`/episodes/` ランディング、シリーズ詳細のエピソード一覧）にも展開し、エピソードレベルのスタッフ表示も統一化した。あわせて映画行のバッジ淡色化・列幅タイト化・タイトル折り返し禁止も実施。

##### 役職カラーマッピング（サイト全体で固定）

`site.css` の `[data-role-code="..."]` 属性セレクタベースでサイト全体に適用される統一カラー：

| 色 | CSS 変数 | 役職コード（シリーズ役職 ⇔ エピソード役職） |
|---|---|---|
| 紫 | `--accent-purple` | `PRODUCER`（プロデューサー） |
| 青 | `--accent-blue` | `SERIES_COMPOSITION`（シリーズ構成）／`SCREENPLAY`（脚本） |
| ティール | `--accent-teal`（新規追加 `#2ea7ad`） | `STORYBOARD`（絵コンテ） |
| ピンク | `--accent-pink` | `SERIES_DIRECTOR`（シリーズディレクター）／`EPISODE_DIRECTOR`（演出） |
| 緑 | `--accent-green` | `CHARACTER_DESIGN`（キャラクターデザイン）／`ANIMATION_DIRECTOR`（作画監督） |
| 黄 | `--accent-yellow` | `ART_DESIGN`（美術デザイン）／`ART_DIRECTOR`（美術） |

シリーズ役職とエピソード役職を意味的なペアで同色にすることで、ページ間を跨いだ視覚的な意味の連続性を担保（脚本 ⇔ シリーズ構成、演出 ⇔ シリーズディレクター 等）。絵コンテは単独役職だが、機能的に近い演出（ピンク）と中間色のティールで隣接させ、シリーズ一覧 TV のサブ行とエピソード行で並んだとき自然に「青系（脚本）→ ティール（絵コンテ）→ ピンク（演出）」のグラデーションを構成する。未登録の役職コードは灰色のフォールバックバッジへ。

`.role-badge` 基本クラスに `.role-badge-sm`（密表示用の縮小版）を併設。バッジはリンク (`<a>`) として `/stats/roles/{repCode}/` に飛ばす（代表 role_code は `RoleSuccessorResolver.GetRepresentative` で解決）。

##### シリーズ一覧（`/series/`）TV サブ行のスタッフ欄構造化

`SeriesGenerator.BuildKeyStaffSummaryBySeriesCacheAsync` を全面書き直し、5 役職（PRODUCER / SERIES_COMPOSITION / SERIES_DIRECTOR / CHARACTER_DESIGN / ART_DESIGN）について：

- 当該シリーズ内全人物の Involvement を `(SeriesId, RoleCode)` で抽出
- 「担当エピソード数 desc → kana asc → name Ordinal asc」でソート、**連名全員列挙**（cap なし）
- 各人物の所属屋号 alias_id を Involvement.AffiliationCompanyAliasId から集計、当該シリーズ内最頻のものを `CompanyAliasesRepository` 経由で名前解決
- 代表 role_code（`RoleSuccessorResolver.GetRepresentative`）も詰める（バッジリンク URL 用）

旧 `_keyStaffSummaryBySeriesCache`（`Dictionary<int, string>`、単一文字列）を `Dictionary<int, IReadOnlyList<KeyStaffRoleGroup>>` に型変更。新規 nested DTO `KeyStaffRoleGroup`（RoleCode, RepRoleCode, RoleLabel, Members）と `KeyStaffMember`（PersonId, DisplayName, AffiliationLabel, EpisodeCount, SortKey）を追加。`TvSeriesRow.KeyStaffSummary` も `string` → `IReadOnlyList<KeyStaffRoleGroup>` に型変更。`CompanyAliasesRepository` を `SeriesGenerator` のコンストラクタで生成し、全屋号を 1 度だけロードして `_companyAliasNameMapCache` にキャッシュ。

テンプレ `series-index.sbn` の TV サブ行は、既存の「[スタッフ]」固定幅ラベル + 内容の sub-row-line レイアウトを維持しつつ、内容部分を新 `staff-badges-row` 構造に置き換え：

```html
<a class="role-badge role-badge-sm" data-role-code="PRODUCER" href="/stats/roles/PRODUCER/">プロデューサー</a>
<a class="staff-name" href="/persons/123/">西尾萌黄</a><span class="staff-affiliation muted">（東映アニメーション）</span>、
<a class="staff-name" href="/persons/456/">鷲尾天</a>
```

役職グループは `staff-badge-group` で 1 単位にまとめ、グループ間で flex-wrap、グループ内では人物が「、」区切り。

##### `/episodes/` エピソード一覧ランディングのスタッフ段追加

旧仕様は「第N話 / サブタイトル / 放送日」の 3 列グリッドで、サブタイトル右に大きな空白領域が残っていた。この空白を埋める形で、各エピソード行を「メイン段 + スタッフ段」の 2 段構成に拡張：

- メイン段 `.ep-row-main`：第N話（4em）+ サブタイトル（1fr）+ 放送日（7em）。放送日は密表示用に **2024.2.4 形式**に短縮（旧「2024年2月4日」→ 新「2024.2.4」、月日は 0 詰めしない）
- スタッフ段 `.staff-badges-row`：5 役職（脚本／絵コンテ／演出／作画監督／美術）を `.role-badge.role-badge-sm` で色付きバッジ表示、人物名は既存 `StaffNameLinkResolver` でリンク済み HTML を流用
- 絵コンテ＝演出（同一人物）のときは バッジ 2 個並び + 名前 1 回の統合表示（既存 `StoryboardDirectorMerged` フラグ活用）

実装の要点はスタッフデータの調達経路。`SeriesGenerator.ExtractStaffSummaryAsync` が既にシリーズ詳細ページ生成時にエピソード単位 staff サマリを抽出済みなので、その結果を memoize して `/episodes/` に渡す形で再走査を避けた：

- `EpisodeStaffSummary` 型を `SeriesGenerator` の private nested class から独立した公開クラス `PrecureDataStars.SiteBuilder/Utilities/EpisodeStaffSummary.cs` に外出し
- `SeriesGenerator.ExtractStaffSummaryAsync` に `Dictionary<int, EpisodeStaffSummary> _episodeStaffByIdCache` への memoize を追加（同一 episode_id 再呼び出しはキャッシュから返す）
- 公開メソッド `SeriesGenerator.GetEpisodeStaffSummaries()` を追加
- `SiteBuilderPipeline.RunAsync` で SeriesGenerator のインスタンスを変数保持し、`EpisodesIndexGenerator` のコンストラクタへ `IReadOnlyDictionary<int, EpisodeStaffSummary>` として渡す
- `EpisodesIndexGenerator` は受け取ったキャッシュから episode_id ベースで `EpisodesIndexRow` のスタッフ HTML 群を組み立て

`SeriesGenerator.FormatJpDate` も `2024.2.4` 形式に変更（シリーズ詳細 dl.ep-list と /episodes/ で表記揃え）。

##### シリーズ詳細（`/series/{slug}/`）のエピソード一覧（`dl.ep-list`）

既存 `<dt class="ep-title">` + `<dd class="ep-meta">` の構造を維持しつつ、`<dt>` の中に放送日も入れて 3 カラム化（話数・サブタイトル・放送日）、`<dd>` の中身を `staff-badges-row` 構造に書き換え。旧 `<span class="ep-staff"><span class="ep-staff-label">脚本</span> ○○</span>` 形式を `<span class="staff-badge-group"><a class="role-badge role-badge-sm" data-role-code="SCREENPLAY" href="/stats/roles/SCREENPLAY/">脚本</a>○○</span>` 形式に置き換え、`/episodes/` と完全に同じ見た目に統一。`StoryboardDirectorMerged` の処理も /episodes/ と揃え、絵コンテと演出のバッジ 2 個並びに。

##### シリーズ一覧 映画セクションの目疲れ軽減＋レイアウト調整

- **バッジ淡色化**：旧 `.movie-badge-fall` の紅葉色グラデーション（`#d97706` → `#c2410c`、白文字）が「目がチカチカする」フィードバックだったため撤廃。新仕様は単色パステル：
  - `.movie-badge-fall` → 水色パステル `rgba(135, 206, 235, 0.42)` 背景＋濃紺文字 `#1f5b80`
  - `.movie-badge-spring` → 薄ピンクパステル `rgba(255, 192, 203, 0.55)` 背景＋濃ピンク文字 `#8b3a62`
  - グラデーション撤廃、彩度を大きく落として情報のメインであるタイトルが前に出るように
- **タイトル折り返し禁止**：長尺タイトル（「映画ふたりはプリキュア Max Heart 雪空のともだち」等）が col-title 内で折り返してレイアウトを壊していたので `.series-tv-table tr.movie-row td.col-title { white-space: nowrap; }` を追加。子作品行（`.movie-child-row .col-title-child`）も同様
- **列幅タイト化**：TV シリーズと共有していた 22em / 7em の colgroup を映画用にオーバーライド：
  - `.series-movie-table colgroup .col-period { width: 9.5em; }` ←「2024年12月31日」最長 11 文字相当
  - `.series-movie-table colgroup .col-episodes { width: 8em; }` ←「2時間04分58秒」最長 9 文字相当

#### 変更ファイル一覧

- `PrecureDataStars.Data/Repositories/SubtitleStatsRepository.cs`（旧 16 種固定メソッド/DTO 削除、新 `GetSeriesSymbolCellsAsync` / `SeriesSymbolCell` 追加）
- `PrecureDataStars.SiteBuilder/Pipeline/CreditInvolvementIndex.cs`（Involvement に ThemeKind 追加、SQL に theme_kind 追加、DTO 拡張、4 箇所の Involvement 生成箇所更新）
- `PrecureDataStars.SiteBuilder/Pipeline/SiteBuilderPipeline.cs`（SeriesGenerator インスタンス保持、EpisodesIndexGenerator への staff cache 連携）
- `PrecureDataStars.SiteBuilder/Utilities/EpisodeStaffSummary.cs`（**新規**、SeriesGenerator private nested class からの外出し）
- `PrecureDataStars.SiteBuilder/Generators/PersonsGenerator.cs`（SongMusicClasses 注入、グループキー複合化、ロールラベル組み立て、50音順/初クレジット順セクション、DTO 再設計、ヘルパー 4 件追加）
- `PrecureDataStars.SiteBuilder/Generators/CompaniesGenerator.cs`（初クレジット順セクション、無関与企業除外、DTO 再設計、ヘルパー 1 件追加）
- `PrecureDataStars.SiteBuilder/Generators/SeriesGenerator.cs`（CompanyAliasesRepository 注入、`BuildKeyStaffSummaryBySeriesCacheAsync` 構造化、`KeyStaffRoleGroup`/`KeyStaffMember` DTO 追加、`TvSeriesRow.KeyStaffSummary` 型変更、`ExtractStaffSummaryAsync` memoize、`GetEpisodeStaffSummaries()` 公開、`FormatJpDate` を 2024.2.4 形式化、旧 nested `EpisodeStaffSummary` 削除）
- `PrecureDataStars.SiteBuilder/Generators/EpisodesIndexGenerator.cs`（コンストラクタで staff cache 受け取り、`EpisodesIndexRow` にスタッフ HTML 群追加、放送日 2024.2.4 形式化）
- `PrecureDataStars.SiteBuilder/Generators/SubtitleStatsGenerator.cs`（`GenerateSymbolsBySeriesAsync` を動的版に全面書き換え）
- `PrecureDataStars.SiteBuilder/Templates/persons-index.sbn`（2 タブ UI に全面書き換え）
- `PrecureDataStars.SiteBuilder/Templates/companies-index.sbn`（シリーズ別セクション形式に全面書き換え）
- `PrecureDataStars.SiteBuilder/Templates/series-index.sbn`（TV サブ行スタッフ欄を staff-badges-row + role-badge 構造に書き換え）
- `PrecureDataStars.SiteBuilder/Templates/episodes-index.sbn`（メイン段 + スタッフ段の 2 段構成に書き換え）
- `PrecureDataStars.SiteBuilder/Templates/series-detail.sbn`（dl.ep-list の `<dd>` を staff-badges-row + role-badge 構造に書き換え、`<dt>` に放送日を統合）
- `PrecureDataStars.SiteBuilder/Templates/stats-subtitles-symbols-by-series.sbn`（動的描画版に書き換え）
- `PrecureDataStars.SiteBuilder/Templates/stats-subtitles-index.sbn`（リンク文言更新）
- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`（`--accent-teal` 追加、`.role-badge` + `.role-badge-sm` 10 役職 5 色マッピング、`.staff-badges-row`/`.staff-badge-group`/`.staff-name`/`.staff-affiliation`、`.ep-row` 2 段レイアウト、`.movie-badge-fall`/`.movie-badge-spring` 淡色化、`.series-movie-table` 列幅・nowrap）
- `README.md`（本エントリ追加）

### v1.3.0 公開直前のデザイン整理 第 N+2 弾 — エピソード詳細クレジットの不具合修正＆リッチ化、主題歌・挿入歌セクションの構造化表示、OP/ED 見出しの装飾バグ修正

#### エピソード詳細クレジット：sibling-role 二重出力の修正（「漫画」が連載直下に空表示で出るバグ）

`role_templates` の DSL は `{ROLE:CODE.PLACEHOLDER}` で同一 Group 内の別役職を**参照（消費）**する仕組みを持つ。例：『プリキュア』シリーズの連載クレジットでは、SERIALIZED_IN ロールのテンプレが `{ROLE:MANGA.PERSONS}` で同 Group の MANGA ロールの人物名義を自分のセル内に埋め込む構造になっており、結果として「連載 ‖ 講談社「なかよし」／漫画・上北 ふたご／「たのしい幼稚園」／「おともだち」ほか」のような **1 つの「連載」行**にまとめて表示される。

しかし旧実装では、MANGA ロール自身もメインループでレンダリングされていたため、連載行の下に**空の「漫画」行**が単独で出てしまう不具合があった。

修正（`CreditTreeRenderer.cs`）：

- Group 内の `cardRoles` を本ループで描画する前に、配下の全ロールの `role_templates.format_template` を事前スキャン。`{ROLE:CODE.` パターンで参照される target `role_code` を集めた `consumedRoleCodes` セットを作る。
- メインループで `cardRoles` を回す際、`consumedRoleCodes` に含まれる `role_code` を持つロールは描画スキップする。
- 軽量検出はテンプレ文字列の `IndexOf` ベース（正規の AST 解析は描画時に `RoleTemplateRenderer` が行う）。AST を再パースせずとも「参照されているか」だけは正確に判定できる。

#### エピソード詳細クレジット：キャスティング協力の役職リンク化

VOICE_CAST テーブル末尾に追記される「協力」行（CASTING_COOPERATION 由来）は、これまで `<td class="character-cell">協力</td>` の素テキスト固定だったため、他のクレジット要素と違い**クリックできない**状態だった。

修正（`CreditTreeRenderer.cs`）：協力ラベルを `BuildRoleNameHtml("CASTING_COOPERATION", roleName, roleMap)` 経由で `/stats/roles/CASTING_COOPERATION/` リンク化済み HTML に置換。`roles` マスタに登録された日本語名（`name_ja`、通常「協力」）を採用し、未登録時は素のフォールバック文字列「協力」（リンクなし）になる。

#### エピソード詳細クレジット：声の出演キャラ名のリンク化

`fallback-vc-table` の `character-cell` でキャラ名はずっと素テキスト（`Esc(charLabel)`）で出力されていた。コメントには「キャラ詳細リンクは将来対応」と書かれていたが、本リリースで対応完了。

修正：

- `LookupCache` に `LookupCharacterIdFromAliasAsync(int aliasId)` を新設（`character_aliases.character_id` を返す）。
- `CreditTreeRenderer` に `ResolveCharacterLabelHtmlAsync` を新設し、CharacterAliasId → CharacterId を解決できれば `<a href="/characters/{character_id}/">{Esc(name)}</a>` を返す。解決できなければ従来通り素テキスト。
- VOICE_CAST フォールバックのキャラ名出力部で `Esc(charLabel)` を `charHtml`（リンク済み HTML）に置換。前行同名比較（`sameAsPrev`）には引き続き素テキスト `charLabel` を使う（HTML 文字列を比較対象にすると aria リンクのありなしで誤判定するため）。

#### エピソード詳細クレジット：役職名・キャラ名カラム幅の統一（ガタつき防止）

クレジット階層は「カード > ティア > グループ > ロール > ブロック > エントリ」と入れ子構造で、各ロールが独立した `<table class="fallback-table">` または `<table class="fallback-vc-table">` を生成する。それぞれの table のカラムは内容に応じて shrink-wrap されるため、隣接ロール間で「役職名カラム」「キャラ名カラム」の右端位置がガタついて見える問題があった。

修正：

- 新規 JS `wwwroot/assets/credit-align.js`（defer 読み込み）を追加。`.credit-section` 配下のすべての `<td class="role-name">` の最大幅と `<td class="character-cell">` の最大幅を `getBoundingClientRect().width` で測定し、CSS 変数 `--credit-role-name-w` / `--credit-character-cell-w` を `.credit-section` 自身の `style` 属性に書き込む。
- CSS 側：`.credit-section table.fallback-table td.role-name`、`.credit-section table.fallback-vc-table td.role-name`、`.credit-section table.fallback-vc-table td.character-cell` の `width` を `var(--credit-role-name-w, auto)` / `var(--credit-character-cell-w, auto)` で指定。fallback の `min-width`（役職名 8em、キャラ名 6em）も残しておき、JS が動かない環境でも下限幅を保証する。
- `document.fonts.ready` を待ってから計測（Kiwi Maru などフォント読み込み完了前に測ると幅が変わるため）。`ResizeObserver` でウィンドウサイズ変更時にも再計測。
- `colspan` を持つセル（leading-company 行）は計測対象から除外。

これにより、クレジット内のすべての役職名・キャラ名カラムが右端で揃い、視認性が大幅に向上する。

#### エピソード詳細：主題歌・挿入歌セクションを構造化クレジット表示に拡張

旧仕様の主題歌・挿入歌セクションは「OP「タイトル」　うた：歌唱者」のような **1 行縦リスト**形式で、`SongRecording.SingerName` のフリーテキストしか表示できておらず、作詞・作曲・編曲やキャラ/CV 付き歌唱者の情報は出ていなかった。

修正（楽曲詳細ページと同じ仕組みを共有）：

- `EpisodeGenerator` に `SongCreditsRepository` / `SongRecordingSingersRepository` / `PersonAliasesRepository`（楽曲用、既存とは別インスタンス）/ `CharacterAliasesRepository` を追加。
- `BuildThemeRowsAsync` 内で `song_credits`（作詞・作曲・編曲）と `song_recording_singers`（歌唱者）を読み、各楽曲行ごとに以下の HTML を構築：
  - `LyricsHtml` / `CompositionHtml` / `ArrangementHtml`：構造化があれば名義リンク（`/persons/{id}/`、`PrecedingSeparator` で連結）、無ければ `Song.LyricistName` 等のフリーテキストにフォールバック。
  - `LyricsRoleLabelHtml` / `CompositionRoleLabelHtml` / `ArrangementRoleLabelHtml`：`roles` マスタの `name_ja` を `/stats/roles/{rep}/`（`RoleSuccessorResolver` で系譜代表 role_code を解決）にリンク化。マスタ未登録時は素のフォールバック文字列。
  - `VocalistsHtml`：構造化があればキャラ名（`/characters/{id}/`）＋ CV（`/persons/{id}/`）形式、無ければ `SongRecording.SingerName` のフォールバック平文。
- 役職マスタ・名義マスタは全エピソードのループで共有するインスタンス変数キャッシュ（`_themeRolesMap` / `_themePersonAliasMap` / `_themeCharacterAliasMap`）に lazy load 投入し、`GetAllAsync` が 1 度だけ走るように最適化。

テンプレ（`episode-detail.sbn`）の主題歌・挿入歌セクションを**カード型 + メタ行群**にリッチ化：
...
CSS（`.theme-songs-list .ts-card` 一式）：

- 各カードは淡いグレー背景 + 左にプリキュアピンクの 3px 縦帯で識別性を高める。
- 区分バッジ（`.ts-kind`）はプリキュアピンクの濃色背景 + 白文字の pill 形。OP/ED/挿入歌 が一目で識別できる。
- メタ行（`.ts-meta-row`）は flex 横並びで、ラベル列を 3.6em 幅で右寄せ固定し、複数行間で「：」位置を揃える。
- 「歌」行（`.ts-meta-vocalists`）は他のメタ行よりひと回り強調（フォントサイズ 1.0em、value 部 font-weight: 500）。

#### エピソード詳細：オープニング／エンディング クレジット見出しに青ドットが重なるバグの修正

サイトのグローバル `h3::before` は「アクセントブルーの 6px 円マーカー」を `position: absolute; left: 0;` で出している。これは `h3` の `padding-left: 12px` と組み合わせて見出しテキストの左に小さなドットを置く設計だが、`.credit-section h3.credit-kind-heading` は `padding: 0 0 4px 0;`（左 padding を 0 に上書き）にしているため、ドットが**見出しテキストの先頭文字に重なって表示**されていた。

修正（`site.css`）：`.credit-section h3.credit-kind-heading::before { display: none; }` を追加し、credit-kind-heading にはドットを出さない方針に統一。

#### 影響範囲

- DB スキーマ：変更なし。
- C# コード：`EpisodeGenerator.cs`（DTO 拡張＋ヘルパ追加）、`Rendering/CreditTreeRenderer.cs`（sibling 消費スキャナ、cooperation リンク化、character リンク化）、`Rendering/LookupCache.cs`（`LookupCharacterIdFromAliasAsync` 追加）。
- 既存ページの見た目：エピソード詳細の主題歌セクションと、クレジット階層の表示が改善。他のページは変化なし。

---

### v1.3.0 公開直前のデザイン整理 第 N+1 弾 — PC でサイトヘッダを sticky 化、左サイドセクションナビの「[年4桁] [件数バッジ] ○ ラベル」レイアウト化

#### PC でサイトヘッダを sticky 化（モバイルは原状維持）

`.site-header` をデスクトップ幅で `position: sticky; top: 0; z-index: 50;` 固定にし、検索ボックス・グローバルナビ・サイトタイトルへ常時アクセスできるようにした。背景は `rgba(255,255,255,0.96)` + `backdrop-filter: saturate(140%) blur(4px)` で透明感を出し、長尺ページでも圧迫感を抑える。

モバイル幅（≤768px）では従来通り `position: static` のままに上書き（メディアクエリで戻す）。代わりに固定常駐するハンバーガーボタンがグローバルナビ＋セクションナビミラー＋検索ジャンプへの導線になる。

v1.3.0 続編 第 3 弾でいったん巻き戻したヘッダ sticky 化を、左サイドセクションナビとの両立形で復活させた構成。

#### 左サイドセクションナビの新レイアウト「[年4桁] [件数バッジ] ○ ラベル」

シリーズごとに区切ったセクション群（`episodes-index` / `songs-index` / `products-index` のシリーズ別タブ等）では、左サイドナビのアイテムを「`[年4桁] [件数バッジ] ○ ラベル`」の 4 列構造で出す。

- **年4桁**：シリーズ開始年（西暦）。等幅数字（`var(--font-num)`）で 4 桁を視覚的に揃え、`tabular-nums` でカーニング除去。
- **件数バッジ**：シリーズ内のエピソード数・録音件数・商品件数など、h2 の「（N 件）」相当を小さい pill 形バッジで表示。通常時は淡ピンク背景（`rgba(233, 30, 99, 0.12)`）＋ダーク文字（`#831843`）、現在地アイテムでは濃ピンク背景＋白文字に切り替わる。
- **○**：従来通りの円アイコン（現在地塗りつぶし＋光彩、通過済み中抜きピンク枠、未到達薄色枠）。縦線（プログレスバー）の貫通位置は年・バッジ列の幅ぶん右にシフト。
- **ラベル**：シリーズタイトルのみ。長いタイトルは末尾省略（`text-overflow: ellipsis`）し、`<a title="フルラベル">` でホバー時にツールチップが出る。

**ガタつき防止のための列予約**：ナビ内に 1 個でも `data-section-nav-year` を持つアイテムがあれば、ナビ全体に `.has-year` クラスが付き、**全アイテム**で年列の幅（`2.6em`）が確保される。値が無いアイテムは `.is-empty` で `visibility: hidden` にしてレイアウトには影響させる。`.has-count` についても同様の仕組み。これにより、年あり/なしや件数あり/なしが混在するページでも ○ の縦軸が左右にずれない。

縦線（`.page-section-nav-list::before`）の `left` 値は `.has-year` / `.has-count` の有無で `calc()` 切替：
- 既定：`9px`（dot 中心）
- has-year：`calc(2.6em + 6px + 9px)`
- has-count：`calc(1.6em + 6px + 9px)`
- 両方：`calc(2.6em + 6px + 1.6em + 6px + 9px)`

ナビ全体の幅は 200px → 220px に拡張（年4桁＋バッジ＋ラベルが綺麗に収まる幅）。`left` 位置も中心引きを `-220` → `-240` に更新して本文 container との重なりを回避。

ラベル領域は旧 2 行クランプ（`-webkit-line-clamp: 2`）から 1 行 ellipsis に変更。年・バッジを横並びにしたぶん、ラベル側は 1 行で詰めて視覚的にすっきりさせる。

**JS 側の変更**（`section-nav.js`）：

- `collectSections()` の戻り値を `{ items, hasAnyYear, hasAnyCount }` に変更し、ナビ全体で year/count が 1 個でも登場するかを呼び出し側に伝える。
- `buildTimelineItem()` で `<a>` 内に `<span class="page-section-nav-year">` と `<span class="page-section-nav-count">` を必ず出力（値が空なら `.is-empty`）、要素順は `[年] [バッジ] ○ [ラベル]`。
- `<a>` に `title` 属性でラベルのフル文字列を保持（ellipsis 発動時のツールチップ用）。
- `rebuild()` で `navHost.classList.toggle('has-year' / 'has-count', ...)` を適用、CSS 側の列予約と縦線位置切替に連動。

**テンプレ側の変更**：シリーズセクションを持つテンプレートに `data-section-nav-year` / `data-section-nav-count` 属性を追加：

| テンプレート | year | count |
|---|---|---|
| `episodes-index.sbn` | `sec.SeriesStartYearLabel` | `array.size sec.Episodes` |
| `songs-index.sbn` | `s.SeriesStartYearLabel` | `array.size s.Recordings` |
| `products-index.sbn`（シリーズ別タブ） | `s.SeriesStartYearLabel` | `array.size s.Members` |
| `products-index.sbn`（ジャンル別タブ） | — | `array.size s.Members` |
| `characters-index.sbn` | — | `array.size s.Members` |

`data-section-nav-label` はシリーズタイトル単体（年・件数を含まない短いラベル）を渡す。

#### 影響範囲

- DB スキーマ：変更なし。
- C# コード：変更なし（DTO 既存資産で完結、Generator 変更不要）。
- 既存ページの見た目：左サイドナビが少し幅広くなり、シリーズセクションのアイテムが「年4桁＋件数バッジ＋ラベル」のリッチ表示に。本文 container には変更なし。
- 既存のアンカー遷移：`scroll-margin-top` は据え置き（`80px` / モバイル `60px`）。ヘッダ sticky 化により実際に必要な余白は変わらない。

---

### v1.3.0 公開直前のデザイン整理 第 N 弾 — 楽曲詳細ページの構造化クレジット表示、収録商品ソート不具合修正、主題歌使用の範囲集約、劇伴一覧での「（未収録）」明示

#### 楽曲詳細ページ：収録商品ソート不具合の修正

`SongsGenerator.cs` の `RecordingTrackRow.ProductReleaseDate` は表示用に日本語フォーマット済み文字列（「2004年2月18日」など）を保持しているが、これを `StringComparer.Ordinal` で並べていたため **`"2004年10月"` が `"2004年2月"` より先に並ぶ**不具合があった（lex 比較で `'1' < '2'`）。

修正：

- `RecordingTrackRow` に `DateTime ProductReleaseDateRaw` フィールドを追加し、`prod.ReleaseDate` 原値を保持。
- ソートキーを `OrderBy(ProductReleaseDateRaw)` → `ThenBy(ProductCatalogNo)` → `ThenBy(DiscNoInSet)` → `ThenBy(TrackNo)` に変更。
- 表示は引き続き `ProductReleaseDate`（日本語フォーマット文字列）を使う。

#### 楽曲詳細ページ：作詞・作曲・編曲を構造化クレジットから出す（フリーテキストはフォールバック）

従来の `SongsGenerator.GenerateDetail` は `Song.LyricistName` / `Song.ComposerName` / `Song.ArrangerName` の **フリーテキスト列のみ** を参照していた。`song_credits` テーブル（v1.2.3 以降の構造化クレジット）が存在するにもかかわらず、SiteBuilder 側で消費されていなかったため、役職名にも名義にもリンクが付かない問題があった。

修正：

- `SongsGenerator` に `SongCreditsRepository` / `RolesRepository` / `PersonAliasesRepository` / `CharacterAliasesRepository` / `SongRecordingSingersRepository` を追加し、`StaffNameLinkResolver` と `RoleSuccessorResolver` をコンストラクタ引数で受け取るように変更。
- `GenerateAsync` の起動時、`song_credits` を曲数分・`song_recording_singers` を録音数分一括ロードして辞書化。
- `BuildCreditRoleHtml` ヘルパで指定役職（LYRICS / COMPOSITION / ARRANGEMENT）の行群を `credit_seq` 順に走査し、`PrecedingSeparator` で連結した名義リンク列を HTML 化（名義は `PersonAlias.GetDisplayName()` + `StaffNameLinkResolver.ResolveAsHtml()` 経由で `/persons/{id}/` リンク化）。
- 構造化行が 1 件も無い役職は、従来通り `Song.LyricistName` 等のフリーテキストを HTML エスケープした平文にフォールバック。
- `BuildRoleLabelLinkHtml` ヘルパで役職名（"作詞" / "作曲" / "編曲"）を `roles.name_ja` から引き、`RoleSuccessorResolver` の系譜代表 role_code をもとに `/stats/roles/{rep}/` へのアンカーに変換。マスタ未登録時は素のフォールバック文字列（リンクなし）。
- `SongView` DTO に `LyricsHtml` / `CompositionHtml` / `ArrangementHtml` と `LyricsRoleLabelHtml` / `CompositionRoleLabelHtml` / `ArrangementRoleLabelHtml` を新設。
- テンプレ `songs-detail.sbn` の基本情報テーブルで、`Song.LyricistName` 直接出力を `Song.LyricsHtml`（HTML 安全性は Generator 側で担保済みなので raw 出力）に置換、th 側も `Song.LyricsRoleLabelHtml` で出すように変更。空判定はテンプレ側 `if Song.LyricsHtml != ""` で行ごと出し分け。

#### 楽曲詳細ページ：「出自シリーズ」を「出典」に表記変更

基本情報テーブルの該当 `<th>` ラベルを「出自シリーズ」→「出典」に変更。語感が短くなり、シリーズ以外（コラボ作品など）から出典されたケースにも自然に対応する。

#### 楽曲詳細ページ：録音バージョン見出しの再構成、「歌：」を主役級表示に

旧仕様は録音バージョンの h2 を「録音バージョン：{SingerName}」固定文字列にし、`VariantLabel`（「フルサイズ」「TVサイズ」「インスト」等）は `<p class="muted">バリアント: ...</p>` の薄い注記として付けていた。録音バージョンの主要な識別子は実は VariantLabel であり、シンガー名は録音の **内容**（ボーカル）であるという観点から、見出し構成を刷新。

新仕様：

- 録音セクションの h2 を **`{VariantLabel}` そのもの** にする（「フルサイズ」など）。
- h2 の下に **`<p class="song-recording-vocalists"><span class="song-recording-vocalists-label">歌：</span>{VocalistsHtml}</p>`** を主役級の見出し感ある段落として置く。
  - フォントサイズ 1.08em、`font-weight: 500`、本文色（`color: var(--fg)`）。
  - 「歌：」プレフィックスだけは少し小さめ（0.92em、muted 色）で人物・キャラリンクが引き立つようにする。
- VariantLabel が空のケース（トップ録音以外で値が抜けているデータ不備）は h2 行ごとスキップする。「歌：」以降は通常通り出す。
- `RecordingView` DTO に `VocalistsHtml` フィールドを追加。Generator 側で `song_recording_singers`（VOCALS 役）から HTML を構築する。
  - PERSON 名義は `/persons/{id}/` リンク化（`StaffNameLinkResolver` 経由、複数 person 共有 alias なら添字付き）。
  - CHARACTER_WITH_CV は「{キャラ}(CV:{CV})」形式で、キャラは `/characters/{id}/`、CV は `/persons/{id}/`。
  - スラッシュ並列（`slash_*_alias_id`）は「{主名義}/{相方名義}」形式で連結。
  - `AffiliationText`（"with ヤング・フレッシュ" 等）は末尾に半角スペース＋平文で添える。
  - `PrecedingSeparator` で各 seq 行を連結。
- 構造化行が無ければ `SongRecording.SingerName` のフリーテキストにフォールバック。

CSS：`.song-recording-vocalists` / `.song-recording-vocalists-label` を新設。リンク色は `inherit`（本文色）でホバー時のみピンクに変える控えめな装飾。

#### 楽曲詳細ページ：主題歌使用エピソードを範囲集約 + 区分名展開

旧仕様は `episode_theme_songs` から拾った行を **1 話 1 行** で表形式に並べていたため、たとえば『ふたりはプリキュア』のオープニングなら 49 行の同じ内容が縦に積み重なっていた。また区分が `OP` / `ED` / `INSERT` のコードそのままで、ユーザビリティが低かった。

修正：

- `BuildThemeUsageRows` ヘルパで `(SeriesId, ThemeKind, IsBroadcastOnly, UsageActuality)` 単位に集約。
- `CompressEpisodeNumbers` ヘルパで連続話番号を範囲圧縮：
  - `[1, 2, 3, ..., 49]` → 「第1〜49話」
  - `[1, 5]` → 「第1, 5話」
  - `[1, 2, 3, 5, 6, 7]` → 「第1〜3, 5〜7話」
- 区分ラベルを展開：`OP` → 「オープニング主題歌」、`ED` → 「エンディング主題歌」、`INSERT` → 「挿入歌」。
- 注記（`IsBroadcastOnly` → 「（本放送のみ）」、`CreditedNotBroadcast` → 「（実際には不使用）」）は引き続き末尾に追記。
- `RecordingThemeRow` DTO を「1 話 1 行」（旧フィールド: `SeriesEpNo` / `EpisodeTitle` / `EpisodeUrl`）から「集約行」（新フィールド: `EpisodeRangeLabel` / `SortStartEpNo`）に再設計。
- テンプレも従来の表（thead/tbody）から **縦リスト型（ul.recording-theme-usages）** に変更。1 件の `<li>` 内に「{シリーズリンク} {エピソード範囲（等幅数字）} {区分ラベル（ピンクバッジ）}」を flex で横並び。集約後は行数が少なく（典型的に 1〜数件）、表のオーバーヘッドが情報量と見合わなくなったため。

CSS：`.recording-theme-usages` / `.recording-theme-usages-series` / `.recording-theme-usages-range` / `.recording-theme-usages-kind` を新設。

#### 劇伴一覧（`/bgms/{series}/`）：未収録 cue にも「（未収録）」のサブ行を出す

`/bgms/{series}/` の cue 表は v1.3.0 公開直前のデザイン整理 第 4 弾で「メイン行（M番号/メニュー/作曲/編曲/尺/使用回数）+ サブ行（収録盤情報を colspan）」の 2 行ペア構造になっているが、サブ行は `Recordings` が 0 件のときに完全省略されていた。商品が紐付かない cue は「データ漏れ」とユーザーに誤解されやすいため、未収録であることを明示する表示に変更した。

修正：

- `bgms-detail.sbn`：サブ行を常に出力するように `{{~ if (array.size c.Recordings) > 0 ~}}` のガードを撤去。0 件のときは `<span class="bgm-cue-unrecorded-label">（未収録）</span>` を出す。
- `<tr>` のクラスに `bgm-cue-sub-unrecorded` を 0 件時に付与し、CSS で背景をより薄く（`rgba(0,0,0,0.012)`）・左ボーダーアクセントを グレー（`#e5e7eb`）に切り替えて「ここは情報が無い」ニュアンスを視覚的に表現。
- 「（未収録）」ラベル自体は `color: #9ca3af; font-style: italic; font-size: 0.88em` の控えめ表示。

---

### v1.3.0 続編（セクションナビ導入）— 長尺ページに左サイド縦タイムライン型ナビ追加、`/episodes/` の `<details>` 折り畳みを廃止、エピソード詳細の一部 h2 をコンパクト化、モバイル時のハンバーガーメニュー導入

#### 背景・問題意識

サイトの一部ページが縦に長く伸びるようになってきたため、ページ内のセクション間を素早く渡り歩ける常駐ナビが必要になった。具体的には：

- エピソード詳細（`/series/{slug}/{n}/`）：基本情報・スタッフ・主題歌・使用音声・クレジット・サブタイトル分析・フォーマット・パート尺統計・備考・外部サイトと 10 セクションが縦に並び、最下部のクレジット階層に至るまでにかなりスクロールが必要。
- シリーズ詳細（`/series/{slug}/`）：基本情報から関連作品・メインスタッフ・プリキュア一覧・エピソード一覧・劇伴・外部サイトまで 9 セクション。エピソード一覧（最長 50 話）でセクション間が遠くなる。
- エピソード一覧（`/episodes/`）：全 TV シリーズの全エピソードを 1 ページで列挙する構造で、エピソード総数が 1000 を超える運用想定のため、シリーズ別 `<details>` で折り畳んで描画コストとスクロール量を抑制していた。ただし、目的のシリーズまで畳まれた summary 行を縦に何回もスクロールするのも煩雑なので、折り畳まなくて済む別解が望まれていた。
- 人物・企業・キャラ・プリキュアの各詳細：基本情報＋名義履歴＋クレジット履歴（さらに企業ならメンバー履歴、プリキュアなら家族関係＋声の出演履歴 等）と中身が増えてくる。

これらに共通する課題を、**左サイドに常駐する縦タイムライン型ナビ**で一括解決する。

#### 設計の試行錯誤（v1.3.0 続編 第 1〜3 弾）

本機能は第 1 弾で「ページ上部の横スクロールチップストリップ」として導入したが、以下の問題が発覚した：

- シリーズ系のチップラベルが「ふたりはプリキュア (2004) 2004年2月1日 〜 2005年1月30日 全 49 話」のように長く、1 チップが画面幅を独占。
- 第 2 弾で略称（`series.title_short`）を `data-section-nav-label` 属性経由でチップに渡す例外措置を入れたが、それでも横方向の情報密度が低く、サイトヘッダと合わせた 2 段スティッキー構成が縦領域を圧迫した。

第 3 弾で抜本的に**左サイド配置の縦タイムライン型**に刷新。横スクロールチップの試みは破棄し、それに伴い：
- シリーズ系の略称（`SeriesTitleShort` / `ShortLabel`）も廃止（DTO ごと巻き戻し）
- サイトヘッダのスティッキー化も巻き戻し（縦領域確保のため非固定の原状に）
- セクションナビ自体は完全に画面の左外に常駐し、本文の縦領域には一切影響しない

#### セクション内ナビの仕組み（`section-nav.js` + `_layout.sbn` プレースホルダ + `site.css`）

実装は **JavaScript による DOM 自動走査＋クライアント側構築** 方式。各テンプレートは、ナビに載せたい `<section>` に `id` 属性を振るだけで対応する。

具体的な動作：

- レンダリングされた HTML には `_layout.sbn` が出力した `<nav id="pageSectionNav" class="page-section-nav" hidden>` プレースホルダだけが残る（中身は空）。
- 新設の `section-nav.js`（`/assets/section-nav.js`、defer 読み込み）が DOM 構築後に走り、`<main>` 配下の `section[id]` 要素を順序通りに収集して、タイムライン型のアイテム列を組み立ててプレースホルダに流し込む。
- 生成構造：`nav > div.page-section-nav-inner > (.page-section-nav-heading「このページ」 + ol.page-section-nav-list > li.page-section-nav-item > a > span.dot + span.label)`
- アイテムのラベルは、`data-section-nav-label` 属性 → 直下の `<h2>` テキスト → `id` 文字列 の優先順位で解決。複数行 `<h2>`（録音バージョン見出しなど）は連続空白を 1 個に潰してから採用する。実運用では `data-section-nav-label` を使う場面は無く、`<h2>` テキストがそのまま採用される。
- セクションが 1 個以下のページではナビ自体が `hidden` のまま残る（描画されない）。
- `display:none` で隠されている `<section>`（ホームの「今日の記念日」が空のとき等）は走査対象から自動的に除外。
- **タイムラインの見た目**：各アイテムは「○ 円アイコン + 縦線 + ラベル」の 3 要素縦並び。
  - 現在地アイテム：プリキュアピンクで塗りつぶしの円 + 淡い光彩（`box-shadow` で 2 段重ね）、`transform: scale(1.15)` で少し大きく、ラベルもピンク + bold。
  - 通過済みアイテム（current より上）：中抜き円（白塗り＋ピンク枠）、ラベルはダーク色。
  - 未到達アイテム（current より下）：薄色円（白塗り＋グレー枠）、ラベルはミュート色。
  - 縦線：`::before` で 1 本貫通させ、`linear-gradient` の停止位置を CSS 変数 `--progress`（0〜100%）で制御。スクロール量に応じて JS が `requestAnimationFrame` 経由で `--progress` を更新し、ピンクのプログレスバーが上から下へ伸びていく。
  - ホバー時：アイテムが `translateX(3px)` で右にぴょこっと飛び出す + 淡ピンク背景。
- 現在地ハイライトは `IntersectionObserver` ベースのスクロールスパイ。視界に入っているセクションのうちドキュメント順で最上位のものを current 状態にする。`rootMargin` は viewport 幅で動的調整（デスクトップ −80px、モバイル −30px）。
- アンカークリック時のジャンプは通常の `#fragment` リンク。`section[id]` 側に `scroll-margin-top: 80px`（スマホ幅では 60px）を当てて、サイトヘッダの高さぶんジャンプ位置を上に逃がす。`html { scroll-behavior: smooth }`（`prefers-reduced-motion: no-preference` のときだけ有効）でスムーススクロール。
- 既存のタブ UI（`/products/` の `.tab-panel.is-active`）と共存。アクティブなタブパネルがあればそれを走査スコープに使い、タブ切替時には `MutationObserver` がパネルの `class` / `hidden` 属性変化を検知してナビを丸ごと再構築する。

CSS（`.page-section-nav` 一式）は `site.css` 末尾近くで定義：

- 配置は `position: fixed`、`top: 88px`、`left: max(16px, calc((100vw - 960px) / 2 - 220px))`、`width: 200px`、`max-height: calc(100vh - 120px)`、`overflow-y: auto`、`z-index: 40`。本文 container（最大 960px、中央寄せ）の左外側にぴったり収まるよう、画面幅から動的に算出する。
- レスポンシブ：
  - `≥1180px`：完全表示（`width: 200px`）
  - `960〜1179px`：左端寄せ + 少し圧縮（`left: 12px; width: 180px;` + ラベルフォント縮小）
  - `≤959px`：完全非表示（後述のモバイルハンバーガー内ミラーで対応）

#### モバイル（≤768px）向け：ハンバーガーメニュー + セクションナビミラー

モバイル幅では左サイドナビが描画されないため、グローバルナビとセクションナビへのアクセスを **画面右上に固定常駐するハンバーガーボタン**経由で提供する。

- ハンバーガーボタン（`.mobile-nav-toggle`）：44×44px のタップ領域、`position: fixed; top: 12px; right: 12px; z-index: 70;`、3 本線アイコンは pure CSS で構成（画像アセット不要）、開状態では X 形に CSS トランジションで変形。
- オーバーレイ（`.mobile-nav-overlay`、`z-index: 65;`）：右側からスライドインするパネル（`width: min(80vw, 320px)`）。内訳：
  1. 上部：✕ ボタン（パネル左上、`.mobile-nav-overlay-close`）
  2. グローバルナビ 9 項目（ホーム／シリーズ／プリキュア／キャラクター／音楽／人物／企業・団体／統計／このサイトについて）
  3. **このページのセクション一覧**（`#mobileSectionNav`、`section-nav.js` が DOM 走査結果をここにもミラーで流し込む。該当セクションが 1 個以下のときは見出しごと `hidden`）
  4. 「🔍 検索する」ボタン（`#mobileNavSearchJump`、パネル最下部）
- 開閉ロジックは `wwwroot/assets/mobile-nav.js`（defer 読み込み）：
  - 開く：ハンバーガーボタンクリックで `hidden` 属性を外し、`aria-expanded="true"` に切替、`body.mobile-nav-open` クラスで背面スクロール抑止（CSS で `overflow: hidden`）。
  - 閉じる：✕ ボタン、背景バックドロップクリック、Escape キー、ブレークポイント遷移（`matchMedia('(min-width: 769px)').change`）で閉じる。閉じるとフォーカスをハンバーガーボタンに戻す。
  - 検索ジャンプ：「🔍 検索する」ボタンはパネルを閉じてから `window.scrollTo({ top: 0, behavior: 'smooth' })` でページ最上部までスムーススクロールし、400ms 後に `#site-search-input` にフォーカス。

サイトヘッダは v1.3.0 続編 第 3 弾でスティッキー化を取りやめ、従来通りの **非固定（スクロールで消える）** 挙動に戻した。検索ボックスはサイトヘッダ内のため、モバイルではスクロール後はハンバーガーオーバーレイの「🔍 検索する」ボタンが代替アクセス経路になる。

#### 各テンプレートへの `id` 付与（v1.3.0 続編 第 1 弾から継続）

セクション内ナビを有効化するため、サイト内の以下テンプレートで `<section>` に安定 `id` を振った。`id` の命名は「英数小文字＋ハイフン」で統一し、ページ内で意味のある短い名前を選んだ。

- `episode-detail.sbn`：`basic-info` / `staff` / `theme-songs` / `episode-uses` / `credits` / `subtitle-char-info` / `format-table` / `partlength-stats` / `notes` / `external-links`
- `series-detail.sbn`：`basic-info` / `parent-series` / `child-works` / `related-siblings` / `key-staff` / `precures` / `episode-list` / `bgms` / `external-links`
- `persons-detail.sbn`：`basic-info` / `aliases` / `credit-history`
- `companies-detail.sbn`：`basic-info` / `brands-logos` / `credit-history` / `member-history`
- `precures-detail.sbn`：`aliases` / `basic-info` / `family-relations` / `voice-cast`
- `characters-detail.sbn`：`basic-info` / `aliases` / `family-relations` / `voice-cast`
- `songs-detail.sbn`：`basic-info` / `recordings`（録音 0 件のとき）／ 録音バージョンごとに `recording-{n}`（`n` は `for.index + 1`）
- `bgms-detail.sbn`：セッションごとに `session-{n}`
- `products-detail.sbn`：`basic-info` / ディスクごとに `disc-{n}`
- `home.sbn`：`db-stats`（`data-section-nav-label="データベース統計"`、`<h2>` を持たないため明示指定）／既存 `home-anniversary-today`（h2「今日の記念日」）／`upcoming-episodes` / `latest-episodes` / `upcoming-products` / `latest-products`
- `products-index.sbn`：タブ間衝突を避けるため、シリーズ別タブは `prod-series-{n}`、ジャンル別タブは `prod-kind-{n}` のプレフィックス
- `stats-landing.sbn`：`credit-stats` / `subtitle-stats` / `episode-stats`
- `stats-roles-index.sbn`：`overall-ranking` / `by-role`
- `stats-role-detail.sbn`：`person-ranking` / `company-ranking`
- `stats-episodes-index.sbn`：`part-length` / `midcm-timing` / `series-parts`
- `stats-subtitles-index.sbn`：`chars` / `length` / `kanji-rate` / `symbol-rate` / `by-series`
- `about.sbn`：`overview` / `design-policy` / `rights-notice`
- `characters-index.sbn`：種別セクションに `kind-{n}`
- `songs-index.sbn`：シリーズセクションに `songs-section-{n}`
- `series-index.sbn`：既存の `tv-series` / `movies` / `otona` / `short-anime` / `event` / `spinoff` をそのまま流用（変更なし）

セクションが単一または無いページ（`persons-index.sbn` / `companies-index.sbn` / `precures-index.sbn` / `bgms-index.sbn` / `music-landing.sbn` 等）はテンプレ変更なし。`section-nav.js` 側で「セクションが 1 個以下なら描画しない」判定が効くため、ナビは自動的に非表示となる。

#### エピソード詳細の一部 h2 をコンパクト化（v1.3.0 続編 第 3 弾）

セクション内ナビのアイテムラベルが本文 h2 をそのまま流用する設計に戻ったため、エピソード詳細の以下 3 セクションは**本文見出しごと**短い表現に変更した。本文の見出しとしても新しい方が締まる&わかりやすいので、見た目と意味の両面でブラッシュアップ。

- 「サブタイトル文字情報」→ **サブタイトル分析**
- 「フォーマット表」→ **フォーマット**
- 「パート尺統計情報」→ **パート尺統計**

これらは `episode-detail.sbn` の `<h2>` テキストを直接書き換えた。`<section id>` は据え置き。

#### `/episodes/` の `<details>` 折り畳み廃止（`episodes-index.sbn` + `site.css`）

セクション内ナビが導入されたことで、各シリーズへ即座にジャンプする経路が確保された。これにより、エピソード一覧の `<details>` / `<summary>` 折り畳み構造は不要になったため廃止し、各シリーズを通常の `<section id="series-{slug}">` として初期から全展開で描画する構造に変更した。

具体的な変更：

- 旧 `<details class="episodes-index-section">` `<summary class="episodes-index-summary">` の対を、`<section id="series-{slug}" class="episodes-index-section">` `<h2 class="episodes-index-heading">` に置換。
- セクションナビのアイテムラベルは `<h2>` テキスト（シリーズタイトル）が自動採用される。
- `site.css` のエピソード一覧用ブロックを書き換え：`.episodes-index-section[open]` / `.episodes-index-summary` / `::-webkit-details-marker` 等の `<details>` 専用セレクタを撤去し、新しい `.episodes-index-heading` セレクタに置換。視覚的なシリーズ見出し背景（淡いピンク）と境界線（下ボーダー）は維持。

これにより、ユーザーはサイドのセクションナビで目的のシリーズを 1 クリックで選択でき、`<details>` を 1 つずつ開く操作は不要になった。

#### 各種スクリプトの読み込み順

`_layout.sbn` の `</body>` 直前で、defer 付きで 3 本の JS を順に読み込む：
...
ホームの `anniversaries.js`（`home.sbn` 内に置かれている）は `<main>` 内のため、`<body>` 終端の 3 スクリプトより先に解析され、defer 規約により先に実行される。これにより、`anniversaries.js` が「今日の記念日」セクションに `display:none` を当てた後で `section-nav.js` が走査するため、空セクションが誤ってナビに乗らない。

#### 影響範囲

- DB スキーマ：変更なし。
- C# コード：変更なし（v1.3.0 続編 第 2 弾で `EpisodesIndexSection.SeriesTitleShort` / `SongSeriesSection.SeriesTitleShort` / `ProductIndexSection.ShortLabel` を追加していたが、第 3 弾で全て巻き戻し）。
- 既存ページの見た目：左サイドナビが画面の左外に常駐するが、本文 container は中央寄せのままなので本文レイアウトに変更はない（≤959px ではナビ自体が消える）。
- 既存のアンカー遷移：`section[id] { scroll-margin-top: 80px }`（スマホ 60px）でジャンプ先見出しが上に隠れない。
- `/products/` のタブ UI：そのまま動作。タブ切替時に `section-nav.js` がナビを再構築する。

---


### v1.3.0 ブラッシュアップ stage 22 — 商品索引タブ化、商品詳細の発売元・販売元表示整理、SiteBuilder 全体でシリーズ略称の使用を廃止＆シリーズ名表示に開始年を併記

#### 背景・問題意識

商品一覧ページ（`/products/`）はこれまでジャンル別（`product_kinds.display_order` 順）の単一ビューだったが、プリキュアシリーズの音楽商品はシリーズ単位（『ふたりはプリキュア』〜『キミとアイドルプリキュア♪』など）でまとめて閲覧したい場面が多い。一方で「同じシリーズの中でも CD と Blu-ray を一気通貫で見たい／音楽商品だけまとめたい」というジャンル別ニーズも消えないため、両系統をタブで切替できる構造にした。

あわせて商品詳細ページ（`/products/{catalog_no}/`）の基本情報テーブルに、v1.3.0 stage20 で旧フリーテキスト列を撤去した後の更新漏れとして「発売元（空欄）」「販売元（空欄）」「レーベル（空欄）」の 3 行が空で表示される不具合が残存していた。表示用語も「日本の音楽商品流通では発売元 ≒ レーベル」という実務慣習に合わせて 2 行に集約整理する。

さらに本工程の後段で、SiteBuilder の生成出力と UI 表現の運用方針として **シリーズ略称（`series.title_short`）を一切使わない** ことを決定し、サイト全体の該当箇所を一括で正式タイトル一本に改めた。代わりにシリーズ名が並列して並ぶ場面（セクション見出し・テーブル列・リスト項目）では **開始年（西暦 4 桁）を併記** することで、年代の見当を瞬時に把握できる視覚補助を加える。

#### 商品索引のタブ UI（`ProductsGenerator` + `products-index.sbn` + `site.css`）

商品索引のテンプレ用モデル `ProductsIndexModel` を **2 系統のセクションリスト併存型** に再設計：

- `KindSections`：ジャンル別（従来通り `product_kinds.display_order` 順）。
- `SeriesSections`：シリーズ別（新規）。`Series.StartDate` 昇順でシリーズセクションを並べ、後ろに「複数シリーズ」「その他」の順で続ける。

各セクション内は両系統とも **発売日昇順・代表品番昇順** で統一。

シリーズ別の振り分けロジックは Generator 側の `ClassifyProductIntoSeriesBucket` に集約：

| 商品の全ディスクの `series_id` 集合 | バケット |
|---|---|
| 同一の非 NULL 値が 1 種類のみ（NULL 混在なし） | そのシリーズ |
| 複数の異なるシリーズに分かれる、または一部 NULL 混在 | 「複数シリーズ」 |
| 全ディスクが NULL、またはディスクが 0 件 | 「その他」 |

テンプレ側はタブボタン 2 つとパネル 2 つの構造で、ボタンクリックで `is-active` クラスと `aria-selected` / `hidden` 属性をトグルするインラインスクリプトを末尾に同梱。既定タブは「シリーズ別」。JS 無効環境では既定タブパネルだけが見える状態に静的に落ちる（タブボタンによる切替は不可、ただし既定表示は確実に表示される）。

CSS は `.products-tabs` 配下にスコープを閉じた専用ブロックを `site.css` に追記。タブボタンは下境界線で「アクティブ＝白い土台 + 太い前景色」「非アクティブ＝控えめなグレー」の典型的なタブ表現。フォーカスリングは `var(--accent)` のピンクを採用してキーボード操作時の視認性を確保。

#### 商品詳細の発売元・販売元表示（`products-detail.sbn`）

旧テンプレで参照していた `Product.Manufacturer` / `Product.Distributor` / `Product.Label` の 3 つの条件ブロックを撤去。これらは v1.3.0 stage20 確定版で DTO 側（`ProductView`）から既に削除されていたが、テンプレ側の追従が漏れていたため Scriban が null を空文字として扱う仕様により「項目名だけある空欄行」が 3 つ並ぶ状態だった。

代替として既存の `Product.LabelText` / `Product.DistributorText`（どちらも `ProductCompaniesRepository` 経由でマスタ和名を解決済みの文字列）を以下の 2 行で表示：

- **発売元** 行 → `LabelText`（`products.label_product_company_id` の和名）
- **販売元** 行 → `DistributorText`（`products.distributor_product_company_id` の和名）

「レーベル」行は廃止し「発売元」行に統合（用語の整理）。両行とも `!= ""` ガードで未紐付け or 論理削除時は行ごと非表示。

#### シリーズ略称（`series.title_short`）の生成・UI 使用を全面廃止

SiteBuilder の生成出力と UI のすべての箇所で `series.title_short` 参照を撤去し、シリーズ名は常に **正式タイトル（`series.title`）** で表示する方針に統一した。略称は DB マスタには引き続き保持するが、サイトには一切現れない。撤去した参照は SiteBuilder 配下の以下 5 ファイル：

- `Data/SiteDataLoader.cs`：ビルドログの「最新放映済み TV エピソード」表記
- `Generators/CompaniesGenerator.cs`：企業詳細クレジット履歴の各シリーズ表示文字列構築
- `Generators/SeriesGenerator.cs`：シリーズ詳細ページの JSON-LD `alternateName` 出力を撤廃
- `Generators/EpisodeGenerator.cs`：パート尺統計表ヘッダの `seriesTitleShortQuoted` 構築、`SeriesRefView.TitleShort` 詰め
- `Templates/episode-detail.sbn`：本テンプレ自体は `SeriesTitleShortQuoted` プロパティを参照しているが、Generator 側で中身が常に正式タイトルに切り替わったため、表示としては正式タイトルが入る（プロパティ名は後方互換のため温存）

#### シリーズ名表示に開始年を併記

略称を使わない代わりに、SiteBuilder の各画面で「シリーズ名が並列で並ぶ」場面では、シリーズ名の直後に **開始年（西暦 4 桁）** を併記する仕様を導入した。視覚的な分類は以下の 2 流派：

**A群（セクション見出し・リスト項目）**：シリーズ名の隣に薄色括弧で `(2004)` を別 span として添える。

```html
<a href="...">ふたりはプリキュア</a>
<span class="muted series-year">(2004)</span>
```

対応箇所：`products-index.sbn` シリーズ別タブ・`songs-index.sbn` セクション見出し・`stats-episodes-series-summary.sbn` の h2・`episodes-index.sbn` のサマリ・`home.sbn` の最新エピソード/今後の放送予定行・`companies-detail.sbn` クレジット履歴・`persons-detail.sbn` クレジット履歴・`precures-detail.sbn` 声の出演履歴・`characters-detail.sbn` 声の出演履歴。

**B群（テーブル列）**：シリーズ列の直後に **「年度」列**（または記号初出表で「初出年」列）を独立カラムとして追加。セル値は数字のみ（例 `2004`）で `tabular-nums` 等幅、右揃え。

```html
<th class="col-series">シリーズ</th>
<th class="col-year">年度</th>
...
<td class="col-series">ふたりはプリキュア</td>
<td class="col-year">2004</td>
```

対応箇所：`bgms-index.sbn`、`stats-episodes-*.sbn` 9 件（avant longest/shortest/skipped、midcm earliest/latest、part-a/b longest/shortest）、`stats-subtitles-*-by-series.sbn` 6 件（avg-length、kanji-rate、symbol-rate、char-types、symbols、top-chars）、`stats-subtitles-chars-symbols-order.sbn`（「初出年」）、`stats-subtitles-*-episode-most/least.sbn` 6 件（length、kanji-rate、symbol-rate）、`companies-detail.sbn` メンバー履歴テーブル。

**C群（1 ピン留めシリーズ参照）**：その画面で対象となるシリーズが 1 件しか登場しない箇所では年表記は付けない（冗長になるため）。`series-detail.sbn` の h1、`bgms-detail.sbn` の h1、`songs-detail.sbn` / `products-detail.sbn` / 各詳細ページの「出自シリーズ」行など。

#### 実装方針：DTO 拡張ベース

各ジェネレータが構築するテンプレ用 DTO（または匿名型）に `SeriesStartYearLabel`（string、西暦 4 桁 or 空文字）プロパティを追加し、年解決は Generator 側でまとめて行う。テンプレ側は `{{ ... }}` 展開で素直に表示するだけ。

シリーズ slug → 開始年の解決には `BuildContext.SeriesIdBySlug` → `SeriesById` の 2 段引きを共通ヘルパー化（`EpisodePartStatsGenerator.ResolveStartYearLabel` / `SubtitleStatsGenerator.ResolveStartYearLabel`）。マスタ未登録の異常系は空文字を返してテンプレ側で行残し（テーブル空セル）にする。

`InvolvementSeriesRow`（人物・企業のクレジット履歴用、内部共有 DTO）には `SeriesStartYearLabel` プロパティを追加。これに対応して `CompaniesGenerator.BuildCompanyInvolvementGroups` と `PersonsGenerator` の同名メソッドの 2 か所ずつ計 4 か所で `series.StartDate.Year.ToString()` を詰める。

`PrecuresGenerator.VoiceCastRow` と `CharactersGenerator.VoiceCastRow`（こちらは Generator ローカルの private DTO で別物）にも同じく `SeriesStartYearLabel` を追加。

#### CSS

`site.css` に `.series-year`（インライン span 用の薄色括弧、`tabular-nums` 等幅数字）と `.col-year`（テーブル列、右揃え・幅 5em・等幅数字）の 2 クラスを追加。
...
- `PrecureDataStars.SiteBuilder/Data/SiteDataLoader.cs`（TitleShort ログ撤去）
- `PrecureDataStars.SiteBuilder/Generators/ProductsGenerator.cs`（タブ 2 系統対応、シリーズ別バケット振り分け、`SeriesStartYearLabel` 追加）
- `PrecureDataStars.SiteBuilder/Generators/SongsGenerator.cs`（セクション DTO に `SeriesStartYearLabel` 追加）
- `PrecureDataStars.SiteBuilder/Generators/MusicGenerator.cs`（`BgmIndexRow` に `SeriesStartYearLabel` 追加）
- `PrecureDataStars.SiteBuilder/Generators/EpisodesIndexGenerator.cs`（`EpisodesIndexSection` に `SeriesStartYearLabel` 追加）
- `PrecureDataStars.SiteBuilder/Generators/EpisodePartStatsGenerator.cs`（4 メソッド × 匿名型へ `SeriesStartYearLabel` 追加、`ResolveStartYearLabel` ヘルパー新設）
- `PrecureDataStars.SiteBuilder/Generators/SubtitleStatsGenerator.cs`（10 メソッド × 匿名型へ `SeriesStartYearLabel` / `FirstSeriesStartYearLabel` 追加、ヘルパー新設）
- `PrecureDataStars.SiteBuilder/Generators/HomeGenerator.cs`（`EpisodeRow` に `SeriesStartYearLabel` 追加）
- `PrecureDataStars.SiteBuilder/Generators/CompaniesGenerator.cs`（`MemberHistoryRow` に追加、`InvolvementSeriesRow` 詰め 2 か所、TitleShort 1 箇所撤去）
- `PrecureDataStars.SiteBuilder/Generators/PersonsGenerator.cs`（共有 DTO `InvolvementSeriesRow` に `SeriesStartYearLabel` 追加、詰め 2 か所）
- `PrecureDataStars.SiteBuilder/Generators/PrecuresGenerator.cs`（`VoiceCastRow` 追加 + 詰め 2 か所）
- `PrecureDataStars.SiteBuilder/Generators/CharactersGenerator.cs`（`VoiceCastRow` 追加 + 詰め 2 か所）
- `PrecureDataStars.SiteBuilder/Generators/SeriesGenerator.cs`（JSON-LD `alternateName` 撤去）
- `PrecureDataStars.SiteBuilder/Generators/EpisodeGenerator.cs`（`TitleShort` フォールバック撤去 2 か所）

テンプレ 27 ファイル：

- `products-index.sbn` / `products-detail.sbn` / `songs-index.sbn` / `bgms-index.sbn`
- `stats-episodes-series-summary.sbn` / `episodes-index.sbn` / `home.sbn`
- `companies-detail.sbn` / `persons-detail.sbn` / `precures-detail.sbn` / `characters-detail.sbn`
- `stats-episodes-avant-longest/shortest/skipped.sbn`、`stats-episodes-midcm-earliest/latest.sbn`、`stats-episodes-part-a-longest/shortest.sbn`、`stats-episodes-part-b-longest/shortest.sbn`（計 9 件）
- `stats-subtitles-avg-length-by-series.sbn`、`stats-subtitles-char-types-by-series.sbn`、`stats-subtitles-kanji-rate-by-series.sbn`、`stats-subtitles-symbol-rate-by-series.sbn`、`stats-subtitles-symbols-by-series.sbn`、`stats-subtitles-top-chars-by-series.sbn`、`stats-subtitles-chars-symbols-order.sbn`、`stats-subtitles-length-episode-most/least.sbn`、`stats-subtitles-kanji-rate-episode-most/least.sbn`、`stats-subtitles-symbol-rate-episode-most/least.sbn`（計 13 件）

CSS / README：

- `PrecureDataStars.SiteBuilder/wwwroot/assets/site.css`（`.products-tabs` 一式と `.series-year` / `.col-year` の 3 ブロック追加）
- `README.md`（本エントリ追加 + 商品索引仕様 + 商品詳細社名表示仕様の更新）

---

### v1.3.0 ブラッシュアップ stage 21 — テンプレ DSL に役職リンク化プレースホルダ `{ROLE_LINK:code=...}` を追加、連載クレジットの「漫画」ラベルをリンク化

#### 背景・問題意識

v1.3.0 の SiteBuilder では、クレジット内のあらゆる要素（人物名義／企業屋号／ロゴ／役職名カラム）が詳細ページへリンク化されるよう統一されていた。しかし `SERIALIZED_IN`「連載」役職のテンプレ内に、stage 19 で導入された `<strong>漫画</strong>` というプレーンな HTML が直書きされていたため、ここだけがリンク化されていなかった。連載クレジットを見たユーザーが「漫画」役職の集計ページ `/stats/roles/MANGA/` に遷移できず、クレジット内導線の一貫性を損なう状態だった。

v1.3.0 リリース時点で「持ち越し項目」として記録していたものを、v1.3.0 続編 stage 21 で消化する。テンプレ DSL 仕様拡張（新プレースホルダ追加）+ DB マイグレーション 1 本のセットで対応した。

#### テンプレ DSL 構文拡張: `{ROLE_LINK:code=ROLE_CODE}`

役職コードから役職統計ページへのリンク化済み HTML を太字付きで埋め込む新プレースホルダ。既存 DSL の `{NAME:opt=val}` 形式と整合する `code=ROLE_CODE` オプション付きシンタックスを採用したため、`TemplateParser` の AST 構造には変更を加えず、通常の `PlaceholderNode` として処理される。レンダラ側（`RoleTemplateRenderer.ResolvePlaceholderAsync`）に `ROLE_LINK` ケースを追加するだけで実装できる構造で、後方互換性は完全に維持される。

**使用例**:
```
{ROLE_LINK:code=MANGA}
```

**SiteBuilder 側の HTML 出力**:
```html
<strong><a href="/stats/roles/MANGA/">漫画</a></strong>
```

**Catalog 側プレビューの HTML 出力**（リンクなし版）:
```html
<strong>漫画</strong>
```

**重要な設計判断**: `<strong>` ラップはレンダラ側で一律に付与する。テンプレ作者が `<strong>` を書く・書かないで揺れないように、「役職リンクなら必ず太字、違えば太字ではない」という見た目ルールを **DSL レンダラの責務**として保証する設計とした。`code` オプションが空、または未登録の役職コードが指定された場合は何も出力しない（タグ残骸も残さない）。

#### `ILookupCache` インターフェース拡張: `LookupRoleHtmlAsync`

`ILookupCache` に第 4 系統の HTML 解決メソッドとして `LookupRoleHtmlAsync(string roleCode)` を追加した。既存の 3 系統（人物名義・企業屋号・ロゴ）と同じ二面展開：

- SiteBuilder 側 `LookupCache` → `<a href="/stats/roles/{role_code}/">表示名</a>` を返す
- Catalog 側 `LookupCache` → 表示名を HTML エスケープしただけのプレーンテキストを返す（リンクなし）

戻り値は `RoleTemplateRenderer` の `ROLE_LINK` ケースで `<strong>` ラップされて埋め込まれる。未登録時は `null` を返し、レンダラ側で空文字に展開されるので `<strong></strong>` のような空タグも残らない。

#### 各実装側の対応

**SiteBuilder 側 `LookupCache`** (`PrecureDataStars.SiteBuilder/Rendering/LookupCache.cs`):

- フィールドに `RolesRepository _rolesRepo` を新規追加（コンストラクタで受け取る）。`EpisodeGenerator` 側で既に持っている `_rolesRepo` を渡して共有する形に。
- `Dictionary<string, Role?> _roleCache` を新設（StringComparer.Ordinal）。負の結果（未登録の役職コード）もキャッシュし、繰り返し DB に問い合わせない設計。
- `LookupRoleHtmlAsync` 本体は `roles.GetByCodeAsync` → `name_ja` 解決 → HTML エスケープ → `<a href="/stats/roles/{code}/">表示名</a>` 組み立て、の流れ。
- 役職継承（`role_successions`）との連動は当面行わない。テンプレ作者が指定した role_code をそのまま URL に埋める設計とし、廃止された旧 role_code が指定されたら 404 になる前提（テンプレ側修正で対応）。将来連動させたくなれば `RoleSuccessorResolver` を本クラスに注入する拡張は容易。

**Catalog 側 `LookupCache`** (`PrecureDataStars.Catalog/Forms/LookupCache.cs`):

- 既存の `_rolesRepo` フィールドと `_roleCache` 辞書（`LookupRoleNameJaAsync` で共用済み）をそのまま流用する形で実装。コンストラクタや依存関係に変更なし。
- `LookupRoleHtmlAsync` 本体は `_roleCache` チェック → 必要なら `_rolesRepo.GetByCodeAsync` → `name_ja` の HTML エスケープ、の流れ。
- プレビュー画面ではリンクは出さない方針（既存方針と整合）。レンダラ側で `<strong>` ラップが付与されるので、見た目は従来通り「漫画だけ太字」のまま（差分は内部表現としての HTML 構造のみ）。

**`RoleTemplateRenderer`** (`PrecureDataStars.TemplateRendering/RoleTemplateRenderer.cs`):

- `ResolvePlaceholderAsync` の switch 文に `ROLE_LINK` ケースを追加。`code` オプション取得 → `lookup.LookupRoleHtmlAsync` → 結果を `<strong>{inner}</strong>` でラップして返す。
- 空 / null 結果の場合は空文字を返す（タグ残骸を残さない）。
- クラス XML コメントの「サポートするプレースホルダ」一覧に `{ROLE_LINK:code=ROLE_CODE}` を追加。

**`EpisodeGenerator`** (`PrecureDataStars.SiteBuilder/Generators/EpisodeGenerator.cs`):

- `new LookupCache(...)` 構築箇所に `_rolesRepo` 引数を追加するだけの微修正。

#### DB マイグレーション: `v1.3.0_stage21_role_link_placeholder.sql`

`SERIALIZED_IN` テンプレの `format_template` を更新するシンプルな 1 行 UPDATE。
...
変更点は中央 1 行の `<strong>漫画</strong>` → `{ROLE_LINK:code=MANGA}` のみ。`<strong>` ラップが消えているように見えるが、レンダラ側で自動付与されるので最終 HTML には変化なし（むしろリンクが追加されて 1 段リッチになる）。冪等性：`format_template` に既に `{ROLE_LINK:code=MANGA}` が含まれていれば UPDATE 対象から除外する `WHERE NOT LIKE` 条件付き（`_` を `\_` でエスケープして `{ROLE:` パターンに誤マッチしないように）。`START TRANSACTION` / `COMMIT` でトランザクション境界を明示。

#### 副作用 / 影響範囲

- 連載クレジットを含むエピソード（プリキュア各シリーズの原作雑誌クレジット）の HTML 出力で「漫画」役職ラベルが太字リンクに昇格する。
- SiteBuilder ビルドの再実行で全該当ページが更新される。Catalog 側プレビューでは見た目は従来通り（リンクなしの太字テキスト）。
- 既存テンプレで `{ROLE_LINK:...}` を使っていたユーザーテンプレは無いはず（v1.3.0 続編 stage 21 で新設したばかり）なので、互換性の問題はゼロ。
- `ILookupCache` を実装する第三者コードがあれば `LookupRoleHtmlAsync` を実装する必要がある（インターフェースが拡張されたため）。本リポジトリ内では SiteBuilder / Catalog の 2 実装のみで、両方とも本作業で実装済み。

#### バージョン番号

`Directory.Build.props` の `<Version>` / `<AssemblyVersion>` / `<FileVersion>` は **`1.3.0` のまま据え置き**。v1.3.0 公開後にまだ正式リリースを切っていないため、続編 stage 21 として v1.3.0 系列内で継続する位置づけ（v1.3.0 続編・公開直前のデザイン整理シリーズと同じ扱い）。
...
#### クレジット展開時のリンク化対応とコード値→ラベル変換（`ILookupCache` + `RoleTemplateRenderer` + `ThemeSongsHandler` + `LookupCache`）

エピソード詳細のクレジット展開セクション内に出る要素を、それぞれの詳細ページにリンク化できるよう、テンプレ展開エンジンを HTML 出力モードに切り替えた。あわせて主題歌の種類コードがそのまま表示されていた問題も解消した。

- `ILookupCache` インターフェースに `LookupPersonAliasHtmlAsync` / `LookupCompanyAliasHtmlAsync` / `LookupLogoHtmlAsync` の 3 メソッドを抽象として追加。SiteBuilder 側 `LookupCache` ではリンク化済み HTML 断片（`<a href="/persons/{id}/">名義</a>` 等）を、Catalog 側 `LookupCache` では HTML エスケープしただけのプレーンテキストを返す形にして、共通エンジン側からは「リンク版が欲しい場面で同じメソッドを呼ぶだけ」で済むようにした。
- SiteBuilder 側 `LookupCache` には `StaffNameLinkResolver` を後注入する `SetStaffLinkResolver` メソッドを設け、共有名義（1 alias → 複数 person）の添字付き複数リンク化（「名義[1] [2]」形式）まで対応する。注入は `EpisodeGenerator` のコンストラクタで行う。
- `RoleTemplateRenderer` のプレースホルダ解決を全面的に HTML 出力モードに改修：
  - `{PERSONS}` / `{COMPANIES}` / `{LOGOS}` / `{LEADING_COMPANY}` は `LookupPersonAliasHtmlAsync` 系を呼んでリンク化済み HTML を埋め込む
  - `{SONG_TITLE}` は `<a href="/songs/{song_id}/">タイトル</a>` の形にリンク化
  - `{SONG_KIND}` は `OP` → `OP`、`ED` → `ED`、`INSERT` → `挿入歌` の表示ラベルに正規化（旧仕様ではコード値 `INSERT` がそのまま読者に見えていた）
  - `{ROLE_NAME}` / `{LYRICIST}` / `{COMPOSER}` / `{ARRANGER}` / `{SINGER}` / `{VARIANT_LABEL}` / `{TEXTS}` は HTML エスケープを通して安全にプレーンテキスト出力
- `ThemeSongsHandler` の `RenderSingleSongBlock` を HTML 版に書き直し：曲タイトルは `/songs/{song_id}/` への `<a>` ラップ、変種ラベル・作家情報・歌唱者情報は HTML エスケープ済みの形で詰める。`columns>=2` の横並びテーブルセル内も HTML エスケープを通さず素通し（既にエスケープ済みのため）。

Catalog 側プレビューは WebBrowser コントロール（IE 互換モード）に HTML を流し込む構造なので、`<a href>` 付きの HTML 出力が混じってもリンクとして見えるだけで表示崩れは起きない。Catalog 側 `LookupCache` 実装はリンクなしのプレーンテキストを返す挙動を維持しているので、プレビュー画面の見え方は従来通り。

これにより、エピソード詳細のクレジット展開セクション内で：

- クレジットされた人物名 → 人物詳細ページへ
- クレジットされた屋号・ロゴ → 企業詳細ページへ
- クレジット内に出る主題歌・挿入歌のタイトル → 楽曲詳細ページへ
- 主題歌の種類が「OP」「ED」「挿入歌」の自然な日本語表記に
...
旧版で「クレジット中の漫画役職・名義のリンク化」を持ち越し項目として記録していたが、**v1.3.0 続編 stage 21 で解消済み**：テンプレ DSL に新プレースホルダ `{ROLE_LINK:code=ROLE_CODE}` を追加し、`SERIALIZED_IN` テンプレ内の `<strong>漫画</strong>` 部分を `{ROLE_LINK:code=MANGA}` に置換する DB マイグレーション（`db/migrations/v1.3.0_stage21_role_link_placeholder.sql`）を投入した。詳細は v1.3.0 ブラッシュアップ stage 21 のセクションを参照。

現時点で持ち越している大物項目は特になし。新規発生分は本セクションに追記する運用を継続する。
...
| `/persons/` `/persons/{personId}/` | 人物軸。索引は「50音順」と「初クレジット順」の 2 タブ構成（v1.3.0 続編 第 N+3 弾）。50音順タブは `あ・か・さ・た・な・は・ま・や・ら・わ・その他` の最大 11 セクションで全人物を並べる（カナ・濁音・半濁音は清音ひらがなに寄せて先頭文字判定）。初クレジット順タブは当該人物が最初にクレジットされたシリーズごとにセクション分け（シリーズ放送開始日昇順、クレジット 0 件の人物は載せない）。人物詳細は旧姓・別名義・ユニット名義を `person_alias_persons` で合算、声優関与は演じたキャラクター名併記。クレジット履歴は主題歌の作詞/作曲/編曲/歌/コーラスを `song_music_classes.name_ja` から引いた「オープニング主題歌 作曲」「エンディング主題歌 編曲」「挿入歌 作詞」のようなプレフィックス付き役職セクションに分解、劇伴は「劇伴 作曲」「劇伴 編曲」のプレフィックス付き |
| `/companies/` `/companies/{companyId}/` | 企業軸。索引は初クレジット順セクション 1 軸構成（タブ無し、v1.3.0 続編 第 N+3 弾）。当該企業を最初にクレジットしたシリーズごとにセクション分け（シリーズ放送開始日昇順）。**クレジット 0 件の企業は索引から完全除外**（詳細ページは引き続き生成）。詳細は企業ごとの全屋号・配下ロゴを合算した形式 |
...
| `/products/` `/products/{product_catalog_no}/` | 商品（CD / Blu-ray / DVD）。索引はタブ UI で「シリーズ別」（既定）と「ジャンル別」を切替表示する。シリーズ別は商品の全ディスクの `series_id` を集めて単一シリーズ／「複数シリーズ」／「その他」に分類し、シリーズセクションは `series.start_date` 昇順、その後ろに「複数シリーズ」「その他」の順で並べる。ジャンル別は `product_kinds.display_order` 順でセクション分け。両タブとも各セクション内は発売日昇順。詳細ではディスクごとのトラック表 |
...
#### セクション内ナビ（クライアント側 JS、v1.3.0 続編 第 3 弾）

長尺ページ（エピソード詳細・シリーズ詳細・エピソード一覧 等）で、ページ内セクションを「渡り歩く」ための**左サイド縦タイムライン型ナビ**を画面の左外側に常駐させる。`_layout.sbn` が `<nav id="pageSectionNav" class="page-section-nav" hidden>` プレースホルダを出力し、新設の `section-nav.js`（`/assets/section-nav.js`、defer 読み込み）がクライアント側で DOM 走査してタイムラインアイテム列を構築する。

- 走査対象：`<main>` 配下の `section[id]`。`id` を持たない `<section>` や `<div>` は無視する。
- ラベル解決：`data-section-nav-label` 属性 → 直下の `<h2>` テキスト → `id` 文字列の優先順。複数行 `<h2>` は連続空白を 1 個に潰してから採用。実運用では `data-section-nav-label` を使う箇所は無く、`<h2>` テキストがそのまま使われる。
- セクションが 1 個以下のページではナビ自体を `hidden` のままに保つ（描画しない）。`display:none` で隠されている `<section>`（ホームの「今日の記念日」が空のとき等）も走査から自動除外。
- **タイムラインギミック**：各アイテムは「○ 円アイコン + 縦線 + ラベル」の縦並び。現在地は塗りつぶし円 + プリキュアピンクの光彩（`box-shadow` 2 段重ね）+ `scale(1.15)`、通過済みは中抜き円（白塗り＋ピンク枠）、未到達は薄色円（白塗り＋グレー枠）。縦線（`::before`）はグレーベース＋上半分が CSS 変数 `--progress`（JS が `requestAnimationFrame` で更新）の値までピンクで塗られる progress bar として機能。ホバー時はアイテムが `translateX(3px)` で右にぴょこっと飛び出す。
- スクロールスパイ：`IntersectionObserver` で視界内セクションを追跡し、ドキュメント順で最上位のものを `.is-current` にハイライト。それより上のアイテムには `.is-passed`（通過済み中抜き円表示）が付く。`rootMargin` の上端は viewport 幅で動的調整（デスクトップ −80px、モバイル −30px）、ブレークポイント遷移を `matchMedia` で検知して再構築する。
- アンカークリック時のジャンプ：通常の `#fragment` リンク。`section[id]` 側に `scroll-margin-top: 80px`（スマホ幅 60px）を当ててサイトヘッダの高さぶんジャンプ位置を上に逃がす。`html { scroll-behavior: smooth }`（`prefers-reduced-motion: no-preference` のときのみ有効）でスムーススクロール。
- タブ UI（`/products/` の `.tab-panel.is-active`）と共存：アクティブなタブパネルがあればそれを走査スコープに使う。タブ切替時には `MutationObserver` がパネルの `class` / `hidden` 属性変化を検知してナビを丸ごと再構築する。
- 各テンプレ側の対応：ナビに載せたい `<section>` に英数小文字＋ハイフンの安定 `id` を振る。動的に増えるセクション（録音バージョン・ディスク・セッション・タブ内のシリーズ／ジャンル列）は Scriban の `for.index + 1` ベースで一貫した命名（`recording-{n}` / `disc-{n}` / `session-{n}` / `prod-series-{n}` / `prod-kind-{n}` 等）。
- 配置とレスポンシブ：
  - `≥1180px`：本文 container（最大 960px、中央寄せ）の左外側に `position: fixed; left: max(16px, calc((100vw - 960px) / 2 - 220px)); width: 200px;` で常駐
  - `960〜1179px`：左端寄せ + 少し圧縮（`left: 12px; width: 180px;`、ラベルフォント縮小）
  - `≤959px`：完全非表示。代わりにモバイルハンバーガーオーバーレイ内 `#mobileSectionNav` にミラー描画

これにより、エピソード一覧 `/episodes/` は旧 `<details>` 折り畳み構造を廃止し、各シリーズを通常の `<section id="series-{slug}">` として初期から全展開で並べる構造に変更した。ユーザーは左サイドのセクションナビで目的のシリーズへ 1 クリックで到達できる。

#### モバイル時のハンバーガーメニュー（v1.3.0 続編 第 3 弾）

モバイル幅（≤768px）では左サイドナビが描画されないため、グローバルナビとセクションナビへのアクセスを画面右上に固定常駐するハンバーガーボタン経由で提供する。サイトヘッダは原状回帰（非スティッキー、スクロールで消える）。

- ハンバーガーボタン（`.mobile-nav-toggle`）：44×44px のタップ領域、`position: fixed; top: 12px; right: 12px; z-index: 70;`、3 本線アイコンは pure CSS で構成（画像アセット不要）、開状態では X 形にトランジションで変形。
- オーバーレイ（`.mobile-nav-overlay`、`z-index: 65;`）：右側からスライドインするパネル（`width: min(80vw, 320px)`）。グローバルナビ 9 項目（ホーム／シリーズ／プリキュア／キャラクター／音楽／人物／企業・団体／統計／このサイトについて）＋ `#mobileSectionNav`（このページのセクション一覧、`section-nav.js` が DOM 走査結果をミラー流し込み、該当セクションが 1 個以下なら見出しごと `hidden`）＋「🔍 検索する」ボタン。
- 開閉ロジック（`wwwroot/assets/mobile-nav.js`、defer 読み込み）：ハンバーガーボタンの `aria-expanded` を切替、`body.mobile-nav-open` クラスで背面スクロール抑止。閉じる手段は ✕ ボタン・背景バックドロップクリック・Escape キー・ブレークポイント遷移（`matchMedia('(min-width: 769px)').change`）。検索ジャンプボタンはパネルを閉じてから `window.scrollTo({ top: 0, behavior: 'smooth' })` でページ最上部までスムーススクロールし、400ms 後に `#site-search-input` にフォーカス。
...
商品詳細ページ（`/products/{catalog_no}/`）の基本情報テーブルには、商品社名マスタ（`product_companies`）への構造化 ID で解決した社名を以下の 2 行で表示する：

- **発売元** 行：レーベル ID（`products.label_product_company_id`）から引いた和名（`product_companies.name_ja`）を出す。日本の音楽商品流通の実務慣習に合わせ、「レーベル名 = 発売元」として扱う（旧テンプレで別行になっていた「レーベル」行は v1.3.0 stage22 で「発売元」行へ統合し撤去した）。
- **販売元** 行：ディストリビューター ID（`products.distributor_product_company_id`）から引いた和名を出す。

いずれもマスタ未紐付け or 論理削除済みなら空文字となり、その行ごと非表示になる（行残骸は出ない）。フォールバック分岐（旧フリーテキスト列 `manufacturer` / `distributor` / `label` の参照）は v1.3.0 stage20 確定版で撤去済みで、本表示でも一切経由しない。
...
| `v1.3.0_stage21_role_link_placeholder.sql` | `SERIALIZED_IN` 役職テンプレ内の `<strong>漫画</strong>` プレーン HTML を新プレースホルダ `{ROLE_LINK:code=MANGA}` に置換。テンプレ DSL 拡張（`{ROLE_LINK:code=ROLE_CODE}` で役職統計ページへの太字リンク化済み HTML を埋め込む）と組み合わせて、連載クレジット中の「漫画」ラベルから `/stats/roles/MANGA/` に遷移可能にする（v1.3.0 続編 stage 21） |

...
(function () {
  ...
  function rebuild() {
    ...
    // デスクトップでは sticky ヘッダ（2 行構造：上段にタイトル + 検索、下段にナビ＝
    // 実高さ約 95px）+ 余白を見越して 110px。CSS 側の scroll-margin-top と一致させる。
    if (currentObserver) currentObserver.disconnect();

    var isMobile = window.matchMedia('(max-width: 768px)').matches;
    var topOffsetPx = isMobile ? 30 : 110;
  ...
  function installSmoothScrollHandler(host) {
    ...
    host.addEventListener('click', function (ev) {
      ...
      // 上端オフセット（sticky ヘッダ分）は IntersectionObserver と同じ判定基準で決める。
      // デスクトップ：2 行ヘッダ実高さ（約 95px）+ バッファで 110px。CSS の scroll-margin-top と
      // 同じ値を使い、ブラウザ既定のフラグメント着地位置と JS smooth scroll の着地位置を揃える。
      var isMobile = window.matchMedia('(max-width: 768px)').matches;
      var topOffsetPx = isMobile ? 30 : 110;

...
public sealed class Track
{
    ...
    /// <summary>トラック内順序。通常のトラックでは 0（既定値）。 1 トラックに複数の曲が入っている場合のみ 1, 2, 3, ... と振って複数行で表現する。</summary>
    public byte SubOrder { get; set; } = 0;

    // ── 内容種別と多態参照 ──

    /// <summary>トラック内容種別コード（→ track_content_kinds）。 "SONG" / "BGM" / "DRAMA" / "RADIO" / "JINGLE" / "CHAPTER" / "OTHER"。</summary>
    ...
    /// <summary>歌トラック時のサイズ種別コード（→ song_size_variants）。 フルサイズ・TVサイズ・映画サイズ等。SONG 以外では NULL。</summary>
    public string? SongSizeVariantCode { get; set; }

    /// <summary>歌トラック時のパート種別コード（→ song_part_variants）。 通常歌入り・オリジナル・カラオケ・コーラス入り等。SONG 以外では NULL。</summary>
    public string? SongPartVariantCode { get; set; }

    /// <summary>劇伴トラック時に参照する bgm_cues の第 1 列（シリーズ ID）。</summary>
    public int? BgmSeriesId { get; set; }

    /// <summary>劇伴トラック時に参照する <c>bgm_cues</c> の第 2 列（M 番号詳細）。</summary>
    public string? BgmMNoDetail { get; set; }

    /// <summary>ディスク固有のトラックタイトル上書き。 ドラマ／ラジオ等の実体テーブルを持たないトラックのタイトル表示にも利用する。</summary>

...
> **v1.1.2** — ディスク・トラック閲覧フォームの一覧 UI を整理し、作詞／作曲／編曲の表示、劇伴タイトルの M 番号注釈、総尺カラム、枚数表記の統合、余白の改善を行いました。詳細は末尾の [変更履歴](#変更履歴) を参照。
...
`db/schema.sql` によりデータベース `precure_datastars` と全テーブル（エピソード系 6 本 + 音楽・映像カタログ系 14 本）が作成されます。スキーマは v1.1.2 時点の最新状態（`discs.series_id` を持ち、`products.series_id` は無い。`songs` の作詞／作曲列は `lyricist_name` / `composer_name` 等の素の命名）。

 ...
 *     <h2> 内にバッジ等の補助テキストが含まれてしまうセクション（楽曲詳細の録音バージョン
 *     見出しなど）では、テンプレ側で data-section-nav-label を明示渡しして曲名のみを
 *     ラベルに採用させる運用にしている。
...
(function () {
  ...
  //
  // data-section-nav-year（4 桁西暦）と data-section-nav-count（件数）の 2 属性も拾い、
  ...
  function rebuild() {
    ...
    // モバイル（≤768px）では site-header が非固定でスクロール時に画面外へ

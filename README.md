...
- [`db/migrations/`](db/migrations/) … バージョン別の差分 SQL 群。新規構築では不要（`db/schema.sql` が常に最新スキーマ）。
 ...
 最新: `v1.3.5_migration_precure_key_color.sql` … `precures` に `key_color`（バッジ地色 #RRGGBB）と CHECK 制約 `ck_precures_key_color` を追加し、暫定地色（precure_id=1 → `#000000`、2 → `#ffffff`）を未設定行のみ初期投入。

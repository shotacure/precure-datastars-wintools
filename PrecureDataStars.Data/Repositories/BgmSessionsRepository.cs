...
public sealed class BgmSessionsRepository : RepositoryBase
{
    /// <summary><see cref="BgmSessionsRepository"/> の新しいインスタンスを生成する。</summary>
    public BgmSessionsRepository(IConnectionFactory factory) : base(factory) { }
    ...
    public async Task<IReadOnlyList<BgmSession>> GetBySeriesAsync(int seriesId, CancellationToken ct = default)
    {
        ...
        return await QueryListAsync<BgmSession>(sql, new { seriesId }, ct).ConfigureAwait(false);
    }
    ...
    public async Task<IReadOnlyList<BgmSession>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return await QueryListAsync<BgmSession>(sql, ct: ct).ConfigureAwait(false);
    }
    ...
    public async Task DeleteAsync(int seriesId, byte sessionNo, CancellationToken ct = default)
    {
        ...
        await ExecuteAsync(sql, new { seriesId, sessionNo }, ct).ConfigureAwait(false);
    }
}

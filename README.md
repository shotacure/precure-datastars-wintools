...
> **最新 v1.3.8** — 歌の音楽種別 `music_class_code` を `song_recordings` 側に移設し、歌索引 `/songs/` を 2 タブ UI（シリーズ別／ジャンル別）化、サイト各セクションの大ページタイトルを「歴代プリキュア〜」体系に統一、`/music/` ・`/creators/` ・`/stats/` のランディングをカード全体リンク方式に統一。各機能の現状仕様は本文を、全バージョンの変更履歴は [`CHANGELOG.md`](CHANGELOG.md) を参照してください。

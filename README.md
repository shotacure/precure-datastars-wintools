...
3. 「既存ディスクと照合 / 新規登録...」ボタンで `DiscRegistrationService` を通じた優先順（MCN → CDDB-ID → TOC 曖昧）の照合が走り、`DiscMatchDialog` が候補を表示。`DiscMatchDialog` のアクションは v1.1.3 から 3 通りに増えた:
   ...
   - **「既存商品に追加ディスクとして登録」（v1.1.3 新設）**: 既存の複数枚組商品に新しいディスクを追加するケース。商品本体は新規作成せず、`AttachToProductDialog` で対象商品を選び、組内番号 (`disc_no_in_set`) を採番して新ディスクを INSERT する。商品の `disc_count` は所属ディスク数 + 1 に自動更新される
...
2. `DiscMatchDialog` で **「既存商品に追加ディスクとして登録」** ボタンを選択
3. 品番入力プロンプトで新しいディスクの品番を入力
4. **`AttachToProductDialog`** が開き、追加先の既存商品を検索・選択する

**`AttachToProductDialog` の操作**

- **既存商品の検索**: 検索キーワードに品番（完全一致）またはタイトル・略称・英語タイトル（部分一致）を入力 → 検索ボタンか Enter で `ProductsRepository.SearchByTitleAsync` + `GetByCatalogNoAsync` を併用検索（最大 200 件 + 品番完全一致 1 件）
- **商品候補グリッド**: 代表品番 / タイトル / 発売日 / 枚数 (`disc_count`) を表示
- **所属ディスクのプレビュー**: 商品を選択すると、その商品に既に登録されているディスク（`DiscsRepository.GetByProductCatalogNoAsync` で取得）が下段グリッドにプレビュー表示される（組内番号 / 品番 / タイトル / メディア / `series_id`）
...
「選択商品に追加して登録」ボタン押下で `DiscRegistrationService.AttachDiscToExistingProductAsync` が呼ばれ、次の順序で DB 更新:
...
- 編集系コンボはすべて短縮名 (`title_short`) 優先表示の設計に統一されているため、`AttachToProductDialog` のシリーズコンボも同じく `title_short` 優先（無ければ `title`）
...
- `DiscMatchDialog` のアクションを 2 通りから 3 通りに増強。「選択したディスクに反映」「**既存商品に追加ディスクとして登録**」「新規商品＋ディスクとして登録」から選べる。
- `AttachToProductDialog` を新設。商品検索（品番完全一致 + タイトル部分一致）、所属ディスクのプレビュー、シリーズの「(既存ディスクから継承)」既定＋上書きコンボを提供。
...
- `PrecureDataStars.Data/Repositories/ProductsRepository.cs`: `GetAllAsync` の並びを発売日昇順に変更、旧降順を `GetAllDescAsync` として残置
...
- `PrecureDataStars.Catalog.Common/Dialogs/AttachToProductDialog.cs` / `.Designer.cs`: 新規ファイル（既存商品への追加ディスク登録ダイアログ）
- `PrecureDataStars.Catalog.Common/Dialogs/DiscMatchDialog.cs` / `.Designer.cs`: 「既存商品に追加ディスクとして登録」ボタン追加、`WantsAttachToExistingProduct` プロパティと `BtnAttachToProduct_Click` ハンドラを新設
...
- `PrecureDataStars.BDAnalyzer/MainForm.cs`: `WantsAttachToExistingProduct` 分岐を追加（既存商品への追加ディスク登録フロー）
- `PrecureDataStars.CDAnalyzer/MainForm.cs`: 同上

...
- **劇伴索引（/bgms/）をカード型リストに刷新**：従来の独自テーブル（音源数・録音セッション・使用回数の 3 数値列）を撤去し、楽曲索引（`.songs-card-list`）や商品索引（`.products-card-list`）と同系列の意匠でカード 1 枚 = 1 シリーズの縦並びリストに再構成。カード内はヘッダ行（タイトル＝リンク／放送・公開期間／件数メタ）とスタッフ表示（作曲・編曲バッジ + 名前）の 2 ブロック構成。ヘッダ行はデスクトップでは `grid-template-columns: 1fr auto auto` の 3 列横並び、モバイル（≤640px）ではタイトルが全幅、その下に放送期間（左揃え）と件数メタ（右揃え）が並ぶ 2 行 2 列構成に倒れる。件数メタは曲数とバージョン数が一致するシリーズ（cue 1 つ = 1 曲のシリーズ）では「N 曲」のみ、異なるシリーズでは「N 曲 M ver.」と表示する（`MusicGenerator` 側で `CountsLabel` 文字列を事前整形）。曲数は `m_no_class` 単位（同一 class を共有する複数 cue は 1 曲扱い、class が NULL/空の cue は `m_no_detail` を独立キーにして 1 曲としてカウント）、バージョン数は cue 総数（仮 M 番号含む）。スタッフ表示は `.staff-badges-row` を直接当てたフラット構造で、主要作曲家・編曲家のグループを横並び（必要に応じて flex-wrap で折り返し）。各グループは「役職バッジ列 + メンバー名カンマ区切り」の固まりで、作曲と編曲のメンバーが同順序で完全一致するシリーズでは作曲・編曲バッジを 1 つの人物カードに連続して並べた「[作曲][編曲] 佐藤直紀」形に統合、一致しないシリーズでは「[作曲] X、Y」と「[編曲] Z」が同じ行内に横並びになる。主要スタッフは役職ごと（作曲・編曲）に、その役職に名前が入っている cue 数を母数とした担当割合 20% 以上の人物を頻度降順で抽出する（`MusicGenerator.BuildKeyStaffEntries`）。スタッフ名は `bgm_cues` 側が人物マスタと紐付かない自由記述列のためリンク化はしない。`BgmIndexRow` は `CountsLabel` と `StaffGroups`（`BgmStaffGroup[]`、各グループが `Roles[]` と `Members[]` を持つ）に整理。テーブル前提だった thead と連番列も廃止。
